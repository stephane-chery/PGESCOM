...
namespace PBsizing.Controllers
{
	public class Component_UPS
	{
		...
		public long recursifCall = 0;
		public string G_PRICE, Sell_PRICE;
		public string CAP1 = Charger_UPS.VIDE;
		public string CAP2 = Charger_UPS.VIDE;
		public string CAP3 = Charger_UPS.VIDE;
		public string CAP4 = Charger_UPS.VIDE;
		public string CAP5 = Charger_UPS.VIDE;
		public string CAP6 = Charger_UPS.VIDE;
		public string CAP7 = Charger_UPS.VIDE;
		...
		public string MainMDI_stMsgXP = "";

		public Component_UPS(string x_phs_out)
		{
			C = Charger_UPS.C;
			REAL_C = Charger_UPS.Real_C;
			P = x_phs_out;

				AvailId = Charger_UPS.AvailId;
		//	AvailId = x_AvailId;
			In_VcsTblName = "COMPUTE_VCS_UPS";
			G_PRICE = Charger_UPS.VIDE;
			Sell_PRICE = Charger_UPS.VIDE;

			G_Desc = Charger_UPS.VIDE;

		}

		public Component_UPS(string x_UPSmodel, string x_phs_out, string x_KVA, string x_ACout, string x_DCbus, string x_phs_in, string x_Acinput, string x_phs_bps, string x_bps_input, long x_Avail_id)
		{
			//
		...
		{
			string Res = "0";
			//		string stSql = (frml_ID == 0) ? "select * from " + In_VcsTblName + "  where VCS_NAME='" + FRML_NAME  : "select * from " + In_VcsTblName+ " where VCS_ID=" + frml_ID ;
			...
			else
			{
				...
				//MessageBox.Show("ERROR in CAL_VCS.....Many or No VCS_ID=" + frml_ID + " FNMA=" + FRML_NAME);
				Res = "0";
			}
			OConn.Close();
			return Res;
		...
		{
			if (frml.IndexOf("C_IDCC") > 0) frml = frml;
			...
			if (Res != "") return Res;
			// end added 
			if (frml == "U_Pi_1_3") frml = frml;
			long currFrml = find_FRML(0, frml);
			if (currFrml != -1)
			{
				string st = Charger_UPS.arr_FRML[currFrml, 4];
				//   MessageBox.Show ("FRML." + currFrml + " Car=" + Charger.arr_FRML[currFrml,2]);
				switch (Charger_UPS.arr_FRML[currFrml, 2])
				{
					case "S":
						...
						//	MessageBox.Show("ERROR In seeking Values in table= " + tableName + "\n" + Cols[1] + " " + Cols[2] + " " + Cols[3] + " " + Cols[4] + " " + Cols[5]);
						else
						{
							Charger_UPS.arr_CAL_FRML[Charger_UPS.ISave] = frml + "||" + Deco_Frml_Res;
							Charger_UPS.ISave++;
						}
						break;
					case "C":
		...
		private long find_FRML(long Vcs_ID, string VCS_name)
		{
			...
			int i = 0;
			if (Charger_UPS.arr_FRML[0, 0] != "")
			{
				while (i < Charger_UPS.NB_FRML && !found && Charger_UPS.arr_FRML[i, 1] != "")
				{
					if (Charger_UPS.arr_FRML[i, 1] == P || Charger_UPS.arr_FRML[i, 1] == "2")
					{
						if (VCS_name == "*") { if (Charger_UPS.arr_FRML[i, 0] == Vcs_ID.ToString()) found = true; }
						else if (Charger_UPS.arr_FRML[i, 3] == VCS_name) found = true;
					}
					find_FRML_Res = i++;
				}
			}
		...
		private string searchTBL6_RST(string tblName, string[] catN, string[] Cn)
		{
			...
			string searchTBL6_RST_Res = Charger_UPS.VIDE;
			string whr1 = "";
			string ord1 = "";
			for (int i = 0; i < 6; i++)
			{
		...
		//	string stSql = "SELECT " + MainMDI.t_SeekTBL6 + ".* FROM " + MainMDI.t_SeekTBL6 + " WHERE ((TABLE_NAME='" + tblName + "')" + whr1 + ")" + ord1;
		...
		private string calul_Amnt(string amnt1, string oper, string amnt2)
		{
			...
			int iiii = amnt2.Length;
			//	MessageBox.Show("st=" + amnt1.Length.ToString());
			double mnt1 = 0, mnt2 = 0;
			if (amnt1 == Charger_UPS.VIDE || amnt2 == Charger_UPS.VIDE) return "0";
			if (oper != "&")
			{
			...
			{
				case "*":
					calul_Amnt_Res = Convert.ToString(Math.Round(mnt1 * mnt2, Charger_UPS.NB_DEC_CAL));
					break;
				case "-":
					calul_Amnt_Res = Convert.ToString(Math.Round(mnt1 - mnt2, Charger_UPS.NB_DEC_CAL));
					break;
				case "/":
					if (mnt2 > 0) calul_Amnt_Res = Convert.ToString(Math.Round(mnt1 / mnt2, Charger_UPS.NB_DEC_CAL));
		...
		private string find_Cal_FRMLOLD(string st)
		{
			...
			for (int i = deb; i < Charger_UPS.NB_FRML; i++)
			{
				if (Charger_UPS.arr_CAL_FRML[i] != "") ipos = Charger_UPS.arr_CAL_FRML[i].IndexOf("||", 0);
				if (Charger_UPS.arr_CAL_FRML[i].Substring(0, ipos - 1) == st)
				{
					find_Cal_FRML_Res = Charger_UPS.arr_CAL_FRML[i].Substring(ipos + 2, Charger_UPS.arr_CAL_FRML[i].Length - 1 - ipos);
					i = Charger_UPS.NB_FRML;
				}
		...
		private string find_Cal_FRML(string st)
		{
			recursifCall++;
			int i = 1;
			while (Charger_UPS.arr_CAL_FRML[i] != "" && i < Charger_UPS.NB_FRML)
			{
				int ipos = Charger_UPS.arr_CAL_FRML[i].IndexOf("||", 0);
				if (ipos > -1)
				{
					if (Charger_UPS.arr_CAL_FRML[i].Substring(0, ipos) == st)
					{

						return Charger_UPS.arr_CAL_FRML[i].Substring(ipos + 2, Charger_UPS.arr_CAL_FRML[i].Length - (ipos + 2));
					}
				}
				i++;
			}
		...
		private string findRealV(string ndx, string cpt_id)
		{
			...
			for (int i = 0; i < Charger_UPS.IMAX_ARR_REALV; i++)
			{
				if (Charger_UPS.arr_REALV[i, 0] == "") i = Charger_UPS.IMAX_ARR_REALV;
				else
				{
					if (Charger_UPS.arr_REALV[i, 0] == cpt_id)
					{
						findRealV_Res = Charger_UPS.arr_REALV[i, Convert.ToInt32(ndx)];
						i = Charger_UPS.IMAX_ARR_REALV;
					}
				}
			}
		...
		public void Disp_FRMLS()
		{
			...
			for (int i = 0; i < Charger_UPS.NB_FRML; i++)
			{
				stOut = stOut + "\n";
				for (int j = 0; j < 5; j++)
					stOut = stOut + " / " + Charger_UPS.arr_FRML[i, j].ToString();
				if ((i % 20) == 0)
				{
		...
		{
			string stOut = "";
			for (int i = 0; i < Charger_UPS.NB_MODELS; i++)
			{
				stOut = stOut + "\n";
				for (int j = 0; j < Charger_UPS.NB_MODELS_Flds; j++)
					stOut = stOut + " / " + Charger_UPS.arr_CModels[i, j].ToString();
				if ((i % 20) == 0)
				{
					//	MessageBox.Show(stOut);
					MainMDI_stMsgXP = stOut;
...
//#########
		public string Cal_chrg_CostADO_Configo(long compntID, string code, string KAac, string KAdc)
		{

		//	 string KAac="0", KAdc= "0";
			string stout = "";
			//      string KAac = HttpContext.Session["kaac"].ToString(), KAdc = HttpContext.Session["kadc"].ToString();

			string Cal_chrg_CostADO_Res = MainMDI.VIDE;
			//	 bool rec_Empty = true;
			string c1 = "", c2 = "", c3 = "";  // To be Verified..... !!!!!!
			string val1 = "", val2 = "", val3 = "", val4 = ""; ;
			string prc = MainMDI.VIDE, stSql = "";

			if (AvailId != -1)
			{
				long cptid = (compntID == 226) ? 147 : compntID;
				stSql = "SELECT TBLAVAIL13_UPS.*, COMPNT_LIST_UPS.*,link_COMPNT_AVAIL_UPS.Qty " +
...
" Where (COMPNT_LIST_UPS.Component_ID = " + compntID + ") and (TBLAVAIL13_UPS.AvailID = " + AvailId + ") ORDER BY TBLAVAIL13_UPS.UPS, COMPNT_LIST_UPS.Component_ID";
				...
				while (Oreadr.Read())
				{
					...
					switch (cc)
					{
						...
						case "C":
							...
							deco_CRIT(Oreadr["nbc3Cat"].ToString(), ref c1, ref c2, ref c3, ref CAP4);
					//		Catn_Val_ado_Configo(Convert.ToInt32(AvailId), Convert.ToInt32(Oreadr["Value_Type"].ToString()), P, Oreadr["charger"].ToString(), Oreadr["vdc"].ToString(), Oreadr["idc"].ToString(), ref val1, ref val2, ref val3, ref Qty, ref cptPrice_id);
							Catn_Val_ado(Convert.ToInt32(AvailId), Convert.ToInt32(Oreadr["Value_Type"].ToString()), Oreadr["UPS"].ToString(), ref val1, ref val2, ref val3, ref val4, ref Qty, ref cptPrice_id, ref stout);
							...
							else if (Qty != MainMDI.VIDE) D_qty = Tools.Conv_Dbl(Qty).ToString();

							if (val1 == "0" || val2 == "0" || val3 == "0") return MainMDI.VIDE;


							if (compntID != 226 && compntID != 350 && compntID != 359)
								{
									double ddCustm_KA = Tools.Conv_Dbl(KAac);
									double dd_DEF_KA = 0, dd_KA = 0;
									if (ddCustm_KA > 0)
									{
										string DEF_prc = seekPrice_Configo_CBN_AC(compntID,val1, val2, val3, c1, c2, c3, code, D_qty, "", ref dd_DEF_KA);
										prc = seekPrice_Configo_CBN_AC(compntID,val1, val2, val3, c1, c2, c3, code, D_qty, KAac, ref dd_KA);
										...
										if (ddCustm_KA <= dd_DEF_KA)
										{
											...
											update_price_REALV(compntID, prc);

										}

									}
									else prc = seekPrice_Configo_CBN_AC(compntID,val1, val2, val3, c1, c2, c3, code, D_qty, KAac, ref dd_KA);
								...
								else
								{
									...
									if (ddCustm_KA > 0)
									{
										string DEF_prc = seekPrice_Configo_226(compntID,val1, val2, val3, c1, c2, c3, code, D_qty, "", ref dd_DEF_KA);
										prc = seekPrice_Configo_226(compntID,val1, val2, val3, c1, c2, c3, code, D_qty, KAdc, ref dd_KA);
									...
									else prc = seekPrice_Configo_226(compntID,val1, val2, val3, c1, c2, c3, code, D_qty, KAdc, ref dd_KA);
								}
								Real_QTY = D_qty;
						...
						case "Z":
							...
							break;
						case "%":
							G_PRICE = Convert.ToString(Math.Round(Tools.Conv_Dbl(Oreadr["Value_Type"].ToString()) * Tools.Conv_Dbl(Oreadr["Qty"].ToString()),2));
							Real_QTY = Oreadr["Qty"].ToString();
							break;
						case "$":

							string st_dollar = Cal_VCS(Int32.Parse(Oreadr["Value_Type"].ToString()), "*");
							G_PRICE = Convert.ToString(Math.Round(Tools.Conv_Dbl(st_dollar) * Tools.Conv_Dbl(Oreadr["Qty"].ToString()), 2));
							Real_QTY = Oreadr["Qty"].ToString();
							break;
						default:
		...
		{
			for (int ii = 0; ii < Charger_UPS.arr_REALV.Length; ii++)
			{
				if (Charger_UPS.arr_REALV[ii, 0] == cpt_id.ToString())
				{
					Charger_UPS.arr_REALV[ii, 7] = prc;
					ii = Charger_UPS.arr_REALV.Length + 1;
				}
			}

		}
		public string Cal_chrg_CostADO(long compntID, string code)
		{

			string Cal_chrg_CostADO_Res = Charger_UPS.VIDE;
			//	 bool rec_Empty = true;
			string c1 = "", c2 = "", c3 = "", c4 = "";  // To be Verified..... !!!!!!
			string val1 = "", val2 = "", val3 = "", val4 = "";
			string prc = Charger_UPS.VIDE, stSql = "";
			string stout = "";
			//if (AvailId != -1)
			stSql = "SELECT TBLAVAIL13_UPS.*, COMPNT_LIST_UPS.*,link_COMPNT_AVAIL_UPS.Qty " +
		   " FROM (TBLAVAIL13_UPS INNER JOIN link_COMPNT_AVAIL_UPS ON TBLAVAIL13_UPS.AvailID = link_COMPNT_AVAIL_UPS.Avail_ID) INNER JOIN COMPNT_LIST_UPS ON link_COMPNT_AVAIL_UPS.Compnt_ID = COMPNT_LIST_UPS.Component_ID " +
		   " Where (COMPNT_LIST_UPS.Component_ID = " + compntID + ") and (TBLAVAIL13_UPS.AvailID = " + AvailId + ") ORDER BY TBLAVAIL13_UPS.UPS, COMPNT_LIST_UPS.Component_ID";
			...
			//               " FROM  COMPNT_LIST WHERE Component_ID =" + compntID + " ORDER BY Component_ID";
			...
			while (Oreadr.Read())
			{
				...
				switch (cc)
				{
					...
					case "S":
					case "C":
						deco_CRIT(Oreadr["nbc3Cat"].ToString(), ref c1, ref c2, ref c3, ref c4);
						Catn_Val_ado(Convert.ToInt32(AvailId), Convert.ToInt32(Oreadr["Value_Type"].ToString()), Oreadr["UPS"].ToString(), ref val1, ref val2, ref val3, ref val4, ref Qty, ref cptPrice_id, ref stout);
						//display results
						...
						//if (Qty == "0") D_qty = Oreadr["Qty"].ToString ();
						//else  if (Qty != Charger_UPS.VIDE)  D_qty = Tools.Conv_Dbl(Qty).ToString ();
						...
						break;
					case "Z":
						val1 = Charger_UPS.VIDE;
						val2 = Charger_UPS.VIDE;
						val3 = Charger_UPS.VIDE;
						val4 = Charger_UPS.VIDE;
						prc = seekPriceADO(Convert.ToInt32(Oreadr["Component_ID"]), Convert.ToInt32(Oreadr["Component_ID"]), val1, val2, val3, "=", "=", "=", code, D_qty);   //* Conv_Dbl(rstAvail!Qty)
						break;
					case "%":
						G_PRICE = Convert.ToString(Math.Round(Tools.Conv_Dbl(Oreadr["Value_Type"].ToString()) * Tools.Conv_Dbl(Oreadr["Qty"].ToString()), Charger_UPS.NB_DEC_AFF));
						Real_QTY = Oreadr["Qty"].ToString();
						break;
					case "$":

						string st_dollar = Cal_VCS(Int32.Parse(Oreadr["Value_Type"].ToString()), "*");
						G_PRICE = Convert.ToString(Math.Round(Tools.Conv_Dbl(st_dollar) * Tools.Conv_Dbl(Oreadr["Qty"].ToString()), Charger_UPS.NB_DEC_AFF));
						Real_QTY = Oreadr["Qty"].ToString();
						break;
					default:
					//	MessageBox.Show("ERROR in Cost Process... bad Compnt_Type=  " + Oreadr["Compnt_Type"]);
						MainMDI_stMsgXP += " ### " + "ERROR in Cost Process... bad Compnt_Type=  " + Oreadr["Compnt_Type"];
						prc = "0";
						break;
				}
		...
		private string seekPrice_Configo_226(long mycptID, string cat1V, string cat2V, string cat3V, string c1, string c2, string c3, string code, string D_qty, string KAdc, ref double KAfound)
		{


			string seekPriceADO_Res = Charger_UPS.VIDE;
			...
			CAP2 = cat2V;
			CAP3 = cat3V;
			string prc = Charger_UPS.VIDE;
			// if (c1 == "")  MessageBox.Show("ERoooooooooooooooooooooooooRRR c1 is EMPTY in SeekingPrice....");
			if (cat1V != Charger_UPS.VIDE && cat2V != Charger_UPS.VIDE && cat3V != Charger_UPS.VIDE && D_qty != Charger_UPS.VIDE)
			{
				if (c1 == "=")
				{
					whr1 = " ([PHASE]='" + cat1V + "') ";
				...
				string stSql = "SELECT *   FROM Configo_CB2xx_UPS  where " + whr1 + whr2 + whr3 + whr4 + ord1 + ord2 + ord3 + ord4 + ",Priority  ";
				SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
				OConn.Open();
				SqlCommand Ocmd = OConn.CreateCommand();
				Ocmd.CommandText = stSql;
				...
				while (Oreadr.Read() && !fin)
				{
					...
					//   prc = Convert.ToString(Math.Round(Tools.Conv_Dbl(Oreadr["Cost Price"].ToString()) * Tools.Conv_Dbl(D_qty), Charger_UPS.NB_DEC_AFF)); //G_Price=COST_Price
					prc = Convert.ToString(Math.Round(Tools.Conv_Dbl(Oreadr["Sell Price"].ToString()) * Tools.Conv_Dbl(D_qty), Charger_UPS.NB_DEC_AFF));
					...
					//2020
					kafnd = (KAdc == "") ? "" : Oreadr[VSCnm.Replace("[", "").Replace("]", "")].ToString();
					//2020


					//old
					// majREALV(147, Oreadr["PHASE"].ToString(), Oreadr["VAC"].ToString(), Oreadr["ICB1"].ToString(),"","","", prc, Desc);
					//		majREALV(226, Oreadr["PHASE"].ToString(), Oreadr["VDC"].ToString(), Oreadr["ICB1"].ToString(), cat2V, Oreadr["ICB1"].ToString(), tt, prc, newDesc);
					majREALV_UPS(mycptID, Oreadr["PHASE"].ToString(), Oreadr["VDC"].ToString(), Oreadr["ICB1"].ToString(), cat2V, Oreadr["ICB1"].ToString(), KAdc, kafnd, prc, newDesc);
				...
				if (!found)
				{
					...
					//	majREALV(226, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, "-99999", "Breaker NOT FOUND.... please call PRIMAX ... phs="+ cat1V +"/ vac=" + cat2V + "/ icb=" + cat3V);
					majREALV_UPS(mycptID, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, cat2V, Charger_UPS.VIDE, KAdc, kafnd, "-99999", "Breaker NOT FOUND.... please call PRIMAX ... phs=" + cat1V + "/ vac=" + cat2V + "/ icb=" + cat3V);
				}
				OConn.Close();
			...
			else
			{
				...
				majREALV(226, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE);
			}
		...
		private string seekPrice_Configo_CBN_AC(long mycptID, string cat1V, string cat2V, string cat3V, string c1, string c2, string c3, string code, string D_qty, string KAac, ref double KAfound)
		{

	
			string seekPriceADO_Res = MainMDI.VIDE;
			string whr1 = "", whr2 = "", whr3 = "", whr4 = "", ord1 = "", ord2 = "", ord3 = "", ord4 = "";
			string kafnd = "0";
			string pref = "0";
			string Desc = "";
			...
			if (cat1V != Charger_UPS.VIDE && cat2V != Charger_UPS.VIDE && cat3V != Charger_UPS.VIDE && D_qty != Charger_UPS.VIDE)
			{
				...
				string stSql = "SELECT *   FROM Configo_CBxx  where " + whr1 + whr2 + whr3 + whr4 + ord1 + ord2 + ord3 + ord4 + ",Priority  ";
				SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
				OConn.Open();
				SqlCommand Ocmd = OConn.CreateCommand();
				Ocmd.CommandText = stSql;
			...
			//2020
				 kafnd = (KAac == "") ? "": Oreadr[VSCnm.Replace("[", "").Replace("]", "")].ToString();
           //2020
					//old
					// majREALV(147, Oreadr["PHASE"].ToString(), Oreadr["VAC"].ToString(), Oreadr["ICB1"].ToString(),"","","", prc, Desc);

					majREALV_UPS(mycptID, Oreadr["PHASE"].ToString(), Oreadr["VAC"].ToString(), Oreadr["ICB1"].ToString(), cat2V, Oreadr["ICB1"].ToString(), KAac, kafnd, prc, newDesc);
				...
				if (!found)
				{
					...
					//   majREALV(147, cat1V, cat2V, cat3V, Charger.VIDE, Charger.VIDE, Charger.VIDE, "n/e","Special Breaker: please call PRIMAX ");
					//	majREALV(mycptID, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, "-99999", "Breaker NOT FOUND.... please call PRIMAX or change UPS Values.. ");
					majREALV_UPS(mycptID, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, cat2V, Charger_UPS.VIDE, KAac, kafnd, "-99999", "Breaker NOT FOUND.... please call PRIMAX ... phs=" + cat1V + "/ vac=" + cat2V + "/ icb=" + cat3V);
			...
			{
				//MessageBox.Show  ("Price Cat#1 Empty...." + "CPT_ID= " + Orig_compnt_ID + "    cat1=" + cat1V + "  cat2=" + cat2V + "  cat3=" + cat3V + "  QTY=" + D_qty + "/ ");
				//majREALV(mycptID, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE);
				majREALV_UPS(mycptID, cat1V, cat2V, cat3V, KAac, kafnd, Charger_UPS.VIDE, Charger_UPS.VIDE, "-99999", "Breaker NOT FOUND.... please call PRIMAX ... phs=" + cat1V + "/ vac=" + cat2V + "/ icb=" + cat3V);
			}

			//else majREALV(147, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger.VIDE, Charger.VIDE, Charger.VIDE,Charger.VIDE);
		...
		private string seekPrice_Configo_147(string cat1V, string cat2V, string cat3V, string c1, string c2, string c3, string code, string D_qty, string KAac, ref double KAfound)
		{
			...
			if (cat1V != Charger_UPS.VIDE && cat2V != Charger_UPS.VIDE && cat3V != Charger_UPS.VIDE && D_qty != Charger_UPS.VIDE)
			{
				...
				if (!found)
				{
					...
					//   majREALV(147, cat1V, cat2V, cat3V, Charger.VIDE, Charger.VIDE, Charger.VIDE, "n/e","Special Breaker: please call PRIMAX ");
					majREALV(147, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, "-99999", "Breaker NOT FOUND.... please call PRIMAX or change Charger_UPS Values.. ");
			...
			{
				//MessageBox.Show  ("Price Cat#1 Empty...." + "CPT_ID= " + Orig_compnt_ID + "    cat1=" + cat1V + "  cat2=" + cat2V + "  cat3=" + cat3V + "  QTY=" + D_qty + "/ ");
				majREALV(147, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE);
			}
		...
		private string seekPrice_Configo_226_inCBxxx(long mycptID, string cat1V, string cat2V, string cat3V, string c1, string c2, string c3, string code, string D_qty, string KAdc, ref double KAfound)
		{
			...
			if (cat1V != Charger_UPS.VIDE && cat2V != Charger_UPS.VIDE && cat3V != Charger_UPS.VIDE && D_qty != Charger_UPS.VIDE)
			{
				...
				while (Oreadr.Read() && !fin)
				{
					...
					string newDesc = (KAdc == "") ? Oreadr["ICB1"].ToString() + "A - " + Oreadr["VAC"].ToString() + "V dc" + KA : Oreadr["ICB1"].ToString() + "A - " + Oreadr["VAC"].ToString() + "V dc - " + Oreadr[VSCnm.Replace("[", "").Replace("]", "")].ToString() + "KA IC";

					//2020
					kafnd = (KAdc == "") ? "" : Oreadr[VSCnm.Replace("[", "").Replace("]", "")].ToString();
					//2020

     		majREALV_UPS(mycptID, Oreadr["PHASE"].ToString(), Oreadr["VAC"].ToString(), Oreadr["ICB1"].ToString(), cat2V, Oreadr["ICB1"].ToString(), KAdc, kafnd, prc, newDesc);
					seekPriceADO_Res = prc;
					fin = true;
					found = true;
				}
		...
		private string seekPrice_Configo_226_good29062020(long mycptID, string cat1V, string cat2V, string cat3V, string c1, string c2, string c3, string code, string D_qty, string KAdc, ref double KAfound)
		{
		...
		//	string stSql="";
		//         if (AvailId !=-1) stSql= "SELECT TBLAVAIL" + P + ".Avail_ID, COMPNT_LIST.Component_ID,COMPNT_LIST.COMPONENT_REF, TBLAVAIL" + P + ".charger, TBLAVAIL" + P + ".vdc, TBLAVAIL" + P +  ".idc, link_COMPNT_AVAIL.Qty, " +
		...
		//              " FROM         COMPNT_LIST WHERE     Component_ID =" +  dccompnt + " ORDER BY Component_ID	";
		//	SqlConnection OConn  = new SqlConnection(MainMDI.M_stCon  );
		//	OConn.Open ();
		//	SqlCommand Ocmd = OConn.CreateCommand();
		//	Ocmd.CommandText = stSql ;
		...
		private string seekPriceADO(long Orig_compnt_ID, long Price_compnt_ID, string cat1V, string cat2V, string cat3V, string c1, string c2, string c3, string code, string D_qty)
		{
			...
			if (cat1V != Charger_UPS.VIDE && cat2V != Charger_UPS.VIDE && cat3V != Charger_UPS.VIDE && D_qty != Charger_UPS.VIDE)
			{
				...
				while (Oreadr.Read())
				{
					...
					if (pref == "0")
					{
						...
						prc = Convert.ToString(Math.Round(Tools.Conv_Dbl(Oreadr["Cost_Price"].ToString()) * Tools.Conv_Dbl(D_qty), Charger_UPS.NB_DEC_AFF)); //G_Price=COST_Price
			...
			{
				//MessageBox.Show  ("Price Cat#1 Empty...." + "CPT_ID= " + Orig_compnt_ID + "    cat1=" + cat1V + "  cat2=" + cat2V + "  cat3=" + cat3V + "  QTY=" + D_qty + "/ ");
				majREALV(Orig_compnt_ID, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE);
			}
			if (pref != "0")
			{
				seekPriceADO_Res = prc;
				if (prc == "0")
				{
					...
					majREALV(Orig_compnt_ID, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, "n/e", Charger_UPS.VIDE);
				}
			}
			else majREALV(Orig_compnt_ID, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE);
		...
		{
			int i = Charger_UPS.lblIRealV;

			CAP1 = v1; CAP2 = v2; CAP3 = v3; CAP4 = v4; CAP5 = v5; CAP6 = v6;
			Charger_UPS.arr_REALV[i, 0] = cpt_id.ToString();
			Charger_UPS.arr_REALV[i, 1] = v1; 
			Charger_UPS.arr_REALV[i, 2] = v2;
			Charger_UPS.arr_REALV[i, 3] = v3;
			Charger_UPS.arr_REALV[i, 4] = v4;
			Charger_UPS.arr_REALV[i, 5] = v5;
			Charger_UPS.arr_REALV[i, 6] = v6;
			Charger_UPS.arr_REALV[i, 7] = prc;
			G_PRICE = prc;
			G_Desc = Desc;
			Charger_UPS.lblIRealV = ++i;

		}

		private void majREALV_UPS(long cpt_id, string v1, string v2, string v3, string v4, string v5, string v6, string v7 ,string prc, string Desc)
		{
			int i = Charger_UPS.lblIRealV;

			CAP1 = v1; CAP2 = v2; CAP3 = v3; CAP4 = v4; CAP5 = v5; CAP6 = v6;
			
			Charger_UPS.arr_REALV[i, 0] = cpt_id.ToString();CAP7 = v7;
			Charger_UPS.arr_REALV[i, 1] = v1;
			Charger_UPS.arr_REALV[i, 2] = v2;
			Charger_UPS.arr_REALV[i, 3] = v3;
			Charger_UPS.arr_REALV[i, 4] = v4;
		...
		private void deco_CRIT(string st, ref string c1, ref string c2, ref string c3, ref string c4)
		{
			c1 = "";
			c2 = "";
			c3 = "";
			...
			if (st.Length >= 8) c4 = deco_oper(st.Substring(7, 1));
		}
		...
		/// 
		/// </summary>
		/// <param name="Avail_id"></param>
		/// <param name="CM_ID"></param>
		...
		/// <param name="cptPrice_id"></param>
		/// <param name="stout"></param>
		private void Catn_Val_ado(long Avail_id, int CM_ID, string UPS, ref string v1, ref string v2, ref string v3, ref string v4, ref string Qty, ref long cptPrice_id, ref string stout)
		{
			string st;
			v1 = "0";
			v2 = "0";
			...
			Qty = "0";
			bool rec_Empty = true;
			string stSql = "select * from COMPUTE_MODELS_UPS where CM_ID=" + CM_ID;
			SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
			OConn.Open();
			SqlCommand Ocmd = OConn.CreateCommand();
			Ocmd.CommandText = stSql;
			...
			while (Oreadr.Read())
			{
				...
				stout += "  Vcs1=" + Oreadr["VCS1"].ToString() + " / " + v1 + "\n"; ;
				...
				stout += "  Vcs2=" + Oreadr["VCS2"].ToString() + " / " + v2 + "\n"; ;
				...
				stout += "  Vcs3=" + Oreadr["VCS3"].ToString() + " / " + v3 + "\n"; ;

				//vcs4 added for ups
				if (Oreadr["VCS4"].ToString() != "0")
				{
					if (Oreadr["TYPcat4"].ToString().Substring(0, 1) == "C")
						v4 = Cal_VCS(Convert.ToInt32(Oreadr["VCS4"].ToString()), "*");
					else
					{
						st = Oreadr["TYPcat4"].ToString();
						v4 = findRealV(st.Substring(1, 1), st.Substring(3, st.Length - 3));
						// v3 = findRealV(d(st, 2, 1), CLng(Mid(st, 4, Len(st) - 2)))
						if (v4 == "0")
						{
							Cal_chrg_CostADO(Convert.ToInt32(st.Substring(3, st.Length - 3)), st.Substring(0, 2));
				...
				stout += "  Vcs4=" + Oreadr["VCS4"].ToString() + " / " + v4;
			...
			//if (rec_Empty) MessageBox.Show("ERROR IN Seeking CModels ... Many or NO Model...check...");
		}
		//UPS
		private string deco_oper(string st)
		{
			string tt = Charger_UPS.VIDE;
			switch (st)
			{
				case "<=":
					tt = "(";
		...
		public string FRML_CostADO(long compntID, string code)
		{
			...
			//          " COMPNT_LIST.CAT1_TABLE_ID, COMPNT_LIST.CAT2_TABLE_ID, COMPNT_LIST.CAT3_TABLE_ID, COMPNT_LIST.Compnt_Type, COMPNT_LIST.Value_Type, COMPNT_LIST.nbc3Cat " +
			//          " FROM (TBLAVAIL" + P + " INNER JOIN link_COMPNT_AVAIL ON TBLAVAIL" + P + ".Avail_ID = link_COMPNT_AVAIL.Avail_ID) INNER JOIN COMPNT_LIST ON link_COMPNT_AVAIL.Compnt_ID = COMPNT_LIST.Component_ID " +
			//          " Where (((link_COMPNT_AVAIL.phs) = '" + P + "') and ((COMPNT_LIST.Component_ID) = " + compntID + ") and ((TBLAVAIL" + P + ".Avail_ID) = " + AvailId + ")) ORDER BY TBLAVAIL" + P + ".charger,cast(TBLAVAIL" + P + ".vdc AS float),cast(TBLAVAIL" + P + ".idc AS float ), COMPNT_LIST.Component_ID";
			//     else stSql = " SELECT  Component_ID, COMPONENT_REF, '" + Charger.C + "' AS charger, '" + Charger.V + "' AS vdc, '" + Charger.I + "' AS idc, 1 AS Qty, CAT1_TABLE_ID, CAT2_TABLE_ID, CAT3_TABLE_ID, Compnt_Type, Value_Type, nbc3Cat, '" + Charger.P + "' AS phs " +
			//" FROM  COMPNT_LIST WHERE Component_ID =" + compntID + " ORDER BY Component_ID";

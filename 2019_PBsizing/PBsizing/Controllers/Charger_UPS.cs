...
namespace PBsizing.Controllers
{
	public class Charger_UPS
	{
		...
		public string in_UPSmodel, in_phs_out, in_KVA, in_ACout, in_DCbus, in_phs_in, in_Acinput, in_phs_bps1, in_bps_input1, in_phs_bps2, in_bps_input2, in_Cbatt, in_PF, 
			in_tcharg, in_FLT, in_EQLZ, in_vdcmin, in_vdcmax,in_freqin, in_freqout, in_CBnkaAC, in_idci, in_ah, in_std;
		...
		public Charger_UPS(long x_AvailID, string x_UPSmodel, string x_phs_out, string x_KVA, string x_ACout, string x_DCbus, string x_phs_in,
						   string x_Acinput, string x_phs_bps1, string x_bps_input1, string x_phs_bps2, string x_bps_input2, string x_Cbatt, string x_PF, string x_tcharg, string x_FLT, string x_EQLZ, 
						   string x_vdcmin, string x_vdcmax,string  frqin, string frqout, string cbnkaac,string  x_idci, string x_ah, string x_std)
		{
			//
			...
			in_UPSmodel = x_UPSmodel;// + "-" +in_phs_out + "-" +in_KVA + "-" +in_ACout+ "-" +in_DCbus;
			...
			in_phs_bps1 = x_phs_bps1;
			in_bps_input1 = x_bps_input1;
			in_phs_bps2 = x_phs_bps2;
			in_bps_input2 = x_bps_input2;
			in_Cbatt = x_Cbatt;
			in_PF = x_PF;
			in_tcharg = x_tcharg;
			in_FLT = x_FLT;
		...
		private long Find_AvailID()
		{
			...
			string stSql = "SELECT AVAILID FROM TBLAVAIL13_UPS  WHERE (UPS='" + in_UPSmodel + "' AND PHSout='" + in_phs_out + "' AND OP_KVA='" + in_KVA + "' AND ACoutputV='" + in_ACout + "' AND DCBus='" + in_DCbus + "' AND PHSin='" + in_phs_in + "' AND ACinputV='" + in_Acinput + "' AND PHSbps='" + in_phs_bps1 + "' AND BpsinputV='" + in_bps_input1 + "')";
		...
		private bool Load_arr_FRML(string VcsTblName)
		{
			...
			SqlDataReader Oreadr = Ocmd.ExecuteReader();
			int i = 0;
			if (Oreadr.FieldCount != NB_FRML_Flds) MainMDI_stMsgXP = "VCS Table is Empty or Fields# is Invalid....must be= " + NB_FRML_Flds.ToString();
			//MessageBox.Show("VCS Table is Empty or Fields# is Invalid....must be= " + NB_FRML_Flds.ToString());
			while (Oreadr.Read())
			{
		...
		private void init_arr_CAL_FRML()
		{
			...
			arr_CAL_FRML[8] = "U_PHSbps1||" + in_phs_bps1;
			arr_CAL_FRML[9] = "U_BPSinput1||" + in_bps_input1;

			arr_CAL_FRML[10] = "U_Cbat||" + in_Cbatt;
			arr_CAL_FRML[11] = "U_PF||" + in_PF;
			arr_CAL_FRML[12] = "U_Tcharg||" + in_tcharg;
			arr_CAL_FRML[13] = "U_IDCI||" + in_idci;
			arr_CAL_FRML[14] = "U_AH||" + in_ah;
			arr_CAL_FRML[15] = "U_STD||" + in_std;
			//		arr_CAL_FRML[16] = (in_phs_out == "3") ? "U_Pi_1_3||1.35047" : "U_Pi_1_3||0.900316";// + "‭0.900316‬";
			double dd = Math.Round( Tools.Conv_Dbl(in_KVA) * Tools.Conv_Dbl(in_PF),5);
			arr_CAL_FRML[16] = "U_PLOAD||" + dd.ToString();// + "‭0.900316‬";
			arr_CAL_FRML[17] = "U_VFLOAT||" + in_FLT;
			arr_CAL_FRML[18] = "U_VEQLZ||" + in_EQLZ;
			arr_CAL_FRML[19] = "U_VDCMIN||" + in_vdcmin;
			arr_CAL_FRML[20] = "U_VDCMAX||" + in_vdcmax;
			arr_CAL_FRML[21] = "U_MX_AHSTD||" + Math.Max (Tools.Conv_Dbl( in_std), Tools.Conv_Dbl(in_ah));
			arr_CAL_FRML[22] = "U_PHSbps2||" + in_phs_bps2;
			arr_CAL_FRML[23] = "U_BPSinput2||" + in_bps_input2;

...
namespace PGESCOM
{
	...
	/// </summary>
	public class Order_TRPrint
	{
		private bool newP = false;

		private string[,] ar_IOTST = new string[100, 7];
		private string[,] ar_LoaddTST = new string[100, 7];
		private string[,] ar_EQ_AL_TST = new string[100, 12];
		...
		//public Order_TRPrint(Order x_RDR)

		public Order_TRPrint(string[,] x_ar_IOTST, string[,] x_ar_LoaddTST, string[,] x_ar_EQ_AL_TST, string[] x_ar_Info)
		{
			ar_IOTST = x_ar_IOTST;
			ar_LoaddTST = x_ar_LoaddTST;
			ar_EQ_AL_TST = x_ar_EQ_AL_TST;
			//In_RDR = x_RDR;
			MessageBox.Show("1 col= " + ar_IOTST[1, 1]);
		...
		public bool TRexport()
		{
			...
			MessageBox.Show("endof Print_IO_tst");
			Print_Perf_tst(); //MessageBox.Show("Print_Perf_tst");
			MessageBox.Show("endof Print_Perf_tst");
			//Print_EQ_AL_OT()
			Print2L_EQ_AL_OT(); //MessageBox.Show("Print2L_EQ_AL_OT");
			MessageBox.Show("endof Print2L_EQ_AL_OT");
			//Print_Botm_Info();
		...
		private void Print_HTBL10(bool pg)
		{
			...
			WPrint10Cols('N', true, "Description (Symbol) ", "Adjust", "Delay", "Relay #", "TimeOut", "Msg Latch", "Relay Latch", "Fail Safe", 
				" Comments ");
		}
		...
		private void Print2L_EQ_AL_OT()
		{
			...
			{ 
				...
				{			
					...
					WPrint10Cols('R', (ar_EQ_AL_TST[i, 0] == "8"), ar_EQ_AL_TST[i, 1], ar_EQ_AL_TST[i, 2], ar_EQ_AL_TST[i, 4], 
						ar_EQ_AL_TST[i, 6], ar_EQ_AL_TST[i, 8], ar_EQ_AL_TST[i, 10], ar_EQ_AL_TST[i, 12], ar_EQ_AL_TST[i, 14], 
						ar_EQ_AL_TST[i, 16]);
					//print Tested values 
					...
					WPrint10Cols('T', (ar_EQ_AL_TST[i, 0] == "8"), ar_EQ_AL_TST[i, 1], ar_EQ_AL_TST[i, 3], ar_EQ_AL_TST[i, 5], 
						ar_EQ_AL_TST[i, 7], ar_EQ_AL_TST[i, 9], ar_EQ_AL_TST[i, 11], ar_EQ_AL_TST[i, 13], ar_EQ_AL_TST[i, 15], 
						ar_EQ_AL_TST[i, 16]);
				}
				else break;
			}
		...
		private void Print_Head_Info()
		{
			...
			string st = (In_RDR.tcust_Model.Text != MainMDI.VIDE) ?In_RDR.PX_Model.Text + "(" + In_RDR.tcust_Model.Text + ")" : 
				In_RDR.PX_Model.Text;
			WPmsg("\nModel: ", 'b', false); 
			WPmsg(st, 'n', false);
		...
		private void Print_IO_tst()
		{
			...
			{ 
				...
				if (ar_IOTST[i, 0] != "")
				{
					if (i == 0)
					{
						WPmsg("\n", 'b', false); WPmsg("INPUT-OUTPUT", 'b', false);
					...
					else
					{
						...
						else op = 'C'; //WPrint5Cols('C', (ar_IOTST[i, 0] == "8"), In_RDR.lvIOTest.Items[i].SubItems[1].Text, In_RDR.lvIOTest.Items[i].SubItems[2].Text, In_RDR.lvIOTest.Items[i].SubItems[3].Text, In_RDR.lvIOTest.Items[i].SubItems[4].Text);
					} 
					WPrint5Cols(op, (ar_IOTST[i, 0] == "8"), ar_IOTST[i, 1], ar_IOTST[i, 2], ar_IOTST[i, 3], ar_IOTST[i, 4]);
				}
				else break;
			}
		}
			...
			for (int i = 0; i < 100; i++)
			{
				if (ar_LoaddTST[i, 0] != "")
				{
					...
					else
					{
						if (newP)
						{
							op = 'N'; //WPrint5Cols('N', (In_RDR.lvLTest.Items[i].ImageIndex == 8), In_RDR.lvLTest.Items[i].SubItems[1].Text, In_RDR.lvLTest.Items[i].SubItems[2].Text, In_RDR.lvLTest.Items[i].SubItems[3].Text, In_RDR.lvLTest.Items[i].SubItems[4].Text);
							newP = false;
						}
						else op = 'C'; //WPrint5Cols('C', (In_RDR.lvLTest.Items[i].ImageIndex == 8), In_RDR.lvLTest.Items[i].SubItems[1].Text, In_RDR.lvLTest.Items[i].SubItems[2].Text, In_RDR.lvLTest.Items[i].SubItems[3].Text, In_RDR.lvLTest.Items[i].SubItems[4].Text);
					}
					WPrint5Cols(op, (ar_LoaddTST[i, 0] == "8"), ar_LoaddTST[i, 1], ar_LoaddTST[i, 2], ar_LoaddTST[i, 3], ar_LoaddTST[i, 4]);
					//WPrint5Cols('C', (ar_LoaddTST[i, 0] == "8"), In_RDR.lvLTest.Items[i].SubItems[1].Text, In_RDR.lvLTest.Items[i].SubItems[2].Text, In_RDR.lvLTest.Items[i].SubItems[3].Text, In_RDR.lvLTest.Items[i].SubItems[4].Text);
				}
				else break;
			}
		}

...
namespace PGESCOM
{
    public partial class CF_PrintSetup : Form
    {
        ...
        private void fill_ConfOLD(string _IrrevLID, string _CFlid)
        {
            ...
            string stSql = (newSetup) ? "select e.ConfigNm, f.d_ItemDesc,f.CfDet_LID,f.d_Rnk" +
                " from PSM_R_CFinfo e inner join PSM_R_CFDetail f on f.d_CFLID=e.CFLID " +
                " where e.CFLID=" + _CFlid + " and e.c_RRevLID=" + _IrrevLID + 
                " order by f.d_Rnk "
                : " SELECT  e.ConfigNm, f.d_ItemDesc, f.CfDet_LID,f.d_Rnk,g.su_LID,g.Bold" +
                " FROM  PSM_R_CFDetail AS f INNER JOIN  PSM_R_CFPSetup AS g ON f.CfDet_LID = g.su_CFdetLID " +
                "        INNER JOIN  PSM_R_CFinfo AS e ON f.d_CFLID = e.CFLID " +
                " WHERE  (e.CFLID = " + _CFlid + ") AND (e.c_RRevLID = " + _IrrevLID + ")" +
                " ORDER BY g.su_Rnk ";
            ...
            while (Oreadr.Read())
            {
                ...
                if (!newSetup)
                {
                    ...
                    lv.SubItems[0].Font = (Oreadr["Bold"].ToString() == "True") ? new Font(lvCurConfig.Font, FontStyle.Bold) : 
                        new Font(lvCurConfig.Font, FontStyle.Regular);
                }
        ...
        private void fill_Conf(string _IrrevLID, string _CFlid)
        {
            ...
            string Col1 = "", Col2 = "", Col3 = "";
            string stSql = "select e.ConfigNm, f.d_ItemDesc,f.CfDet_LID,f.d_Rnk ,f.d_Rnk,f.su_Rnk,f.Bold" +
                " from PSM_R_CFinfo e inner join PSM_R_CFDetail f on f.d_CFLID=e.CFLID " +
                " where e.CFLID=" + _CFlid + " and e.c_RRevLID=" + _IrrevLID + 
                " order by f.su_Rnk ";
        ...
        private void lvCurConfig_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            lvCurConfig.Items[ndx].Font = (!lvCurConfig.Items[ndx].Checked) ? new Font(lvCurConfig.Font, FontStyle.Bold) : 
                new Font(lvCurConfig.Font, FontStyle.Regular);
        }
        ...
        private void vider_arr_CFsu()
        {
            for (int i = 0; i < MainMDI.MAX_Quote_lines; i++)
                for (int j = 0; j < 4; j++) arr_CFsu[i, j] = "~";
            LstNdx = 0;
        }
        ...
        private void CutCopy(char c)
        {
            ...
            LstNdx = i;
            if (c == 'D') 
                while (lvCurConfig.SelectedItems.Count > 0) lvCurConfig.Items[lvCurConfig.SelectedItems[0].Index].Remove();
            //aff();
        ...
        private void Bold()
        {
            for (int ndx = 0; ndx < lvCurConfig.SelectedItems.Count; ndx++)
            {
                ...
                lvCurConfig.Items[i].Font = (lvCurConfig.Items[i].SubItems[0].Text == "0") ? new Font(lvCurConfig.Font, FontStyle.Bold) : 
                    new Font(lvCurConfig.Font, FontStyle.Regular);
        ...
        private void paste(int InsertNdx)
        {
            ...
            {
                K++;
                for (int j = 0; j < lvCurConfig.Items[i].SubItems.Count; j++) arr_CFsu[K, j] = lvCurConfig.Items[i].SubItems[j].Text;
        ...
        void ref_BOLD()
        {
            for (int j = 0; j < lvCurConfig.Items.Count; j++) lvCurConfig.Items[j].Font = (lvCurConfig.Items[j].SubItems[0].Text == "1") ? 
                    new Font(lvCurConfig.Font, FontStyle.Bold) : new Font(lvCurConfig.Font, FontStyle.Regular);
        }
                    ...
                    if (lvCurConfig.Items.Count > 0)
                    {
                        for (int i = 0; i < lvCurConfig.Items.Count; i++)
                        {
                            string stSql = "UPDATE PSM_R_CFDetail SET " +
                                " [su_Rnk]='" + i.ToString() +
                                "', [Bold]='" + lvCurConfig.Items[i].SubItems[0].Text + "' WHERE CfDet_LID=" + 
                                lvCurConfig.Items[i].SubItems[2].Text;
                        ...
                        else //Save Changes made in RRev Info
                        {
                            for (int i = 0; i < lvCurConfig.Items.Count; i++)
                            {
                                string stSql = "UPDATE PSM_R_CFPSetup SET " + " [su_Rnk]='" + i.ToString() +
                                    "', [Bold]='" + lvCurConfig.Items[i].SubItems[0].Text + "' WHERE su_CFdetLID=" + 
                                    lvCurConfig.Items[i].SubItems[2].Text;
                                MainMDI.Exec_SQL_JFS(stSql, " Update new setup for CF=" + lConfigName.Text);
                            }
                        }
        ...
        {
            MainMDI.Exec_SQL_JFS("update PSM_R_CFDetail set [su_Rnk]=[d_Rnk], [Bold]= 0 where d_CFLID=" + in_CFlid, 
                " Init new CF setup....CF=" + lConfigName.Text);

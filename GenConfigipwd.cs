    ...
    public partial class GenConfigipwd : Form
    {
        ...
        private void Fill_users(string compID)
        {
            ...
            while (Oreadr.Read())
            {
                ...
                lv.SubItems.Add(Oreadr["email"].ToString());
            }
            OConn.Close();
        }
        ...
        private void fill_Companies()
        {
            ...
            MainMDI.fill_Any_CB(cbcompanies, stsql, true, "Select Company"); //"ALL Companies");
        }

        private void GenConfigipwd_Load(object sender, EventArgs e)
        {
            Del_itm.Visible = MainMDI.User.ToLower() == "ede";
            find.Visible = MainMDI.User.ToLower() == "ede";
            fill_Companies();
        }

        private void cbcompanies_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbcompanies.Text == "Select Company") //"ALL Companies")
            {
                lcompID.Text = "0";
        ...
        private void pictureBox1_Click_1(object sender, EventArgs e)
        {
            ...
            Clipboard.SetText("user Name: " + txuser.Text + "    password: " + txpwd.Text, TextDataFormat.Text);
        }

        private void NewItm_Click(object sender, EventArgs e)
        {
            pan_find.Visible = false;
            if (!pan_new.Visible)
            {
                if (cbcompanies.Text != "Select Company") //"ALL Companies")
                {
                ...
                if (usrLID.Text == "")
                {
                    if (!User_exist(txuser.Text, lcompID.Text))
                    {
                        string stSql = " INSERT INTO configo_Usetup ([customersp] ,[userid] ,[usrname] ,[usrpwd] ,[multipl_chrgr] ,[multipl_acc]  ,[acc_phs1] ,[acc_phs3], " +
                            " [savpwd] ,[FnmLnm] ,[actif] ,[email],[cpnyID]) " +
        ...
        {
            if (txfullNM.Text.Length > 3 && txLastnm.Text.Length > 3 && MainMDI.emailIsValid(txmail.Text))
            {
                txuser.Text = txfullNM.Text[0].ToString() + txLastnm.Text;
                if (txuser.Text.Length > 4) txpwd.Text = GenNewPWD();
                else MessageBox.Show("User Name must be more than 4 characters.....");
            }
            else MessageBox.Show("Invalid e-mail OR short Names........");
        }

        private void Del_itm_Click(object sender, EventArgs e)
        {
            //MessageBox.Show("Please contact you Admin. to delete a User....thanks....");
            if (ed_lvITM.SelectedItems.Count == 1)
            {
                int ndx = ed_lvITM.SelectedItems[0].Index;
                string stsql = "delete configo_Usetup where usrname ='" + ed_lvITM.Items[ndx].SubItems[3].Text + "' and cpnyID=" + ed_lvITM.Items[ndx].SubItems[2].Text;
                MainMDI.Exec_SQL_JFS(stsql, "delete user from configo");
        ...
        private void Find_Click(object sender, EventArgs e)
        {
            ...
            pan_find.Visible = true;
        }

        private void Piclook_Click_1(object sender, EventArgs e)
        {
            if (txfindUSR.Text.Length > 3) Fill_ONEusers(txfindUSR.Text);
        }

        private void Fill_ONEusers(string userNM)
        {
            //(Depnb == "Select Departement")
            //string CondDep = (Depnb == "0" && pnlDisp.Visible) ? "" : " AND XCNG_Employees.Depcode =" + Depnb;
            //string CondDep = (Depnb == "0") ? "" : " AND XCNG_Employees.Depcode =" + Depnb;
            ...
            string stSql = "SELECT  * from  configo_Usetup inner join [dbo].[configo_Usetup_cpny] on [cpny_lid]=[cpnyID] where usrname='" + userNM + "'";
            ...
            {
                ListViewItem lv = ed_lvITM.Items.Add(Oreadr["lid"].ToString());
                lv.SubItems.Add(Oreadr["SP_cpny_Name"].ToString() + " / " + Oreadr["FnmLnm"].ToString());
                lv.SubItems.Add(Oreadr["cpnyID"].ToString());
            ...
            OConn.Close();
            if (!found) MessageBox.Show("User not found........");
        }
    }
}
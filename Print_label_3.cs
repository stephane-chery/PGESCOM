...
namespace PGESCOM
{
	...
	public class Print_label_3
	{
		...
		public Print_label_3(char x_doctype, string x_lbl1, string x_lbl2, string x_lbl3, string x_prtNme, Order x_ORFrm, QuoteV3 x_QFrm)
		{
			In_ORFrm = x_ORFrm;
			In_QFrm = x_QFrm;
		...
		/*
		private void SelectPrinterAndPrintLabelExample()
		{
			...
			{
				//find the printer we want to print to
				if (printer == "My LabelWriter")
				{
					//select it first
					_dymoAddin.SelectPrinter(printer);
 
					//then print the currently open label
					if (_dymoAddin.IsTwinTurboPrinter(printer))
					{
		...
		*/
			...
			switch (in_docType)
			{
				case 'L': 
					for (int j = 1; j < doc.Bookmarks.Count + 1; j++)
					{
						...
						{
							case "lbl1":	
								string stt = in_lbl1.PadLeft(20, ' '); //20
								rng.InsertAfter(stt);
								rng.Font.Bold = 1;
								rng.Font.Size = 20;
								rng.Select();
								break;
							case "lbl2":
								if (in_lbl2.Length < 41) in_lbl2 = in_lbl2.PadLeft(40, ' '); //40
								...
								break;
							case "lbl3":
								if (in_lbl3.Length < 41) in_lbl3 = in_lbl3.PadLeft(40, ' '); //40
				...
				case 'O': 
					for (int j = 1; j < doc.Bookmarks.Count + 1; j++)
					{
						...
						switch (Bkname)
						{
							...
							case "NBCPA":
								//rng.InsertAfter("P" + In_ORFrm.LRID.Text + "-" + In_ORFrm.lcurRRevNm.Text);
								rng.InsertAfter("P" + In_ORFrm.LRID.Text + "/" + In_ORFrm.lSn.Text);
								rng.Select();
								break;
								//case "cuRR":	 
								//rng.InsertAfter(In_ORFrm.lcurDol.Text);
							...
							case "dlvdate":	 
								...
								string appvd = MainMDI.Find_One_Field("SELECT Aprvd FROM  PSM_R_Detail WHERE PrimaxSN ='" + 
									In_ORFrm.lSn.Text + "'");
								string stt = "";
		...
		{ 
			...
			if (in_docType == 'L')
			{
				app.ActiveDocument.PrintOut(ref myTrue, ref myFalse, ref range, ref missingValue, ref missingValue, ref missingValue, 
					ref items, ref copies, ref pages, ref pageType, ref myFalse, ref myTrue, ref missingValue, ref myFalse, ref missingValue, 
					ref missingValue, ref missingValue, ref missingValue);
			}
			object doNotSaveChanges = Word.WdSaveOptions.wdDoNotSaveChanges;
			app.ActiveDocument.Close(ref doNotSaveChanges, ref missingValue, ref missingValue);
		...
		{
			object fn = Ofn, fn2 = Ofn.Replace(".doc", ".PDF");

			object ff2 = Word.WdSaveFormat.wdFormatPDF;
			...
			app.ActiveDocument.SaveAs(ref fn2, ref ff2, ref lc, ref pwd, ref atr, ref wpwd, ref ron, ref embd, ref svN, ref svF, ref svLett, 
				ref enc, ref inLin, ref Asub, ref Linend, ref addmrk);
		}
		...
		private void Print_Det_Config(bool chk, string itm, string dwg, int pBOLD, string newLine)
		{
			int fnt = 8;
			Word.Table TQdet;
			...
			TQdet = app.ActiveDocument.Tables[app.ActiveDocument.Tables.Count - 6];
			TQdet.Rows.Add(ref MissV1);
			int j = TQdet.Rows.Count;
			//string st = (nb > 0) ? nb.ToString() + ". " : "";
			string st = (!chk) ? "□" : "√";
			if (itm == "\n") st = "";
			TQdet.Cell(j, 1).Range.Text = st; 
			TQdet.Cell(j, 1).Range.Font.Size = fnt; 
			TQdet.Cell(j, 1).Range.Font.Bold = pBOLD; 
			TQdet.Cell(j, 1).Range.Font.Underline = 0;

			TQdet.Cell(j, 2).Range.Text = st; 
			TQdet.Cell(j, 2).Range.Font.Size = fnt; 
			TQdet.Cell(j, 2).Range.Font.Bold = pBOLD; 
			TQdet.Cell(j, 2).Range.Font.Underline = 0;

			TQdet.Cell(j, 3).Range.Text = st; 
			TQdet.Cell(j, 3).Range.Font.Size = fnt; 
			TQdet.Cell(j, 3).Range.Font.Bold = pBOLD; 
			TQdet.Cell(j, 3).Range.Font.Underline = 0;

			TQdet.Cell(j, 4).Range.Text = st; 
			TQdet.Cell(j, 4).Range.Font.Size = fnt; 
			TQdet.Cell(j, 4).Range.Font.Bold = pBOLD; 
			TQdet.Cell(j, 4).Range.Font.Underline = 0;

			TQdet.Cell(j, 5).Range.Text = st; 
			TQdet.Cell(j, 5).Range.Font.Size = fnt; 
			TQdet.Cell(j, 5).Range.Font.Bold = pBOLD; 
			TQdet.Cell(j, 5).Range.Font.Underline = 0;

			TQdet.Cell(j, 6).Range.Text = itm + newLine; 
			TQdet.Cell(j, 6).Range.Font.Size = fnt; 
			TQdet.Cell(j, 6).Range.Font.Bold = pBOLD; 
			TQdet.Cell(j, 6).Range.Font.Underline = 0;

			TQdet.Cell(j, 7).Range.Text = dwg; 
			TQdet.Cell(j, 7).Range.Font.Size = fnt; 
			TQdet.Cell(j, 7).Range.Font.Bold = pBOLD; 
			TQdet.Cell(j, 7).Range.Font.Underline = 0;
		...
		private void Print_Det_ConfigOLD(bool chk, string itm, string dwg) //no BOLD
		{
			Word.Table TQdet;
			Object MissV1 = Type.Missing;
			Object MissV2 = Type.Missing;
			...
			string st = (!chk) ? "□" : "√";
			TQdet.Cell(j, 1).Range.Text = st; 
			TQdet.Cell(j, 1).Range.Font.Size = 9; 
			TQdet.Cell(j, 1).Range.Font.Bold = 0; 
			TQdet.Cell(j, 1).Range.Font.Underline = 0;

			TQdet.Cell(j, 2).Range.Text = st; 
			TQdet.Cell(j, 2).Range.Font.Size = 9; 
			TQdet.Cell(j, 2).Range.Font.Bold = 0; 
			TQdet.Cell(j, 2).Range.Font.Underline = 0;

			TQdet.Cell(j, 3).Range.Text = st; 
			TQdet.Cell(j, 3).Range.Font.Size = 9; 
			TQdet.Cell(j, 3).Range.Font.Bold = 0; 
			TQdet.Cell(j, 3).Range.Font.Underline = 0;

			TQdet.Cell(j, 4).Range.Text = st; 
			TQdet.Cell(j, 4).Range.Font.Size = 9; 
			TQdet.Cell(j, 4).Range.Font.Bold = 0; 
			TQdet.Cell(j, 4).Range.Font.Underline = 0;

			TQdet.Cell(j, 5).Range.Text = itm; 
			TQdet.Cell(j, 5).Range.Font.Size = 9; 
			TQdet.Cell(j, 5).Range.Font.Bold = 0; 
			TQdet.Cell(j, 5).Range.Font.Underline = 0;

			TQdet.Cell(j, 6).Range.Text = dwg; 
			TQdet.Cell(j, 6).Range.Font.Size = 9; 
			TQdet.Cell(j, 6).Range.Font.Bold = 0; 
			TQdet.Cell(j, 6).Range.Font.Underline = 0;
			//TQdet.Cell(j, 4).Range.Text = c3; TQdet.Cell(j, 4).Range.Font.Size = 9; TQdet.Cell(j, 4).Range.Font.Bold = 0; TQdet.Cell(j, 4).Range.Font.Underline = 0;
			//TQdet.Cell(j, 5).Range.Text = c4; TQdet.Cell(j, 5).Range.Font.Size = 9; TQdet.Cell(j, 5).Range.Font.Bold = 0; TQdet.Cell(j, 5).Range.Font.Underline = 0;
		}

		private void Print_Det_Config_5Cols(bool chk, string itm, string dwg) //details 5 columns saved 18112010
		{
			Word.Table TQdet;
			Object MissV1 = Type.Missing;
			Object MissV2 = Type.Missing;
			...
			TQdet.Cell(j, 4).Range.Text = itm; 
			TQdet.Cell(j, 4).Range.Font.Size = 9; 
			TQdet.Cell(j, 4).Range.Font.Bold = 0; 
			TQdet.Cell(j, 4).Range.Font.Underline = 0;

			TQdet.Cell(j, 5).Range.Text = dwg; 
			TQdet.Cell(j, 5).Range.Font.Size = 9; 
			TQdet.Cell(j, 5).Range.Font.Bold = 0; 
			TQdet.Cell(j, 5).Range.Font.Underline = 0;
			//TQdet.Cell(j, 4).Range.Text = c3; TQdet.Cell(j, 4).Range.Font.Size = 9; TQdet.Cell(j, 4).Range.Font.Bold = 0; TQdet.Cell(j, 4).Range.Font.Underline = 0;
		...
		private void Print_Det_ConfigOK_160807(bool chk, string itm, string dwg)
		{
			...
			TQdet.Cell(j, 2).Range.Text = itm; 
			TQdet.Cell(j, 2).Range.Font.Size = 9; 
			TQdet.Cell(j, 2).Range.Font.Bold = 0; 
			TQdet.Cell(j, 2).Range.Font.Underline = 0;

			TQdet.Cell(j, 3).Range.Text = dwg; 
			TQdet.Cell(j, 3).Range.Font.Size = 9; 
			TQdet.Cell(j, 3).Range.Font.Bold = 0; 
			TQdet.Cell(j, 3).Range.Font.Underline = 0;
			//TQdet.Cell(j, 4).Range.Text = c3; TQdet.Cell(j, 4).Range.Font.Size = 9; TQdet.Cell(j, 4).Range.Font.Bold = 0; TQdet.Cell(j, 4).Range.Font.Underline = 0;
		...
		private void Print_Det_QAlias(bool chk, string affID, string itm, string Qty, string Ext)
		{
			...
			TQdet.Cell(j, 1).Range.Text = affID; 
			TQdet.Cell(j, 1).Range.Font.Size = 9; 
			TQdet.Cell(j, 1).Range.Font.Bold = 0; 
			TQdet.Cell(j, 1).Range.Font.Underline = 0;
		...
		private void Print_Det_SummOLD(int nb, string c1, string c2, string c3, string c4, string QtyBO)
		{
			...
			TQdet.Cell(j, 3).Range.Text = QtyBO; 
			TQdet.Cell(j, 3).Range.Font.Size = 9; 
			TQdet.Cell(j, 3).Range.Font.Bold = 0; 
			TQdet.Cell(j, 3).Range.Font.Underline = 0;

			TQdet.Cell(j, 4).Range.Text = c3; 
			TQdet.Cell(j, 4).Range.Font.Size = 9; 
			TQdet.Cell(j, 4).Range.Font.Bold = 0; 
			TQdet.Cell(j, 4).Range.Font.Underline = 0;

			TQdet.Cell(j, 5).Range.Text = c4; 
			TQdet.Cell(j, 5).Range.Font.Size = 9; 
			TQdet.Cell(j, 5).Range.Font.Bold = 0; 
			TQdet.Cell(j, 5).Range.Font.Underline = 0;
		}
		...
		private void Print_Det_Summ(char BorF, string line, string c1, string c2, string c3, string c4, string QtyBO)
		{
			...
			else 
			{
				...
				TQdet.Cell(j, 2).Range.Text = c1; 
				TQdet.Cell(j, 1).Range.Font.Size = 9; 
				TQdet.Cell(j, 1).Range.Font.Bold = 0; 
				TQdet.Cell(j, 1).Range.Font.Underline = 0;

				TQdet.Cell(j, 3).Range.Text = c2; 
				TQdet.Cell(j, 2).Range.Font.Size = 9; 
				TQdet.Cell(j, 2).Range.Font.Bold = 0; 
				TQdet.Cell(j, 2).Range.Font.Underline = 0;
		...
		*/
			...
			if (In_ORFrm.checkBox1.Checked)
			{
				for (int i = 0; i < In_ORFrm.lvQITEMS.Items.Count; i++)
				{
					...
					if (NewAff == oldaff && NewAff != " ")
					{
						...
						m5UP = Tools.Conv_Dbl(In_ORFrm.lvQITEMS.Items[i].SubItems[5].Text);
						m3Qty += Tools.Conv_Dbl(In_ORFrm.lvQITEMS.Items[i].SubItems[3].Text);
						m7Ext = (m5UP > 0) ? Math.Round(m5UP * m3Qty, MainMDI.Q_NB_DEC_AFF) : 
							Tools.Conv_Dbl(In_ORFrm.lvQITEMS.Items[i].SubItems[7].Text);
					}
					else 
					{
						if (!BO) Print_Det_Summ('B', "0", m2Desc, m3Qty.ToString(), MainMDI.A00(m5UP.ToString()), 
							MainMDI.A00(m7Ext.ToString()), QtyBO.ToString());
						m2Desc = In_ORFrm.lvQITEMS.Items[i].SubItems[2].Text;
						m5UP = Tools.Conv_Dbl(In_ORFrm.lvQITEMS.Items[i].SubItems[5].Text);
						m3Qty = Tools.Conv_Dbl(In_ORFrm.lvQITEMS.Items[i].SubItems[3].Text);
						m7Ext = Tools.Conv_Dbl(In_ORFrm.lvQITEMS.Items[i].SubItems[7].Text);
				...
				Print_Det_Summ('B', "0", m2Desc, m3Qty.ToString(), MainMDI.A00(m5UP.ToString()), MainMDI.A00(m7Ext.ToString()), 
					QtyBO.ToString());
			...
			else
			{
				for (int i = 0; i < In_ORFrm.lvQITEMS.SelectedItems.Count; i++)
				{
					...
					if (NewAff == oldaff && NewAff != " ")
					{
						...
						m3Qty += Tools.Conv_Dbl(In_ORFrm.lvQITEMS.SelectedItems[i].SubItems[3].Text);
						m7Ext = (m5UP > 0) ? Math.Round(m5UP * m3Qty, MainMDI.Q_NB_DEC_AFF) : 
							Tools.Conv_Dbl(In_ORFrm.lvQITEMS.SelectedItems[i].SubItems[7].Text);
					}
					else 
					{
						if (!BO) Print_Det_Summ('B', "0", m2Desc, m3Qty.ToString(), MainMDI.A00(m5UP.ToString()), 
		...
		private void Page_BILs_Details(char BorF)
		{
			...
			string stSql = " SELECT PSM_R_SBills.BilNm AS Expr1, PSM_R_SBillsDetail.*, PSM_R_SBills.Bil_LID AS Expr2 " +
				" FROM PSM_R_SBills INNER JOIN PSM_R_SBillsDetail ON PSM_R_SBills.Bil_LID = PSM_R_SBillsDetail.b_BilLID " +
				" WHERE     (PSM_R_SBills.b_RRevLID = " + In_ORFrm.lIRRevID.Text + ") AND (PSM_R_SBillsDetail.b_d_affID <> 0)" +
				"        AND (PSM_R_SBills.BilNm = '" + In_ORFrm.lcurBilNm.Text + "') " +
				" ORDER BY PSM_R_SBills.b_Rnk, PSM_R_SBillsDetail.b_d_Rnk ";
			...
			while (Oreadr.Read())
			{
				...
				m2Desc = (BorF == 'F') ? Oreadr["b_d_ItemDesc"].ToString() : 
					Convert.ToString(daff++) + "- " + Oreadr["b_d_ItemDesc"].ToString();
			...
			{
				string TF = "", TB = "";
				stSql = " SELECT SUM(FReight) AS totFrt, SUM(BilTOT) AS TotBT" +
					" FROM  PSM_R_SBills " +
					" WHERE b_RRevLID = " + In_ORFrm.lIRRevID.Text + 
					" GROUP BY Bil_LID, b_Rnk" +
					" ORDER BY b_Rnk";
		...
		*/
		...
		private void Page_ONELD_DetailsOLD() //print from CF directly
		{
			Object MissV1 = Type.Missing;
			Object MissV2 = Type.Missing;
			Word.Range Rng = app.ActiveDocument.Content; //.Range(ref start, ref end);
			...
			for (int i = 0; i < In_ORFrm.lvCurConfig.Items.Count; i++)
			{
				...
				if (In_ORFrm.lvCurConfig.Items[i].Checked) Print_Det_ConfigOLD(false, In_ORFrm.lvCurConfig.Items[i].SubItems[1].Text, 
					In_ORFrm.lvCurConfig.Items[i].SubItems[2].Text);
			}
		}

		private bool isOPtion(string ItemDesc, string revID)
		{
			double res = Tools.Conv_Dbl(MainMDI.Find_One_Field("select PSM_R_Detail.Aff_ID  from dbo.PSM_R_Detail inner join dbo.PSM_Q_Details on Detail_LID=Qdet_LID where IRRev_LID=" + 
				revID + " and [desc]= '" + ItemDesc.Replace("'", "''") + "'"));
			return (res > 1);
		}
		...
		private void Page_ONELD_Details()
		{
			...
			string stSql = "SELECT     PSM_R_CFDetail.*, PSM_R_CFinfo.*" +
				...
				" WHERE     PSM_R_CFDetail.d_CFLID = '" + In_ORFrm.lCFLID.Text + "' AND PSM_R_CFinfo.c_RRevLID =" + In_ORFrm.lIRRevID.Text + 
			...
			while (Oreadr.Read())
			{
				string newLine = (isOPtion(Oreadr["d_ItemDesc"].ToString(), Oreadr["c_RRevLID"].ToString())) ? "\n" : "";
				Print_Det_Config(false, Oreadr["d_ItemDesc"].ToString(), Oreadr["DWGnb"].ToString().Replace("~~", "  "), 
					(Oreadr["Bold"].ToString() == "True") ? 1 : 0, newLine);
				//Print_Det_Config(false, Oreadr["d_ItemDesc"].ToString(), Oreadr["DWGnb"].ToString().Replace("~~", "  "), (Oreadr["Bold"].ToString() == "True") ? 1 : 0);
			}
			Print_Det_Config(false, "\n", "", 0, "");
			OConn.Close();
		}
		...
		private void Page_Quote_Details()
		{
			...
			for (int i = 0; i < In_QFrm.lvQITEMS.Items.Count; i++)
			{
				...
				Print_Det_QAlias(false, In_QFrm.lvQITEMS.Items[i].SubItems[1].Text, In_QFrm.lvQITEMS.Items[i].SubItems[2].Text, 
					In_QFrm.lvQITEMS.Items[i].SubItems[3].Text, In_QFrm.lvQITEMS.Items[i].SubItems[7].Text);

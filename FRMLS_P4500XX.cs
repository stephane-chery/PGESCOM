...
namespace PGESCOM
{
    public partial class FRMLS_P4500XX : Form
    {
...
/*
 ...
 /*
  *    private int Price_BIGtoXL_ALLCPT(string _phs, string _Vdc, string _Idc, string Avail_id)
        {
            ...
            string _Pxx = "P4500", _Vac = "", _VdcMax = "", _lcptID = "", Curr_CPTNM = "", PXX_P_VDC = "P4500-" + _phs + "-" + _Vdc, CatNM2 = MainMDI.VIDE, CatNM3 = MainMDI.VIDE;
            Component Cpt = null;
            Charger CHRGR = null;
            ...
            while (Oreadr.Read())
            {
                if (Oreadr["Compnt_Type"].ToString() != "%" && Oreadr["COMPONENT_REF"].ToString() != MainMDI.VIDE)
                {
                    ...
                    if (Oreadr["COMPONENT_REF"].ToString().IndexOf("EN1") > -1 ) _lcptID = _lcptID;
                    string Row_type = "C";
                    Cpt = new Component();
                    _lcptID = Oreadr["Component_ID"].ToString();
                    Curr_CPTNM = Oreadr["COMPONENT_REF"].ToString();
                    ...
                    if (ndx_IDC == 1 || arr_BIGtoXL[Curr_LN, 0] == "~")
                    {
                        arr_BIGtoXL[Curr_LN, 0] = Curr_CPTNM;
                        arr_BIGtoXL[Curr_LN, 33] = PXX_P_VDC;
                        arr_BIGtoXL[Curr_LN, 34] = Row_type;

                    }
                    arr_BIGtoXL[Curr_LN++, ndx_IDC] = Tools.Conv_Dbl(Cpt.G_PRICE).ToString();
                    if (ndx_IDC == 1 || arr_BIGtoXL[Curr_LN, 0] == "~")
                    {
                        arr_BIGtoXL[Curr_LN, 0] = "   [" + HT_CPT_cat[Curr_CPTNM + "_CAT1"].ToString() + "]";
                        arr_BIGtoXL[Curr_LN, 33] = PXX_P_VDC;
                        arr_BIGtoXL[Curr_LN, 34] = Row_type;

                    }
                    arr_BIGtoXL[Curr_LN++, ndx_IDC] = Cpt.CAP1;

                    CatNM2 = HT_CPT_cat[Curr_CPTNM + "_CAT2"].ToString();
                    if (ndx_IDC == 1 || arr_BIGtoXL[Curr_LN, 0] == "~")
                    {
                        arr_BIGtoXL[Curr_LN, 0] = "   [" + CatNM2 + "]";
                        arr_BIGtoXL[Curr_LN, 33] = PXX_P_VDC;
                        arr_BIGtoXL[Curr_LN, 34] = Row_type;
                    }
                    arr_BIGtoXL[Curr_LN++, ndx_IDC] = Cpt.CAP2;

                    CatNM3 = HT_CPT_cat[Curr_CPTNM + "_CAT3"].ToString();
                    if (ndx_IDC == 1 || arr_BIGtoXL[Curr_LN, 0] == "~")
                    {
                        arr_BIGtoXL[Curr_LN, 0] = "   [" + CatNM3 + "]";
                        arr_BIGtoXL[Curr_LN, 33] = PXX_P_VDC;
                        arr_BIGtoXL[Curr_LN, 34] = Row_type;
                    }
                    arr_BIGtoXL[Curr_LN++, ndx_IDC] = Cpt.CAP3;

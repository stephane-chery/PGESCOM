...
namespace PGESCOM
{
    public partial class CMS_fromSYSPRO_Rates : Form
    {
        ...
        string rates_table = "SalCommissionInsideSales";
        //string rates_table = "SalCommissionInsideSales_Hakim";

        public CMS_fromSYSPRO_Rates(char x_SA)
        {
            InitializeComponent();
        ...
        private void fill_cbSales(string _branch)
        {
            string stSql = "", condi = "";
            condi = "Branch='" + _branch + "' and ";
            System.Web.UI.WebControls.ListItem itm = new System.Web.UI.WebControls.ListItem();
            switch (in_SA)
            {
                case 'S':
                    stSql = "SELECT ' (' + [Salesperson] +') '  +[Name]  ,[Salesperson]     FROM [SysproCompanyP].[dbo].[SalSalesperson] where " + condi + " SUBSTRING (Salesperson,1,1)='I' order by [Salesperson] ";
                    MainMDI.fill_CB_SYSP(cbSA, stSql, true, "ALL");
                    cbSA.SelectedIndex  = 0;

                    stSql = "SELECT ' (' + [Salesperson] +') '  +[Name] ,[Salesperson]    FROM [SysproCompanyP].[dbo].[SalSalesperson] where " + condi + " SUBSTRING (Salesperson,1,1)<>'I' and SUBSTRING (Salesperson,1,1)<>'A' order by [Salesperson] ";
                    MainMDI.fill_CB_SYSP(cbTerri, stSql, true, "ALL");
        ...
        private void fill_SalesRates()
        {
            ...
            lRateTbl.Text = rates_table;
            string cond1 = (cbTerri.Text == "ALL") ? "" : " and " + rates_table + ".ExtSalesperson='" + lTerri_lid.Text + "' ";
            string cond2 = (cbSA.Text == "ALL") ? "" : " and " + rates_table + ".IntSalesperson='" + lSA_ID.Text + "' ";

            /*
            string stSql = "SELECT   distinct SalCommissionInsideSales.Branch, SalCommissionInsideSales.ExtSalesperson,SalSalesperson.Name as terri_Name, SalCommissionInsideSales.IntSalesperson,  SalSalesperson_1.Name AS SalesName, SalCommissionInsideSales.IntRate, " +
                " SalCommissionInsideSales.Active, SalCommissionInsideSales.RateEffectiveDate, SalCommissionInsideSales.PreviousIntRate " +
                "FROM     SalCommissionInsideSales INNER JOIN SalSalesperson ON SalSalesperson.Salesperson = SalCommissionInsideSales.ExtSalesperson INNER JOIN " +
                " SalSalesperson AS SalSalesperson_1 ON SalSalesperson.Branch = SalSalesperson_1.Branch AND  SalCommissionInsideSales.IntSalesperson = SalSalesperson_1.Salesperson " +
                " where SalCommissionInsideSales.Branch='" + cbBranch.Text + "' " + cond1 + cond2 +
                " order by SalCommissionInsideSales.Branch,SalCommissionInsideSales.ExtSalesperson, SalCommissionInsideSales.IntSalesperson ";
            */
            string stSql = "SELECT   distinct " + rates_table + ".Branch, " + rates_table + ".ExtSalesperson,SalSalesperson.Name as terri_Name, " + rates_table + ".IntSalesperson,  SalSalesperson_1.Name AS SalesName, " + rates_table + ".IntRate,  " +
                rates_table + ".Active, " + rates_table + ".RateEffectiveDate, " + rates_table + ".PreviousIntRate " +
                "FROM     " + rates_table + " INNER JOIN SalSalesperson ON SalSalesperson.Salesperson = " + rates_table + ".ExtSalesperson INNER JOIN " +
                " SalSalesperson AS SalSalesperson_1 ON SalSalesperson.Branch = SalSalesperson_1.Branch AND  " + rates_table + ".IntSalesperson = SalSalesperson_1.Salesperson " +
                " where " + rates_table + ".Branch='" + cbBranch.Text + "' " + cond1 + cond2 +
                " order by " + rates_table + ".Branch," + rates_table + ".ExtSalesperson, " + rates_table + ".IntSalesperson ";
            ...
            while (Oreadr.Read())
            {
                ListViewItem lv = ed_lvITM.Items.Add(Oreadr["Branch"].ToString());

                lv.SubItems.Add(Oreadr["terri_Name"].ToString());
                lv.SubItems.Add(Oreadr["SalesName"].ToString());
                lv.SubItems.Add(Oreadr["IntRate"].ToString());

                string actf = (Oreadr["Active"].ToString() == true.ToString()) ? "YES" : "NO";
                lv.SubItems.Add(actf);
                lv.SubItems.Add(MainMDI.Eng_date(Oreadr["RateEffectiveDate"].ToString(), "/"));
                lv.SubItems.Add(Oreadr["PreviousIntRate"].ToString());

                lv.SubItems.Add(Oreadr["ExtSalesperson"].ToString());
                lv.SubItems.Add(Oreadr["IntSalesperson"].ToString());
            }
            OConn.Close();
        }

        private void fill_SalesRatesoldddd()
        {
        ...
        private void edit_CmsLine(int _ndx)
        {
            ...
            txBranch.Text = ed_lvITM.Items[_ndx].SubItems[0].Text;
            txTerri.Text = ed_lvITM.Items[_ndx].SubItems[1].Text;
            txSName.Text = ed_lvITM.Items[_ndx].SubItems[2].Text;

            txSrate.Text = ed_lvITM.Items[_ndx].SubItems[3].Text;
            loldrate.Text = ed_lvITM.Items[_ndx].SubItems[3].Text;
       
            chk_active.Checked = (ed_lvITM.Items[_ndx].SubItems[4].Text == "YES") ? true : false;
            dpDelvry.Text = ed_lvITM.Items[_ndx].SubItems[5].Text;
            txPrate.Text = ed_lvITM.Items[_ndx].SubItems[6].Text;
        ...
        private void TS_AGTerr_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {

        }

        private void pic_disprates_Click(object sender, EventArgs e)
        {
            lScmsLID.Text = "";
            //lSrate.Visible = false;
            //txSrate.Visible = false;
        ...
        {
            txDateeff.Text = dpDelvry.Value.Year + "/" + MainMDI.A00(dpDelvry.Value.Month, 2) + "/" + MainMDI.A00(dpDelvry.Value.Day, 2);
        }

        private void btnOK_Click(object sender, EventArgs e)
        {
            string rez = SaveRate(ndx);
            if (rez == "")
            {
                fill_SalesRates();
                grpRate.Visible = false;
            ...
            else MessageBox.Show(rez);
        }

        private string  SaveRate(int __ndx)
        {
            string rez = "";
            if (Tools.Conv_Dbl(txSrate.Text) < 1)
            {
                if (__ndx != -1)
                {
                    int activ = (chk_active.Checked) ? 1 : 0;
                    if (txSrate.Text != loldrate.Text) txPrate.Text = loldrate.Text;
                    string _terri = ed_lvITM.Items[__ndx].SubItems[7].Text;
                    string _salid = ed_lvITM.Items[__ndx].SubItems[8].Text;
                    string _branch = ed_lvITM.Items[__ndx].SubItems[0].Text;
                    txSrate.Text = Tools.Conv_Dbl(txSrate.Text).ToString();
                    MainMDI.Exec_SQL_JFS_SYSPRO("update  " + rates_table + " set [IntRate]=" + txSrate.Text + " , [PreviousIntRate]=" + txPrate.Text + " , [Active]=" + activ.ToString() + " , [RateEffectiveDate]=Convert(datetime,'" + txDateeff.Text + "') "  +
                        " where Branch='" + _branch + "' and  ExtSalesperson='" + _terri + "' and  IntSalesperson='" + _salid + "'", " CMS Update sales cms Rates...");
                    ndx = -1;
                }
            }
            else rez = "Sorry ....Invalid CMS Rate !!!!";
            return rez;
        }

        private void txSrate_TextChanged(object sender, EventArgs e)
        {
            double dd = Tools.Conv_Dbl(txSrate.Text) * 100;
            lratePct.Text = dd.ToString() + "%";
        }

        private void cbBranch_SelectedIndexChanged(object sender, EventArgs e)
        {
            ed_lvITM.Items.Clear();
            fill_cbSales(cbBranch.Text);
        }

        private void btnApply_Click(object sender, EventArgs e)
        {
            bool ok = false;
            for (int i = 0; i < ed_lvITM.SelectedItems.Count; i++)
            {
                string rez = SaveRate(ed_lvITM.SelectedItems[i].Index);
                if (rez != "")
                {
                    MessageBox.Show(rez);
                    ok = false;
                    i = ed_lvITM.SelectedItems.Count;
                }
                else ok = true;
            }
            if (ok)

...
namespace PGESCOM
{
	public class Modified_EditListView : ListView 
	{

       private bool freeTxt=false;
		...
		//	this.Name = "lvLTest";
			...
			this.MouseDown += new System.Windows.Forms.MouseEventHandler(this.ModifiedMouseDown);
			this.DoubleClick += new System.EventHandler(this.ModifiedDoubleClick);
			this.GridLines = true ;
			this.Columns.Clear(); 
			// 
			// columnHeader1
		...
		private void CmbSelected(object sender , System.EventArgs e)
		{
			...
			if ( sel >= 0 )
			{
				...
				li.SubItems[subItemSelected].Text = itemSel;
			 	li.ImageIndex =(itemSel=="ON") ? 8 : 9;
			//                                  	li.ImageIndex =(li.ImageIndex==9) ? 8 : 9;
				cmbBox.Hide(); //added by hak 
			}
		}
...
//			{	 
				if ( e.KeyChar == 13 ) 
				{
					if (editBox.Text=="") editBox.Text="?";
					li.SubItems[subItemSelected].Text =(freeTxt) ? "[" + editBox.Text + "]" :  editBox.Text;
					editBox.Hide();
				}
	...
	//		}
		...
		private void EditOverold(object sender, System.Windows.Forms.KeyPressEventArgs e)
		{
			if ( e.KeyChar == 13 ) 
			{
		...
		public  void ModifiedDoubleClick(object sender, System.EventArgs e)
		{
            bool statCHK = li.Checked;
           

          //  if (Modifiable && colNm.Length >2 && colNm.Substring (0,2)!="__" ) 
            if (Modifiable) // '__' in name column means do not modify it
            {
                ...
                string toto = this.Columns[subItemSelected].Name;
                string toto2 = li.Name;
                if (colName.Length > 2 && colName.Substring(0, 2) != "__")   
                {
                    if ((subItemText.Length > 0 && colName.Length > 0 && colName[0] != ' ' && subItemText[0] != '~') || (subItemText.Length > 0 && subItemText[0] == '[') || (colName == "Tested"))
                    {


                        if (subItemText.Length > 0) freeTxt = (subItemText[0] == '[' && colName[0] == ' ');
                        if (colName == "State" || colName == "Tested" || colName == "TST ") //|| colName[0]!= '[' ) 
                        {
                            if (colName == "State")
                            {
                                if (subItemText == "OFF")
                                {
                                    li.SubItems[subItemSelected].Text = "ON";
                                    li.ImageIndex = 8;
                                }
                                else
                                {
                                    if (subItemText == "ON")
                                    {
                                        li.SubItems[subItemSelected].Text = "OFF";
                                        li.ImageIndex = 9;
                                    }
                                }
                            }
                            if (colName == "Tested") li.ImageIndex = (li.ImageIndex == 8) ? 9 : 8;
                        ...
                        else
                        {
                            ...
                            if (freeTxt)
                            {
                                if (subItemText == "[] ") subItemText = "";
                                else subItemText = subItemText.Substring(1, subItemText.Length - 2);
                            }
...
/*
		...
		public  void ModifiedDoubleClickbaddd(object sender, System.EventArgs e)
		{
			// Check the subitem clicked .
			freeTxt=false;
			int nStart = X ;
		...
		//	Console.WriteLine("SUB ITEM SELECTED = " + li.SubItems[subItemSelected].Text);
			...
			string colName = this.Columns[subItemSelected].Text ;
		//	if (  ( subItemText.Length>0 && colName.Length >0  &&  colName[0]!= ' ' && subItemText[0]!='~' ) || ( subItemText.Length>0 &&  subItemText[0]== '[') || ( colName == "Tested") )
			if (  ( colName.Length >0  &&  colName[0]!= ' ' && subItemText[0]!='~' ) || ( subItemText.Length>0 &&  subItemText[0]== '[') || ( colName == "Tested") )
			{
...
*/
		...
		public void ModifiedMouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{

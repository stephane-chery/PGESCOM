...
namespace PGESCOM
{
	...
	//		Save_Doc();
		...
		private void Save_Doc()
		{
            ...
            app.ActiveDocument.SaveAs(ref fn2, ref ff2, ref lc, ref pwd, ref atr, ref wpwd, ref ron, ref embd, ref svN, ref svF, ref svLett, ref enc, ref inLin, ref Asub, ref Linend, ref addmrk);
        ...
        private void Print_Det_Config(bool chk, string itm, string dwg, int pBOLD,string newLine)
        {
          ...
          //  TQdet = app.ActiveDocument.Tables[app.ActiveDocument.Tables.Count - 1];
            ...
            string st = (!chk) ? "□" : "√";
            if (itm == "\n") st = "";
            TQdet.Cell(j, 1).Range.Text = st; TQdet.Cell(j, 1).Range.Font.Size = fnt; TQdet.Cell(j, 1).Range.Font.Bold = pBOLD ; TQdet.Cell(j, 1).Range.Font.Underline = 0;
            TQdet.Cell(j, 2).Range.Text = st; TQdet.Cell(j, 2).Range.Font.Size = fnt; TQdet.Cell(j, 2).Range.Font.Bold = pBOLD; TQdet.Cell(j, 2).Range.Font.Underline = 0;
            TQdet.Cell(j, 3).Range.Text = st; TQdet.Cell(j, 3).Range.Font.Size = fnt; TQdet.Cell(j, 3).Range.Font.Bold = pBOLD; TQdet.Cell(j, 3).Range.Font.Underline = 0;
            TQdet.Cell(j, 4).Range.Text = st; TQdet.Cell(j, 4).Range.Font.Size = fnt; TQdet.Cell(j, 4).Range.Font.Bold = pBOLD; TQdet.Cell(j, 4).Range.Font.Underline = 0;
            TQdet.Cell(j, 5).Range.Text = st; TQdet.Cell(j, 5).Range.Font.Size = fnt; TQdet.Cell(j, 5).Range.Font.Bold = pBOLD; TQdet.Cell(j, 5).Range.Font.Underline = 0;

            TQdet.Cell(j, 6).Range.Text = itm + newLine; TQdet.Cell(j, 6).Range.Font.Size = fnt; TQdet.Cell(j, 6).Range.Font.Bold = pBOLD; TQdet.Cell(j, 6).Range.Font.Underline = 0;
            TQdet.Cell(j, 7).Range.Text = dwg ; TQdet.Cell(j, 7).Range.Font.Size = fnt; TQdet.Cell(j, 7).Range.Font.Bold = pBOLD; TQdet.Cell(j, 7).Range.Font.Underline = 0;
        ...
        private void Print_Det_ConfigOLD(bool chk, string itm, string dwg) //no BOLD
        {
            ...
            TQdet.Cell(j, 4).Range.Text = st; TQdet.Cell(j, 4).Range.Font.Size = 9; TQdet.Cell(j, 4).Range.Font.Bold = 0; TQdet.Cell(j, 4).Range.Font.Underline = 0;

            TQdet.Cell(j, 5).Range.Text = itm; TQdet.Cell(j, 5).Range.Font.Size = 9; TQdet.Cell(j, 5).Range.Font.Bold = 0; TQdet.Cell(j, 5).Range.Font.Underline = 0;
            TQdet.Cell(j, 6).Range.Text = dwg; TQdet.Cell(j, 6).Range.Font.Size = 9; TQdet.Cell(j, 6).Range.Font.Bold = 0; TQdet.Cell(j, 6).Range.Font.Underline = 0;
            //TQdet.Cell(j,4).Range.Text = c3 ;TQdet.Cell(j,4).Range.Font.Size =9;TQdet.Cell(j,4).Range.Font.Bold =0; TQdet.Cell(j,4).Range.Font.Underline =0; 
            //TQdet.Cell(j,5).Range.Text = c4;TQdet.Cell(j,5).Range.Font.Size =9;TQdet.Cell(j,5).Range.Font.Bold =0; TQdet.Cell(j,5).Range.Font.Underline =0; 
...
*/
        private void Page_ONELD_DetailsOLD() //print from CF directly
		{
			...
			for (int i=0;i<In_ORFrm.lvCurConfig.Items.Count;i++)
			{
				...
				if (In_ORFrm.lvCurConfig.Items[i].Checked ) Print_Det_ConfigOLD (false,In_ORFrm.lvCurConfig.Items[i].SubItems[1].Text ,In_ORFrm.lvCurConfig.Items[i].SubItems[2].Text); 
        ...
        {
            double res = Tools.Conv_Dbl(MainMDI.Find_One_Field("select PSM_R_Detail.Aff_ID  from dbo.PSM_R_Detail inner join dbo.PSM_Q_Details on Detail_LID=Qdet_LID where IRRev_LID=" + revID + " and [desc]= '" + ItemDesc.Replace("'", "''") + "'"));
            return (res >1 ); 
        ...
        private void Page_ONELD_Details()
        {
     	...
     	string stSql="SELECT     PSM_R_CFDetail.*, PSM_R_CFinfo.* FROM  PSM_R_CFDetail INNER JOIN  PSM_R_CFinfo ON PSM_R_CFDetail.d_CFLID = PSM_R_CFinfo.CFLID " +
				" WHERE     PSM_R_CFDetail.d_CFLID = '" +In_ORFrm.lCFLID.Text  + "' AND PSM_R_CFinfo.c_RRevLID =" + In_ORFrm.lIRRevID.Text + " ORDER BY PSM_R_CFDetail.su_Rnk";
			SqlConnection OConn  = new SqlConnection(MainMDI.M_stCon  );
			OConn.Open ();
			SqlCommand Ocmd = OConn.CreateCommand();
			Ocmd.CommandText = stSql ;
			...
			while (Oreadr.Read ()) 
			{
                ...
                Print_Det_Config(false, Oreadr["d_ItemDesc"].ToString(), Oreadr["DWGnb"].ToString().Replace("~~", "  "), (Oreadr["Bold"].ToString() == "True") ? 1 : 0,newLine );

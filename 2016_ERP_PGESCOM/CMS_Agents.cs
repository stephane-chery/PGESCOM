    ...
    public partial class CMS_Agents : Form
    {
        ...
        void Fill_lvCMS(string _inv)
        {
            ...
            string stSql = " SELECT distinct u_SalCommissionsPrimax.Invoice,CAST( u_SalCommissionsPrimax.FiscalMonth as varchar)  +'-' + cast(u_SalCommissionsPrimax.FiscalYear as varchar) as YYMM, " +
                           ...
                           " FROM         u_SalCommissionsPrimax INNER JOIN SalSalesperson ON u_SalCommissionsPrimax.Salesperson = SalSalesperson.Salesperson INNER JOIN ArInvoice ON u_SalCommissionsPrimax.Invoice = ArInvoice.Invoice AND (u_SalCommissionsPrimax.PriceCAD>0) " +
                           " WHERE     (SUBSTRING(u_SalCommissionsPrimax.Invoice, 1, 1) <> '7') AND (CAST(u_SalCommissionsPrimax.Invoice AS decimal) = " + Tools.Conv_Dbl(_inv) + ") AND (ArInvoice.InvoiceBal1 = 0) " +
            ...
            while (Oreadr.Read())
            {
                ...
                for (int i = 1; i < 11; i++)                
                    lv.SubItems.Add(Oreadr[i].ToString().TrimEnd());
                ...
                if (arr_infSP[0, 0] == "")
                {
                    ...
                    arr_infSP[II, 0] = "Address "; arr_infSP[II++, 1] = "||";
                    stSql = "select RTRIM (ShipAddr1)+' ' +RTRIM ( ShipAddr2)+' ' +RTRIM ( ShipAddr3)+' ' +RTRIM (ShipAddr4) +' ' +RTRIM (ShipAddr5)+' ' +RTRIM (PostalCode) as ADRS from SorMasterRep where SorMasterRep.InvoiceNumber='" + _inv + "'";
                    string res = MainMDI.Find_One_Field_SYSPRO (stSql);
                    arr_infSP[II, 0] = "     Shipping ";arr_infSP[II++, 1] = res;

                    stSql = "select RTRIM (SoldToAddr1)+' ' +RTRIM ( SoldToAddr2)+' ' +RTRIM ( SoldToAddr3)+' ' +RTRIM (SoldToAddr4) +' ' +RTRIM (SoldToAddr5)+' ' +RTRIM (ShipPostalCode) from ArCustomer where Customer='" + Oreadr["CustCode"].ToString()+ "'";
                    res = MainMDI.Find_One_Field_SYSPRO(stSql);
                    arr_infSP[II, 0] = "     Billing"; arr_infSP[II++, 1] = res;

                    //stsql = " " + oreadr["customer"].tostring() + "'";
                    res = MainMDI.VIDE;// MainMDI.find_one_field_syspro(stsql);
                    arr_infSP[II, 0] = "     Quoting "; arr_infSP[II++, 1] = res;
                    arr_infSP[II, 0] = "Sales Person "; arr_infSP[II++, 1] = Oreadr["Name"].ToString(); 
        ...
        {
            for (int i = 0; i < arr.Length / 2; i++) { arr[i, 0] = ""; arr[i, 1] = ""; }
        }
      ...
      //  string stSql = "SELECT *   FROM [Orig_PSM_FDB].[dbo].[PSM_R_Rev] where RID='"+_Prj + "' and RRev_Name='" + _Rev + "' ";
            string stSql = " SELECT  PSM_R_Rev.IRRevID, PSM_R_Rev.RRev_Name, PSM_R_Rev.RID, PSM_R_Rev.shiped, PSM_R_Rev.AGency, PSM_Q_Details.Mult,LTRIM(PSM_Q_Details.[Desc]) as [DESC], PSM_Q_Details.Qty, PSM_Q_Details.Xch_Mult, " +
                           "         PSM_Q_Details.Uprice, PSM_Q_Details.Ext, PSM_R_Detail.PrimaxSN , PSM_R_Rev.Custm_PO , PSM_R_Rev.cpnyID" +
                           " FROM    PSM_R_Rev INNER JOIN PSM_R_Detail ON PSM_R_Rev.IRRevID = PSM_R_Detail.IRRev_LID INNER JOIN PSM_Q_Details ON PSM_R_Detail.Qdet_LID = PSM_Q_Details.Detail_LID " +
                           " WHERE    (PSM_R_Rev.shiped<>'C' ) AND (PSM_R_Rev.shiped<>'D' ) AND (PSM_R_Rev.RID = '" + _Prj + "') AND (PSM_R_Rev.RRev_Name = '" + _Rev + "') AND (PSM_Q_Details.Ext <> 0) "; 
          ...
          //  lvINV_PGC.Items.Clear();
            ...
            while (Oreadr.Read())
            {
                if (arr_infPGC[0, 0]=="") 
                {
                    ...
                    string _stSql = " SELECT  PSM_Q_IGen.Quot_Req +':   ' +   PSM_COMPANY.M_Adrs " +
                                  " FROM PSM_R_Rev INNER JOIN PSM_Q_IGen ON PSM_R_Rev.iQID = PSM_Q_IGen.i_Quoteid INNER JOIN  PSM_COMPANY ON PSM_Q_IGen.Quot_Req = PSM_COMPANY.Cpny_Name1 " +
                                  " WHERE        (PSM_R_Rev.shiped <> 'C') AND (PSM_R_Rev.shiped <> 'D') AND (PSM_R_Rev.RID = '" + _Prj + "') AND (PSM_R_Rev.RRev_Name = '" + _Rev + "')";
                    arr_infSP[3, 1] = MainMDI.Find_One_Field(_stSql);
                    fill_dgInfoSP ();

                    arr_infPGC[II, 0] = "CUSTOMER "; arr_infPGC[II++, 1] = "";
                    arr_infPGC[II, 0] = "Project# "; arr_infPGC[II++, 1] = Oreadr["RID"].ToString();
                    arr_infPGC[II, 0] = "Revision "; arr_infPGC[II++, 1] = Oreadr["RRev_Name"].ToString();
                    
                    string _mltpl="", _act="", cpnyName="";
                    stSql = "SELECT   'CAN=' + cast (PSM_CmpnyTYPE.multpl1 as nvarchar) + '  /  US=' +cast (PSM_CmpnyTYPE.multpl1_US as nvarchar)   as t1, PSM_CmpnyTYPE.CpnyType, Cpny_Name1 FROM  PSM_COMPANY INNER JOIN  PSM_CmpnyTYPE ON PSM_COMPANY.CustomerType = PSM_CmpnyTYPE.CpnyType_ID where PSM_COMPANY.Cpny_ID =" + Oreadr["cpnyID"].ToString();
                    MainMDI.Find_2_Field (stSql, ref _mltpl, ref _act,ref cpnyName );
                    arr_infPGC[0, 1] = cpnyName;
                    arr_infPGC[II, 0] = "Activty, Mltpl. "; arr_infPGC[II++, 1] = _act + ",  " + _mltpl;
              //      arr_infPGC[II, 0] = "Multipl. "; arr_infPGC[II++, 1] = _mltpl; 

                    arr_infPGC[II, 0] = "Customer PO#"; arr_infPGC[II++, 1] = Oreadr["Custm_PO"].ToString();
                    arr_infPGC[II, 0] = "Agencies"; arr_infPGC[II++, 1] = "||"; //(Oreadr["AGency"].ToString() == "1") ? "Yes" : "No";
                    ...
                    if (Oreadr["AGency"].ToString() == "1")
                    {
                        ...
                        string Stsql = "SELECT [AG_ALL] ,[AG_Dest] ,[AG_Infl]  ,[AG_Eng]   ,[AG_PO]  FROM [Orig_PSM_FDB].[dbo].[PSM_R_REV_agCMS]   where A_CMS_REVID=" + Oreadr["IRRevID"].ToString();
            ...
            if (lvINV_PGC.Items.Count > 0)
            {
                ...
                arr_infPGC[II, 0] = "Category Total "; arr_infPGC[II++, 1] = "||";
                arr_infPGC[II, 0] = "     A         "; arr_infPGC[II++, 1] = ddAA.ToString();
                arr_infPGC[II, 0] = "     B         "; arr_infPGC[II++, 1] = ddBB.ToString();
                arr_infPGC[II, 0] = "     C         "; arr_infPGC[II++, 1] = ddCC.ToString();
                arr_infPGC[II, 0] = "     D         "; arr_infPGC[II++, 1] = ddDD.ToString();
     ...
     //                             fill PGC price   , Sales price, agents price   OVRG
       ...
       //   select * from ArCustomer    Bill to adress
        ...
        bool RevHasAgencies(string _Prj_Rev)
        {
            ...
            if (_Rev.Length == 4) _Rev = _Rev.Substring(0, 2) + "-(" + _Rev.Substring(2, 2) + ")";
            //  string stSql = "SELECT *   FROM [Orig_PSM_FDB].[dbo].[PSM_R_Rev] where RID='"+_Prj + "' and RRev_Name='" + _Rev + "' ";
            string stSql = "SELECT [AGency]  FROM [Orig_PSM_FDB].[dbo].[PSM_R_Rev] WHERE    (PSM_R_Rev.shiped<>'C' ) AND (PSM_R_Rev.shiped<>'D' ) and (PSM_R_Rev.RID = '" + _prj + "') AND (PSM_R_Rev.RRev_Name = '" + _Rev + "') ";
...
//#############
        ...
        //    string cond = (opINV.Checked) ? " CAST(u_SalCommissionsPrimax.Invoice AS decimal) > " + Tools.Conv_Dbl(txInv.Text) : " ArInvoice.InvoiceDate >=" + MainMDI.SSV_date(dpFrom.Value.ToShortDateString()) + "  AND ArInvoice.InvoiceDate <=" + MainMDI.SSV_date(dpTo.Value.ToShortDateString());
            ...
            {
                if (pnlINV.Visible) cond = (txInv.Text != "" && txInv.Text != "000000000000000") ? " u_SalCommissionsPrimax.Invoice='" + txInv.Text + "'" : " ArInvoice.InvoiceDate >=" + MainMDI.SSV_date(dpFrom.Value.ToShortDateString()) + "  AND ArInvoice.InvoiceDate <=" + MainMDI.SSV_date(dpTo.Value.ToShortDateString());
                else if (pnlDate.Visible) cond = " ArInvoice.InvoiceDate >=" + MainMDI.SSV_date(dpFrom.Value.ToShortDateString()) + "  AND ArInvoice.InvoiceDate <=" + MainMDI.SSV_date(dpTo.Value.ToShortDateString());
            ...
            else
            {
                ...
                string stSql = " SELECT distinct u_SalCommissionsPrimax.Invoice, u_SalCommissionsPrimax.Project FROM         u_SalCommissionsPrimax INNER JOIN SalSalesperson ON u_SalCommissionsPrimax.Salesperson = SalSalesperson.Salesperson INNER JOIN ArInvoice ON u_SalCommissionsPrimax.Invoice = ArInvoice.Invoice AND (u_SalCommissionsPrimax.PriceCAD>0)  " +
                               " WHERE     (SUBSTRING(u_SalCommissionsPrimax.Invoice, 1, 1) <> '7') AND (" + cond + ") AND (ArInvoice.InvoiceBal1 = 0) and (lower([Project])<>'cigentec') AND (SUBSTRING([Project],len([Project])-1,2)='RV')  " +
                               " ORDER BY u_SalCommissionsPrimax.Invoice ";

...
if (cbSales.Text== NSD_Sales)
            ...
            try
            {
                ...
                while (Oreadr.Read())
                {
                    ...
                    if (!pass)
                    {
                        ...
                        string custRES = MainMDI.Find_One_Field("select Customer_SPcode from SalSal_OSI_Y where Salesperson='" + Oreadr["IntSaleperson"].ToString() + "' and Terri='" + Oreadr["Saleperson"].ToString() +"'");
        ...
        void Check_Quota_Access()
        {
            ...
            string stSql = " SELECT dbo_SalCommissionSalesThreshold.Threshold FROM dbo_SalCommissionSalesThreshold " +
               " WHERE (((dbo_SalCommissionSalesThreshold.ExtSalesperson)='" + lSalesCode.Text + "') AND ((dbo_SalCommissionSalesThreshold.Active)=True))";
            Quotas = Tools.Conv_Dbl(MainMDI.Find_One_Field_ACCESS(stSql));

            stSql = " SELECT dbo_SalCommissionDefaultRates.Rate FROM dbo_SalCommissionDefaultRates WHERE (((dbo_SalCommissionDefaultRates.RateCode)='X'))";
        ...
        void Check_Quota()
        {
            ...
            string stSql = " SELECT SalCommissionSalesThreshold.Threshold FROM SalCommissionSalesThreshold " +
               " WHERE (((SalCommissionSalesThreshold.ExtSalesperson)='" + lSalesCode.Text + "') AND ((SalCommissionSalesThreshold.Active)=1))";
            Quotas = Tools.Conv_Dbl(MainMDI.Find_One_Field_SYSPRO(stSql));

            stSql = " SELECT SalCommissionDefaultRates.Rate FROM SalCommissionDefaultRates WHERE (((SalCommissionDefaultRates.RateCode)='X'))";
...
// ORDER BY Project
           ...
           try
            {
                ...
                while (Oreadr.Read())
                {
                    ...
                    if (Quotas > 0)
                    {
                        if (!isThresholdOK(txSalesName.Text, Oreadr["DateLastInvPrt"].ToString(), MM, Oreadr["Invoice"].ToString(), Oreadr["SalesOrderLine"].ToString(), ref diff)) amt = 0;
        ...
        string find_QT_date(string _project)
        {
           ...
           string dat = "?????",rid="";
            ...
            if (Tools.Conv_Dbl (rid) >100)   dat = MainMDI.Find_One_Field("SELECT PSM_Q_IGen.Opndate FROM   PSM_R_Rev INNER JOIN       PSM_Q_IGen ON PSM_R_Rev.iQID = PSM_Q_IGen.i_Quoteid	 where RID=" + rid);
        ...
        bool isThresholdOK(string _SalesName,  string _InvDate ,string _MM, string _Invoice, string _SOL   ,ref double diff)
        {
            ...
            if (Quotas != 0) 
            {
                for (i = 0; i < LimX; i++)
                {
                    ...
                    if (arr_Quotas[i, 0] == _SalesName && arr_Quotas[i, 1] == _InvDate && arr_Quotas[i, 2] == _MM && arr_Quotas[i, 3] == _Invoice && arr_Quotas[i, 4] == _SOL)
                    {
        ...
        void Load_ventesAn(string _SalesName, int _YY)
        {
            ...
            string stSQL = " SELECT SalSalesperson.Name, u_SalCommissionsPrimax.DateLastInvPrt, u_SalCommissionsPrimax.FiscalMonth, u_SalCommissionsPrimax.Invoice, " +
                           "       u_SalCommissionsPrimax.SalesOrderLine, u_SalCommissionsPrimax.ShipQty, [PriceCAD]*[ShipQty] AS TOTAL " +
                           " FROM u_SalCommissionsPrimax INNER JOIN SalSalesperson ON (u_SalCommissionsPrimax.Branch = SalSalesperson.Branch) AND (u_SalCommissionsPrimax.Salesperson = SalSalesperson.Salesperson) " +
   ...
   //                        " ORDER BY dbo_u_SalCommissionsPrimax.FiscalMonth, dbo_u_SalCommissionsPrimax.Invoice ";
        ...
        void Load_ventesAn_Access(string _SalesName, int _YY)
        {
            ...
            string stSQL = " SELECT dbo_SalSalesperson.Name, dbo_u_SalCommissionsPrimax.DateLastInvPrt, dbo_u_SalCommissionsPrimax.FiscalMonth, dbo_u_SalCommissionsPrimax.Invoice, " +
                           "       dbo_u_SalCommissionsPrimax.SalesOrderLine, dbo_u_SalCommissionsPrimax.ShipQty, [PriceCAD]*[ShipQty] AS TOTAL " +
                           " FROM dbo_u_SalCommissionsPrimax INNER JOIN dbo_SalSalesperson ON (dbo_u_SalCommissionsPrimax.Branch = dbo_SalSalesperson.Branch) AND (dbo_u_SalCommissionsPrimax.Salesperson = dbo_SalSalesperson.Salesperson) " +
                           " WHERE (((dbo_SalSalesperson.Name)='" + _SalesName + "') AND ((dbo_u_SalCommissionsPrimax.FiscalYear)=" + cb_YY.Text + ") AND ((dbo_u_SalCommissionsPrimax.Salesperson)<>'A')) " +
                           " ORDER BY dbo_u_SalCommissionsPrimax.FiscalMonth, dbo_u_SalCommissionsPrimax.Invoice ";
...
//Visibility
           ...
           // groupBox2.Visible = (MainMDI.User.ToLower() == "ede");
            ...
            tlsRepair.Visible = (MainMDI.User.ToLower() == "ede");
       ...
       //     cb_YY.Text = cb_YY.Items[0].ToString();  
        ...
        {
            if (MainMDI.User.ToLower() != "amvoinescu")
            {
        ...
        {
            if (MainMDI.User.ToLower() == "ede" || MainMDI.User.ToLower() == "mmellouli" || MainMDI.User.ToLower() == "hnasrat" || MainMDI.User.ToLower() == "ddarai")
                   Fill_TOT_bySALESName();
            ...
            for (int i = 0, T = 0; i < cbSales.Items.Count; i++, T++)
            {
                ...
                switch (lSalesN.Text)
                {
                    case "Inside Sales Names":
                        if (cbSales.Items[i].ToString() == NSD_Sales)
                ...
                switch (lSalesN.Text)
                {
                    case "Inside Sales Names":
                        stSql = " SELECT Sum(InsideSales.Amt) AS TOT FROM InsideSales   " +
                                " WHERE InsideSales.IntSalesperson ='" + cbSales.Items[i].ToString() + "' AND InsideSales.FiscalYear=" + cb_YY.Text + " AND InsideSales.FiscalMonth=" + MM;
                    ...
                    case "Outside Sales Names":
                        stSql = " SELECT   sum(CommissionAmt1) AS 1CommissionAmt1 FROM dbo_u_SalCommissionsPrimax INNER JOIN dbo_SalSalesperson ON (dbo_u_SalCommissionsPrimax.Salesperson = dbo_SalSalesperson.Salesperson) AND (dbo_u_SalCommissionsPrimax.Branch = dbo_SalSalesperson.Branch) " +
                               " WHERE dbo_SalSalesperson.Name='" + cbSales.Items[i].ToString() + "' AND dbo_u_SalCommissionsPrimax.FiscalYear=" + cb_YY.Text + " AND dbo_u_SalCommissionsPrimax.FiscalMonth=" + MM + " AND dbo_u_SalCommissionsPrimax.Salesperson<>'A'";
        ...
        ////    cb_MM.Text = cb_MM.Items[0].ToString();
        ////    cb_YY.Text = cb_YY.Items[0].ToString();  
            if (MainMDI.User.ToLower() == "ede" || MainMDI.User.ToLower() == "hnasrat" || MainMDI.User.ToLower() == "ylavoie"  || MainMDI.User.ToLower() == "amvoinescu")
            {
        ...
        string get_Curr_agency(string AG)
        {
            ...
            string stt = "SELECT distinct Salesperson + ' - ' + Name FROM  dbo.SalSalesperson where Salesperson ='" + AG+"' and(Branch = 'U1' OR Branch = 'C1') ";
            return (MainMDI.Find_One_Field_SYSPRO(stt));
        ...
        void Send_email()
        {
            if (MainMDI.User.ToLower() == "ede" || MainMDI.User.ToLower() == "mmellouli" || MainMDI.User.ToLower() == "hnasrat")
            {
                ...
                string msg = "Bonjour,\n  " + sbj + " ont étés importées de SYSPRO. \n Veuillez vérifier l’état des territoires de chaque facture dans :  INVOICE LISTE (PGESCOM). \n " +
                            "Si vous voulez faire une correction envoyez le fichier EXCEL corrigé à Hakim ,  sinon veuillez aviser Mohammed (comptabilité)  pour procéder à la distributions. \n Merci ";
                string TO = "ylavoie@primax-e.com,bcimon@primax-e.com,mmellouli@primax-e.com,cfouche@primax-e.com,mrouleau@primax-e.com,mdimassi@primax-e.com,bmustapha@primax-e.com,blombard@primax-e.com,mmaturi@primax-e.com,hedebbab@primax-e.com,amvoinescu@primax-e.com";
                string CC = "mmellouli@primax-e.com,cfouche@primax-e.com,mrouleau@primax-e.com,mdimassi@primax-e.com,bmustapha@primax-e.com,hedebbab@primax-e.com";
                MainMDI.send_email("PGESCOM@primax-e.com", TO, sbj, msg);

                MainMDI.Exec_SQL_JFS(" update PSM_C_GConfig set [f2]='" + cb_MM.Text + "', [f3]='" + cb_YY.Text + "'  where F1_code='CMS_MMYY' ", "CMS Calc. - Accounting");
 ...
 * */
        ...
        private string Seek_CMS_TOT_MESA(string SP_Customer)
        {
            ...
            double TOT_CMS = 0;
            for (int i = 0; i < ed_lvITM.Items.Count; i++) if (ed_lvITM.Items[i].SubItems[3].Text.Substring (0,7) == SP_Customer)    TOT_CMS += Tools.Conv_Dbl(ed_lvITM.Items[i].SubItems[9].Text);
        ...
        private void Seek_CMS(int seekCol)
        {
            ...
            if (!found) { MessageBox.Show("Sorry, Not Found !!!..."); ndx_CMS = -1; }
        }
        private void Seek_INV(int seekCol)
        {
            ...
            if (!found) { MessageBox.Show("Sorry, Not Found !!!..."); ndx_INV = -1; }
        }

        void reset_backcolor(char IC)
        {

            if (IC == 'I') if (old_ndx_INV > -1) ed_LVallInvoices.Items[old_ndx_INV].BackColor = Color.Honeydew;


            if (IC == 'C') if (old_ndx_CMS > -1) ed_lvITM.Items[old_ndx_CMS].BackColor = Color.Wheat;
        ...
        private void tsbSearch_Click(object sender, EventArgs e)
        {
            ...
            string _pkt="usr=" + MainMDI.User.ToLower();

...
namespace PGESCOM
{
	...
	/// </summary>
	public class Charger_UPSCOST_P850i
	{
		...
		public Charger_UPSCOST_P850i(ManualResetEvent x_Stop, ManualResetEvent x_Stopped, UPS_maker x_frm_UPS_maker)
		{
			//
		...
		private void fill_VDC_IDC(char c)
		{
			...
			for (i=0;i<100;i++) if (c=='V') arr_VDC[i++]=""; else        arr_IDC[i++]="";
			i=0;
			int ic=1;
			while (Oreadr.Read ())	if (c=='V') arr_VDC[i++]=Oreadr[0].ToString(); else  arr_IDC[ic++]=Oreadr[0].ToString();
		...
		public void Cal_ALL_UPS_COST13()
		{
			...
			while (Oreadr.Read ())
			{
				CHARGER_UPS_COST( Oreadr["UPS"].ToString(),Oreadr["PHSout"].ToString(),Oreadr["OP_KVA"].ToString(), Oreadr["ACoutputV"].ToString(), Oreadr["DCBus"].ToString(), Oreadr["PHSin"].ToString(), Oreadr["ACinputV"].ToString(), Oreadr["PHSbps"].ToString(), Oreadr["BpsinputV"].ToString()); 
	...
	//		XL_ALL_CHRGR13();
		...
		private void CHARGER_UPS_COST(string UPS, string PHSout, string OP_KVA, string ACoutputV,string DCBus,string PHSin,string ACinputV, string PHSbps, string BpsinputV)
		{
			//fill CHARGERS_COST0
			...
			Ocmd.CommandText = stSql ;
			SqlDataReader Oreadr = Ocmd.ExecuteReader();
			CHRGR_UPS  =new Charger_UPS(0 ,UPS ,PHSout,OP_KVA,ACoutputV,DCBus,PHSin,ACinputV,PHSbps,BpsinputV );

			//MainMDI.ExecSql("delete CHARGERS_COST0"+PHS );
			while (Oreadr.Read ())
			{
				CPT_UPS_COST( Convert.ToInt32(Oreadr["Component_ID"].ToString()),Charger.AvailId,PHS,'F');
		...
		private void CPT_UPS_COST(long dccompnt,long availID, char Cd)
		{
			//fill CHARGERS_COST0
##########################
				...
				while (Oreadr.Read ())
				{
					...
					if (Cpt.G_PRICE != Charger.VIDE )
					{
						...
						if (Cd=='D')
						{
							...
							in_frm_UPS_maker.CH_COST+=Tools.Conv_Dbl(Cpt.G_PRICE) ; 
						}
						else 
						{
		...
		private void writeTBXL( string CAP_REF, string[] arr_val, string opera, string p, string c , string v, ref string[] arr_TOT_GEN, string cRec )
		{	
				...
				if (opera[0] == '%')
				{
					if (arr_val[i] != "" && arr_val[i] != MainMDI.VIDE  )
					{
						...
						string rstPrctV= MainMDI.Find_One_Field("SELECT CHARGERS_COST0" + p + ".COST, CHARGERS_COST0" + p + ".cost_type " +
							...
							" WHERE (((COMPNT_LIST.COMPONENT_REF)='" + CAP_REF + "') AND ((TBLAVAIL" + p + ".charger)='" + c + "') AND (vdc='" + v + "') AND ((TBLAVAIL" + p + ".idc)='" + arr_IDC[i] + "') AND ((CHARGERS_COST0" + p + ".cost_type)='%') AND ((COMPNT_LIST.nbc3Cat)='B')) " +
		...
		{
			string stSql = " SELECT COMPNT_LIST.COMPONENT_REF FROM (CHARGERS_COST0" + p + " INNER JOIN TBLAVAIL" + p + " ON CHARGERS_COST0" + p + ".Avail_ID = TBLAVAIL" + p + ".Avail_ID) INNER JOIN COMPNT_LIST ON CHARGERS_COST0" + p + ".Compnt_ID = COMPNT_LIST.Component_ID " + 
		...
		private void XL_ALL()
		{
			...
			string CHRG=MainMDI.Find_One_Field("SELECT  TBLAVAIL" + P + ".charger FROM link_COMPNT_AVAIL INNER JOIN TBLAVAIL" + P + " ON link_COMPNT_AVAIL.Avail_ID = TBLAVAIL" + P + ".Avail_ID " +

...
namespace PGESCOM
{
	...
	public class Print_label
	{
		...
		public Print_label(char x_doctype, string x_lbl1, string x_lbl2, string x_lbl3, string x_prtNme, Order x_ORFrm, QuoteV2 x_QFrm)
		{
			In_ORFrm = x_ORFrm;
			In_QFrm = x_QFrm;
		...
		*/
		...
		private void Page_BILs_Details(char BorF)
		{
			...
			string stSql = " SELECT PSM_R_SBills.BilNm AS Expr1, PSM_R_SBillsDetail.*, PSM_R_SBills.Bil_LID AS Expr2 " +
				" FROM PSM_R_SBills INNER JOIN PSM_R_SBillsDetail ON PSM_R_SBills.Bil_LID = PSM_R_SBillsDetail.b_BilLID " +
				" WHERE     (PSM_R_SBills.b_RRevLID = " + In_ORFrm.lIRRevID.Text + 
				") AND (PSM_R_SBillsDetail.b_d_affID <> 0) AND (PSM_R_SBills.BilNm = '" + In_ORFrm.lcurBilNm.Text + "') " +
				" ORDER BY PSM_R_SBills.b_Rnk, PSM_R_SBillsDetail.b_d_Rnk ";
			...
			while (Oreadr.Read()) 
			{
				...
				m2Desc = (BorF == 'F') ? Oreadr["b_d_ItemDesc"].ToString() : Convert.ToString(daff++) + "- " + 
					Oreadr["b_d_ItemDesc"].ToString();

...
namespace PGESCOM
{
    public partial class QuoteV3 : Form
    {
        char TV_RSA = 'V';
        ...
        bool debut = true;
        string XLName = "";
        string CelFrom = "", CelTo = "";
        string[,] Idata;
        int XL_MaxRow = 21, XL_MaxCol = 94;
        ...
        public QuoteV3(long x_QID, string x_CpnyName, char x_opera)
		{
            //
        ...
        // Loads the quote multiplier from the db, and set the quote field input
        private void LoadMultiplierField()
        {
            string stSql = "SELECT Cust_Mult FROM PSM_Q_IGen WHERE Quote_ID = @Quote_ID";
            try
            {
                SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
                OConn.Open();
                SqlCommand Ocmd = OConn.CreateCommand();
                Ocmd.Parameters.AddWithValue("@Quote_ID", tQuoteID.Text);
                Ocmd.CommandText = stSql;
                SqlDataReader Oreadr = Ocmd.ExecuteReader();
                while (Oreadr.Read())
                {
                    double queriedMultiplier = Oreadr.GetDouble(0);
                    Console.WriteLine(queriedMultiplier);
                    if (queriedMultiplier != 1)
                    {
                        tCust_Mult.Text = queriedMultiplier.ToString();
                    }
                }
            }
        ...
        private void cbContacts_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            string[] arr_Val = new string[9] { "", "", "", "", "", "", "", "", "" };
            string stSql = "SELECT PSM_Contacts.Contact_ID, PSM_Prefix.Prefix, PSM_Contacts.[First_Name], PSM_Contacts.Last_Name, PSM_Contacts.JOBTitle, Extension,Main_TEL,PSM_Contacts.[Fax Number],PSM_Contacts.[Email Address] " +
                " FROM PSM_Contacts INNER JOIN PSM_Prefix ON PSM_Contacts.Prefix_ID = PSM_Prefix.[Prefix ID]  " +
                " WHERE ([PSM_Contacts].[First_Name] + ' ' + [PSM_Contacts].[Last_Name])='" + cbContacts.Text.Replace("'", "''") + 
                "' and JOBTitle<>'~~' and Company_ID=" + lcpnyID.Text;
        ...
        private void majContact()
        {
            ...
            string stSql = "SELECT PSM_Contacts.Contact_ID, PSM_Prefix.Prefix, PSM_Contacts.[First_Name], PSM_Contacts.Last_Name, PSM_Contacts.JOBTitle, Extension " +
                " FROM PSM_Contacts INNER JOIN PSM_Prefix ON PSM_Contacts.Prefix_ID = PSM_Prefix.[Prefix ID]  WHERE ([PSM_Contacts].[First_Name] + ' ' + [PSM_Contacts].[Last_Name])='" + 
                cbContacts.Text.Replace("'", "''") + "' ";
        ...
        private void import_OldQInfo(string r_IQID)
        {
            string stSql = "SELECT PSM_Q_IGen.Quote_ID, PSM_Q_IGen.ProjectName, PSM_SALES_AGENTS.First_Name + ' ' + PSM_SALES_AGENTS.Last_Name AS Emp, PSM_SALES_AGENTS_1.First_Name + ' ' + PSM_SALES_AGENTS_1.Last_Name AS IPMGR, PSM_Q_IGen.curr, PSM_Q_IGen.Lang,SP_AG2_id " +
                " FROM (PSM_Q_IGen INNER JOIN PSM_SALES_AGENTS ON PSM_Q_IGen.Employ_ID = PSM_SALES_AGENTS.SA_ID) INNER JOIN PSM_SALES_AGENTS AS PSM_SALES_AGENTS_1 ON PSM_Q_IGen.IPmgr = PSM_SALES_AGENTS_1.SA_ID WHERE (((PSM_Q_IGen.i_Quoteid)=" + 
                r_IQID + "))";
            SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
            OConn.Open();
            SqlCommand Ocmd = OConn.CreateCommand();
        ...
        private void cpy_Sol(string OldQid, string NewQid, string OldSlid)
        {
            ...
            while (Oreadr.Read())
            {
                stSql = "INSERT INTO PSM_Q_SOL ([I_Quoteid],[Sol_Name],[img], [Rnk]," +
                    ...
                    Oreadr["img"].ToString() + "', '" + Oreadr["Rnk"].ToString() + "', '" + MainMDI.User + "', " + 
                    MainMDI.SSV_date(System.DateTime.Now.ToShortDateString()) + ")";
                ...
                stSql = MainMDI.Find_One_Field("select Sol_LID from PSM_Q_SOL where I_Quoteid=" + NewQid + " AND Sol_Name='" + 
                    Oreadr["Sol_Name"].ToString().Substring(0, 2) + "-00" + "' and Rnk=" + Oreadr["Rnk"].ToString());
        ...
        private void Cpy_SPEC(string OldSlid, string NewSlid)
        {
            ...
            while (Oreadr.Read())
            {
                ...
                MainMDI.ExecSql(stSql);
                MainMDI.Write_JFS(stSql);
                stSql = MainMDI.Find_One_Field("select SPC_LID from PSM_Q_SPCS where Sol_LID=" + NewSlid + " AND SPC_Name='" + 
                    Oreadr["SPC_Name"].ToString().Replace("'", "''") + "' and Rnk=" + Oreadr["Rnk"].ToString());
        ...
        private void Cpy_ALS(string OldSpcId, string NewSpcId)
        {
            ...
            while (Oreadr.Read())
            {
                ...
                stSql = MainMDI.Find_One_Field("select ALS_LID from PSM_Q_ALS where SPC_LID=" + NewSpcId + " AND ALS_Name='" + 
                    Oreadr["ALS_Name"].ToString().Replace("'", "''") + "' and Rnk=" + Oreadr["Rnk"].ToString());
        ...
        private void add_ALRM_EQ(string CH_FRML)
        {
            ...
            if (AlrmEQ.lSave.Text == "Y")
            {
                for (int i = 0; i < AlrmEQ.lvAlrmPL.Items.Count; i++)
                {
                    ...
                    {
                        ItemCount++;
                        add_LVO(1, 0, ItemCount.ToString(), AlrmEQ.lvAlrmPL.Items[i].SubItems[1].Text, "1", tCust_Mult.Text, 
                            AlrmEQ.lvAlrmPL.Items[i].SubItems[2].Text, 
                            Convert.ToString(Math.Round(Tools.Conv_Dbl(AlrmEQ.lvAlrmPL.Items[i].SubItems[2].Text) * 
                            Tools.Conv_Dbl(tCust_Mult.Text), MainMDI.Q_NB_DEC_AFF)), "04-06", "ALEQ_???", 
                            AlrmEQ.lvAlrmPL.Items[i].SubItems[3].Text, "A");
                    }
                }
                Ref_ALSTOT('A');
            }
                ...
                if (btnOK(btn))
                {
                    ...
                    if (btn == 3) //|| btn == 20)
                    {
                        ...
                        else
                        {
                            if (tQuoteID.Text != "")
                            {
                                string stId = MainMDI.Find_One_Field("select I_Quoteid from PSM_Q_IGen where Quote_ID=" + tQuoteID.Text + 
                                    " AND CPNY_ID=" + lcpnyID.Text);
                            ...
                            { 
                                MessageBox.Show("Quote ID is empty...."); 
                                tQuoteID.Focus(); 
                            }
                        }
                    }
                    ...
                    {
                        if ((btn == 21) || (lCurrIQID.Text != "0" && tQuoteID.Text != "" && (lcurSol_Status.Text != "C" || btn == 7 || 
                            btn == 4)))
                        {
                            switch (btn)
                            {
                                ...
                                case 5:
                                    if (lTVSel.Text == "Y")
                                    {
                                        ...
                                        if (LBL_Exist_Newa(lCurrNAME.Text)) lCurrNAME.Text = "Alt#" + 
                                                Convert.ToString(tvSol.SelectedNode.Nodes.Count + 1);
                                ...
                                case 6:
                                    if (lTVSel.Text == "Y")
                                    {
                                        ...
                                        lCurrNAME.Text = "New_" + MainMDI.arr_EFSdict[38, MainMDI.Lang] + "#" + 
                                            Convert.ToString(tvSol.SelectedNode.Nodes.Count + 1);
                                        if (LBL_Exist_Newa(lCurrNAME.Text)) lCurrNAME.Text = MainMDI.arr_EFSdict[38, MainMDI.Lang] + "#" + 
                                                Convert.ToString(tvSol.SelectedNode.Nodes.Count + 1);
                                        tvSol.SelectedNode.Nodes.Add(lCurrNAME.Text);
                                        tvSol.SelectedNode.Expand();
                                ...
                                case 8:
                                    if (lTVSel.Text == "Y")
                                    {
                                        DialogResult dr = MessageBox.Show("Do You want to DELETE : " + tvSol.SelectedNode.Text, 
                                ...
                                case 15: //add alarms
                                    if (lvQITEMS.SelectedItems.Count > 0 && 
                                        lvQITEMS.SelectedItems[0].SubItems[12].Text.IndexOf("n/a U_CHARGER||") > -1)
                                    {
                                ...
                                case 17:  //Del Current Als
                                    ...
                                    else if (MainMDI.Confirm("WANT TO DELETE : " + tvSol.SelectedNode.Text + " ?  ")) 
                                        del_Als(lCurSoln.Text, lCurSPCn.Text, lCurALSn.Text);
                                    Ref_ALSTOT('D');
                                    break;
                                case 18: //PBsizing
                                    try
                                    ...
                                    {
                                        MessageBox.Show("Cannot Find PBSIZING.EXE at path: " + MainMDI.PBSPath + " System-msg: " + 
                                            Oexp.Message);
                                    }
                                    break;
                                case 19: //Print
                                    ...
                                    {
                                        //test(MainMDI.Lang);
                                        string solId = MainMDI.Find_One_Field("select  Sol_LID from PSM_Q_SOL where I_Quoteid=" + 
                                            lCurrIQID.Text + " and Sol_Name='" + lCurSoln.Text.Replace("'", "''") + "'");
                                        ...
                                        if (FC.NXT)
                                        {
                                            ...
                                            grpPB.Visible = true;
                                            grpPB.Refresh();
                                            FichWord_3 FW = new FichWord_3(this, FC);
                                            FW.Wexport();
        ...
        {
            string res = MainMDI.Find_One_Field("select status_Rev from PSM_Q_SOL where I_Quoteid=" + iqid + " and Sol_Name='" + 
                revName.Replace("'", "''") + "'");
            return (res == "C");
        }
        ...
        private void Sol_Rep_SPPOLD(char s)
        {
            ...
            switch (s)
            {
                case 'V':
                    ...
                    if (LBL_Exist_Newa(lCurrNAME.Text)) lCurrNAME.Text = "RV-" + tvSol.Nodes.Count.ToString() + 
                            Convert.ToString(tvSol.Nodes.Count + 1);
                    nb = 2;
                    break;
                case 'S':
                    lCurrNAME.Text = "SP-" + tvSol.Nodes.Count.ToString();
                    if (LBL_Exist_Newa(lCurrNAME.Text)) lCurrNAME.Text = "SP-" + tvSol.Nodes.Count.ToString() + 
                            Convert.ToString(tvSol.Nodes.Count + 1);
                    nb = 4;
                    break;
                case 'R':
            ...
            if (in_opera == 'C')
            {
                if (!lvQITEMS.Items[e.Index].Checked)
                {
                    if (in_opera == 'C' && lvQITEMS.Items[e.Index].SubItems[7].Text != "")
                        if (seek_LvOrder(lvQITEMS.Items[e.Index].SubItems[11].Text, 'c') == -1) 
                            add_LVR("      " + lvQITEMS.Items[e.Index].SubItems[2].Text, lCurSolNDX.Text, lCurSPCNDX.Text, lCurALSNDX.Text, 
                                lvQITEMS.Items[e.Index].SubItems[11].Text, e.Index.ToString(), lCurSPCn.Text + "/" + lCurALSn.Text, 
                                lvQITEMS.Items[e.Index].SubItems[7].Text);
                }
        ...
        private void cbCompanyy_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            MainMDI.Find_2_Field("select BLack_List,  BL_Cmnt, BL_usr  from PSM_COMPANY Where Cpny_Name1='" + cbCompanyy.Text.Replace("'", 
                "''") + "'", ref InBL, ref BLcmnt, ref usr);
        ...
        */
        ...
        void switchRSA()
        {
            switch (TV_RSA)
            {
                ...
                case 'A':
                    ...
                    toolBar1.Items[25].Visible = true;
                    break;
            }
        }
        ...
        private void tvSol_Click(object sender, System.EventArgs e)
        {
            ...
            lTVSel.Text = "Y";
            if (tvSol.SelectedNode != null) 
                if (tvSol.SelectedNode.ImageIndex == 0 || tvSol.SelectedNode.ImageIndex == 3) tvSol.SelectedNode.SelectedImageIndex = 0;
        }
        ...
        private void cbEmploy_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            string[] arr_Val = new string[6] { "", "", "", "", "", "" };
            string stSql = "select SA_ID ,Extension,sfx,Email_Address from PSM_SALES_AGENTS where (First_Name + ' ' + Last_Name) ='" + 
                cbEmploy.Text.Replace("'", "''") + "'";
            ...
            else
            {
                ...
                //Permet de corriger l'adresse email
                lemail.Text = lemail.Text.Replace("-e", "power");
            }
        }
        ...
        private void tvSol_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
        {
            ...
            switch (tvSol.SelectedNode.ImageIndex)
            {
                ...
                //case 4:
                    ...
                    MainMDI.Find_2_Field("select status_Rev ,date_Rev from PSM_Q_SOL where I_Quoteid=" + lCurrIQID.Text + " and Sol_Name='" + 
                        lCurSoln.Text.Replace("'", "''") + "'", ref st1, ref st2);
                ...
                case 3:
                    ...
                    if (oldsysNDX != -1)
                        if (oldsysNDX < tvSol.Nodes[oldsolNDX].Nodes[oldspcNDX].Nodes.Count) 
                            tvSol.Nodes[oldsolNDX].Nodes[oldspcNDX].Nodes[oldsysNDX].BackColor = Color.WhiteSmoke;
                    ...
                    if (Tools.Conv_Dbl(lCurrIQID.Text) > 0)
                    {
                        ...
                        MainMDI.Find_2_Field("select status_Rev ,date_Rev from PSM_Q_SOL where I_Quoteid=" + lCurrIQID.Text + 
                            " and Sol_Name='" + lCurSoln.Text.Replace("'", "''") + "'", ref st1, ref st2);
                        lcurSol_Status.Text = st1; lrevDATE.Text = "Rev. Date: " + MainMDI.Eng_date(st2, "/");
                    }
        ...
        private void del_Spc(string sName, string pName)
        {
            string stSql = "SELECT PSM_Q_SPCS.SPC_LID FROM PSM_Q_SOL INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID " +
                " WHERE PSM_Q_SOL.Sol_Name='" + sName.Replace("'", "''") + "' AND PSM_Q_SPCS.SPC_Name='" + pName.Replace("'", "''") + 
                "' AND PSM_Q_SOL.I_Quoteid=" + lCurrIQID.Text;
        ...
        private void del_Als(string sName, string pName, string aName)
        {
            ...
            string stSql = "SELECT PSM_Q_ALS.ALS_LID FROM (PSM_Q_SOL INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID) INNER JOIN PSM_Q_ALS ON PSM_Q_SPCS.SPC_LID = PSM_Q_ALS.SPC_LID " +
                " WHERE PSM_Q_SOL.Sol_Name='" + sName.Replace("'", "''") + "' AND PSM_Q_SPCS.SPC_Name='" + pName.Replace("'", "''") + 
                "' AND PSM_Q_ALS.ALS_Name='" + aName.Replace("'", "''") + "' AND PSM_Q_SOL.I_Quoteid=" + lCurrIQID.Text;
            string st = MainMDI.Find_One_Field(stSql);
        ...
        private void Reo_ALS()
        {
            ...
            string SpcLid = MainMDI.Find_One_Field(" SELECT PSM_Q_ALS.SPC_LID " +
                ...
                " WHERE PSM_Q_SPCS.SPC_Name ='" + lCurSPCn.Text + "' AND PSM_Q_SOL.I_Quoteid =" + lCurrIQID.Text + 
                " AND PSM_Q_SOL.Sol_Name ='" + lCurSoln.Text + "'");
            if (SpcLid != MainMDI.VIDE)
            {
        ...
        private void fill_cb_Contacts(long cpnyID)
        {
            ...
            string stSql = (in_opera == 'N') ? "select * FROM PSM_Contacts  where  Company_ID=" + cpnyID + 
                "and JOBTitle<>'~~' Order by First_Name" : "select * FROM PSM_Contacts  where  Company_ID=" + cpnyID + 
                "  Order by First_Name";
            SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
            OConn.Open();
            SqlCommand Ocmd = OConn.CreateCommand();
        ...
        private void fill_cb_Terms()
        {
            ...
            cbTerms.Items.Clear();
            while (Oreadr.Read()) 
                cbTerms.Items.Add(Oreadr[0].ToString());
            OConn.Close();
        }
        ...
        private void fill_cb_Via()
        {
            ...
            while (Oreadr.Read()) 
                cbShipVia.Items.Add(Oreadr[0].ToString());
            OConn.Close();
        }
        ...
        private void fill_cb_Inco()
        {
            ...
            while (Oreadr.Read()) 
                cbIncoTerm.Items.Add(Oreadr[0].ToString());
            OConn.Close();
        }
        ...
        private void Ref_GetActivy_MLTPL()
        {
            ...
            while (Oreadr.Read())
            {
                if (lActivty.Text != Oreadr["CpnyType"].ToString())
                {
                    ...
                    if (_st != MainMDI.VIDE) Console.WriteLine(_st);
        ...
        {
            bool msg_err = false;
            string stSql = "SELECT PSM_Company.*, PSM_CmpnyTYPE.multpl1, PSM_CmpnyTYPE.multpl1_US,PSM_CmpnyTYPE.multpl1_EURO,  PSM_CmpnyTYPE.CpnyType FROM PSM_Company INNER JOIN PSM_CmpnyTYPE ON PSM_Company.CustomerType = PSM_CmpnyTYPE.CpnyType_ID where  Cpny_Name1='" + 
                cpnyName.Replace("'", "''") + "'";
            SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
            OConn.Open();
            SqlCommand Ocmd = OConn.CreateCommand();
            ...
            while (Oreadr.Read())
            {
                if (adrs == '*')
                {
                    ...
                    lSP_Name.Text = MainMDI.Find_One_Field_SYSPRO("select distinct  dbo.v_PGSalesperson.Salesperson +'  ' +dbo.v_PGSalesperson.Name from dbo.v_PGCustomerXRef inner join dbo.v_PGSalesperson on dbo.v_PGSalesperson.Salesperson=dbo.v_PGCustomerXRef.Salesperson where Customer='" + 
                        Q_sysPcod.Text + "'");
                    lSalesName.Text = lSP_Name.Text;

                    //22062017
                    string sysPcurr = MainMDI.Find_One_Field_SYSPRO("SELECT [Currency]  FROM [SysproCompanyP].[dbo].[v_PGCustomerXRef] where Customer='" + 
                        Q_sysPcod.Text + "'");
                    btnSP_Currncy.Text = sysPcurr;
                    alrm_SPcurrncy.Visible = (Q_sysPcod.Text[Q_sysPcod.Text.Length - 1] =='U' && sysPcurr == "CAD");
                    if (alrm_SPcurrncy.Visible) 
                        MessageBox.Show("This quote cannot be saved because of Currency ERROR (US Customers must have USD not CAD... ");
                    //22062017

                    //07112016
                    string salP = lSP_Name.Text.Substring(0, 3);
                    string SalPemail = MainMDI.VIDE, SalPCell = MainMDI.VIDE;
                    MainMDI.Find_2_Field("select  Email_Address, [Cell Number] from PSM_SALES_AGENTS where SP_CODE ='" + salP + "'", 
                        ref SalPemail, ref SalPCell);
                    ...
                    lExtSid.Text = MainMDI.Find_One_Field_SYSPRO("select dbo.v_PGCustomerXRef.Salesperson from dbo.v_PGCustomerXRef where Customer='" + 
                        Q_sysPcod.Text + "'");
                    ...
                    //gets current MLTP if exists else it gets default One
                    string _st = MainMDI.Find_One_Field("select " + Curr_SQLMLTP + " from PSM_Cmpny_CurrMLTP where Cpny_ID=" + lcpnyID.Text);
                    //get updated multiplier if user has changed it
                    //string multiplier = MainMDI.Find_One_Field("select Cust_Mult from PSM_Q_IGen where Quote_ID=" + tQuoteID.Text);
                    if (_st != MainMDI.VIDE)
                    {
                        tCust_Mult.Text = _st;
                        /*
                        ...
                        {
                            tCust_Mult.Text = multiplier;
                            MainMDI.multiplier = multiplier;
                        }
                        */
                    }
                    else tCust_Mult.Text = STDMultp.Text;
                ...
                else
                {
                    ...
                    {
                        case 'Q':
                            lQA.Text = (Oreadr["Q_Adrs"].ToString().Length < 4) ? Oreadr["M_Adrs"].ToString() : 
                                Oreadr["Q_Adrs"].ToString().Replace("\r\n", " ");
                            break;
                        case 'S':
                            //lSA.Text = Oreadr["S_Adrs"].ToString();
                            lSA.Text = (Oreadr["S_Adrs"].ToString().Length < 4) ? Oreadr["M_Adrs"].ToString() : 
                                Oreadr["S_Adrs"].ToString().Replace("\r\n", " ");
                            break;
                        case 'I':
                            //lIA.Text = Oreadr["I_Adrs"].ToString();
                            lIA.Text = (Oreadr["I_Adrs"].ToString().Length < 4) ? Oreadr["M_Adrs"].ToString() : 
                                Oreadr["I_Adrs"].ToString().Replace("\r\n", " ");
                            break;
                        case 'P':
                            //lPA.Text = Oreadr["P_Adrs"].ToString();
                            lPA.Text = (Oreadr["P_Adrs"].ToString().Length < 4) ? Oreadr["M_Adrs"].ToString() : 
                                Oreadr["P_Adrs"].ToString().Replace("\r\n", " ");
                            break;
                    }
                }
            }
        ...
        private void fill_details()
        {
            ...
            string stSql = "SELECT PSM_Q_Details.*, PSM_Q_SOL.I_Quoteid, PSM_Q_SOL.Sol_Name, PSM_Q_SPCS.SPC_Name, PSM_Q_ALS.ALS_Name, PSM_Q_ALS.PxPrice,PSM_Q_ALS.AGPrice ,PSM_Q_ALS.AlsQty,PSM_Q_ALS.SV_Ovrg, PSM_Q_SOL.Sol_LID, PSM_Q_SPCS.SPC_LID " +
                ...
                " WHERE (PSM_Q_SOL.I_Quoteid=" + lCurrIQID.Text + " AND PSM_Q_SOL.Sol_Name='" + lCurSoln.Text + 
                "' AND PSM_Q_SPCS.SPC_Name='" + lCurSPCn.Text.Replace("'", "''") + "' AND PSM_Q_ALS.ALS_Name='" + 
                lCurALSn.Text.Replace("'", "''") +
            ...
            while (Oreadr.Read())
            {
                ...
                if (Tools.Conv_Dbl(tPxPrice.Text) == 0 && Oreadr["PxPrice"].ToString() != "0")
                {
                    ...
                    AlsTOT.Text = MainMDI.A00(Convert.ToString(Math.Round(Tools.Conv_Dbl(Oreadr["PxPrice"].ToString()) / 
                        Tools.Conv_Dbl(Oreadr["AlsQty"].ToString()), MainMDI.NB_DEC_AFF)));
                ...
                {
                    if (Oreadr["Desc"].ToString().IndexOf("= ", 0) != -1) 
                    { 
        ...
        private bool fill_Qot(long Qid, string CpnyName)
        {
            ...
            string stSql = "SELECT PSM_Q_IGen.*, PSM_Company.Cpny_Name1, PSM_SALES_AGENTS_8.First_Name + ' ' + PSM_SALES_AGENTS_8.Last_Name AS employ, PSM_Terms.Descr, PSM_IncoTerm.IT_DESC, PSM_ShipMeth.ShipEng, PSM_Contacts.First_Name, PSM_Contacts.Last_Name, PSM_SALES_AGENTS.First_Name + ' ' + PSM_SALES_AGENTS.Last_Name AS SI_nm, PSM_SALES_AGENTS_2.First_Name + ' ' + PSM_SALES_AGENTS_2.Last_Name AS SO_nm, PSM_SALES_AGENTS_1.First_Name + ' ' + PSM_SALES_AGENTS_1.Last_Name AS SE_nm, PSM_SALES_AGENTS_3.First_Name + ' ' + PSM_SALES_AGENTS_3.Last_Name AS SP_nm, PSM_SALES_AGENTS_4.First_Name AS AD_nm, PSM_SALES_AGENTS_5.First_Name AS AI_nm, PSM_SALES_AGENTS_6.First_Name AS AE_nm, PSM_SALES_AGENTS_7.First_Name AS AP_nm, [PSM_SALES_AGENTS_9].[First_Name] + ' ' + [PSM_SALES_AGENTS_9].[Last_Name] AS IPM, PSM_Contacts_1.First_Name + ' ' + PSM_Contacts_1.Last_Name AS CPM" +
                ...
                " WHERE (((PSM_Company.Cpny_Name1)='" + CpnyName.Replace("'", "''") + "') AND ((PSM_Q_IGen.Quote_ID)=" + Qid + 
                ")) ORDER BY PSM_Q_IGen.Quote_ID, PSM_Q_IGen.CPNY_ID";
        ...
        private void fill_cbSal_AG(string SA)
        {
            ...
            string stSql = "select First_Name, Last_Name FROM PSM_SALES_AGENTS where SA='" + SA + "' " + stAND + 
                " AND status='1' order by First_Name"; //:"select First_Name, Last_Name FROM PSM_SALES_AGENTS where SA='" + SA + "'"; 
            SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
            OConn.Open();
            SqlCommand Ocmd = OConn.CreateCommand();
            Ocmd.CommandText = stSql;
        ...
        /*
        //this function fill the inside sales listview with the updated list coming directly from Syspro
        private void fillInsideSalesSyspro ()
        {
            string stSql = "select * from v_PGSalesperson where Salesperson like 'I%'";
            ...
            while (Oreadr.Read())
            {
                cbEmploy.Items.Add($"{Oreadr["Salesperson"].ToString()} {Oreadr["Name"].ToString()}");
            }
            OConn.Close();
        }
        */
        ...
        {
            string brnch = MainMDI.Find_One_Field_SYSPRO("SELECT [Branch]  FROM [SysproCompanyP].[dbo].[v_PGCustomerXRef]  where Customer='" + 
                Q_sysPcod.Text + "'");
            if (cbNo == 1) 
            { 
                cbAG1.Items.Clear(); 
                cbAG1.Items.Add(MainMDI.VIDE);
            }
            if (cbNo == 2) 
            { 
                cbAG2.Items.Clear(); 
                cbAG2.Items.Add(MainMDI.VIDE); 
            }

            string stSql = "SELECT [Name] FROM [v_PGSalesperson]   where [Branch]='" + brnch + 
                "' and substring([Salesperson],1,1)='A'  order by [Name]";
        ...
        private void QuoteXAdrs(char c_adrs, string adrs)
        {
            ...
            if (dAdrs.tStreet.Text != "")
            {
                ...
                {
                    case 'Q':
                        lQA.Text = dAdrs.tStreet.Text + "," + dAdrs.cbCity.Text + "," + dAdrs.cbSP.Text + "," + dAdrs.tZip.Text + "," + 
                            dAdrs.cbCountry.Text;
                        break;
                    case 'S':
                        lSA.Text = dAdrs.tStreet.Text + "," + dAdrs.cbCity.Text + "," + dAdrs.cbSP.Text + "," + dAdrs.tZip.Text + "," + 
                            dAdrs.cbCountry.Text;
                        break;
                    case 'I':
                        lIA.Text = dAdrs.tStreet.Text + "," + dAdrs.cbCity.Text + "," + dAdrs.cbSP.Text + "," + dAdrs.tZip.Text + "," + 
                            dAdrs.cbCountry.Text;
                        break;
                    case 'P':
                        lPA.Text = dAdrs.tStreet.Text + "," + dAdrs.cbCity.Text + "," + dAdrs.cbSP.Text + "," + dAdrs.tZip.Text + "," + 
                            dAdrs.cbCountry.Text;
                        break;
                }
            }
        ...
        private bool Entry_OK(ref int ALRM)
        {
            ...
            return (tQuoteID.Text != "" && lEmp_ID.Text != "" && lEmp_ID.Text != "0" && lContact_ID.Text != "" && lContact_ID.Text != "0" && 
                lLang.Text != "" && lcpnyID.Text != "" && lcpnyID.Text != "0");
        }
        ...
        private void Save_Q_ALL_Details()
        {
            ...
            //{
                ...
                if (lCurrIQID.Text != "0")
                {
                    long SID = Save_SOL(lCurrIQID.Text, lCurSoln.Text, lCurSolNDX.Text, 
                        tvSol.Nodes[Convert.ToInt32(lCurSolNDX.Text)].ImageIndex.ToString());
                    if (SID != 0)
                    {
                        ...
                        {
                            ref_PXAG_Price('V');
                            long ALSID = Save_ALS(SPCID, lCurALSn.Text, lCurALSNDX.Text, AlsTOT_orig.Text, tPxPrice.Text, tAGprice.Text, 
                                tALSnb.Text);
                            ...
                            if (ALSID != 0)
                            {
                                ...
                                for (int i = 0; i < lvQITEMS.Items.Count; i++)
                                {
                                    if (lvQITEMS.Items[i].SubItems[1].Text != "")
                                    {
                                        ...
                                        {
                                            MessageBox.Show("Error Occurs while Saving current Details......contact your Admin !!!" + 
                                                MainMDI.stXP);
                                            break;
                                        }
        ...
        private string SPEC_TOT_TOT1(string r_IQID, string Sname, string SpecName)
        {
            string stSql = "SELECT Sum(PSM_Q_ALS.Tot) AS SommeDeTot FROM (PSM_Q_SOL INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID) INNER JOIN PSM_Q_ALS ON PSM_Q_SPCS.SPC_LID = PSM_Q_ALS.SPC_LID " +
                " GROUP BY PSM_Q_SOL.I_Quoteid, PSM_Q_SOL.Sol_Name, PSM_Q_SPCS.SPC_Name HAVING (((PSM_Q_SOL.I_Quoteid)=" + r_IQID + 
                ") AND ((PSM_Q_SOL.Sol_Name)='" + Sname + "') AND ((PSM_Q_SPCS.SPC_Name)='" + SpecName.Replace("'", "''") + "'))";
        ...
        */
        ...
        private long Save_SOL(string iQid, string s_name, string Rnk, string img)
        {
            ...
            else
            {
                ...
                MainMDI.ExecSql(stSql);
                MainMDI.Write_JFS(stSql);
                stSql = MainMDI.Find_One_Field("select Sol_LID from PSM_Q_SOL where I_Quoteid=" + iQid + " AND Sol_Name='" + s_name + 
                    "' and Rnk=" + Rnk);
                if (stSql != MainMDI.VIDE) return Convert.ToInt32(stSql);
        ...
        private long Save_SPEC(long SID, string spc_name, string Rnk) //, out string msg)
        {
            string stSql = "";
            //string stSql = MainMDI.Find_One_Field("select SPC_LID from PSM_Q_SPCS where Sol_LID=" + SID.ToString() + " AND SPC_Name='" + spc_name + "' and Rnk=" + Rnk);   
            if (spc_name == MainMDI.VIDE) stSql = MainMDI.Find_One_Field("select SPC_LID from PSM_Q_SPCS where Sol_LID=" + SID.ToString() + 
                " AND SPC_Name='" + spc_name + "' and Rnk=" + Rnk);
            else stSql = MainMDI.Find_One_Field("select SPC_LID from PSM_Q_SPCS where Sol_LID=" + SID.ToString() + " AND SPC_Name='" + 
                spc_name.Replace("'", "''") + "'");
            if (stSql != MainMDI.VIDE) return Convert.ToInt32(stSql);
            else
            {
                ...
                MainMDI.ExecSql(stSql);
                MainMDI.Write_JFS(stSql);
                stSql = MainMDI.Find_One_Field("select SPC_LID from PSM_Q_SPCS where Sol_LID=" + SID.ToString() + " AND SPC_Name='" + 
                    spc_name.Replace("'", "''") + "' and Rnk=" + Rnk);
                if (stSql != MainMDI.VIDE) return Convert.ToInt32(stSql);
        ...
        private void ref_PXAG_PriceokOLD(char opera)
        {
            ...
            if (Tools.Conv_Dbl(AlsTOT.Text) != Tools.Conv_Dbl(AlsTOT_orig.Text) || 
                Tools.Conv_Dbl(tAGprice.Text) != Tools.Conv_Dbl(tPxPrice.Text))
            {
        ...
        private long Save_ALS(long SPCID, string als_Name, string Rnk, string Tot, string PXPrice, string AGPrice, string r_qty)
        {
            ...
            string stSql = MainMDI.Find_One_Field("select ALS_LID from PSM_Q_ALS where SPC_LID=" + SPCID.ToString() + " AND ALS_Name='" + 
                als_Name.Replace("'", "''") + "' ");
            if (stSql != MainMDI.VIDE)
            {
                string stt = "UPDATE PSM_Q_ALS SET  [Tot]=" + Tools.Conv_Dbl(Tot) + ", [PxPrice]=" + Tools.Conv_Dbl(PXPrice) + 
                    ", [AGPrice]=" + Tools.Conv_Dbl(AGPrice) + ", [AlsQty]=" + Tools.Conv_Dbl(r_qty) + ", [SV_Ovrg]=" + _ovrg.ToString() + 
                    " where ALS_LID=" + stSql;
                MainMDI.ExecSql(stt);
            ...
            else
            {
                ...
                stSql = MainMDI.Find_One_Field("select ALS_LID from PSM_Q_ALS where SPC_LID=" + SPCID.ToString() + " AND ALS_Name='" + 
                    als_Name.Replace("'", "''") + "' and Rnk=" + Rnk);
                if (stSql != MainMDI.VIDE) return Convert.ToInt32(stSql);
        ...
        */
        ...
        {
            Options frmOpt = new Options('A', "*", 'T');
            this.Hide();
            frmOpt.ShowDialog();
            this.Visible = true;
            ...
            if (frmOpt.lConsopt.Text == "Y")
            {
                ...
                add_LVO(1, 0, ItemCount.ToString(), stt + "  " + frmOpt.lFullDesc.Text + " [" + frmOpt.tPX_code.Text + "]", 
                    frmOpt.tOptqty.Text, tCust_Mult.Text, frmOpt.tUPrice.Text, 
                    Convert.ToString(Math.Round(Tools.Conv_Dbl(frmOpt.tUPrice.Text) * Tools.Conv_Dbl(tCust_Mult.Text), 
                    MainMDI.Q_NB_DEC_AFF)), frmOpt.tDlvDelay.Text, prtNB, "C_TCC||A", "A");
            ...
            else
            {
                if (frmOpt.lConsopt.Text == "L")
                {
                    ...
                    {
                        ItemCount++;
                        add_LVO(1, 0, ItemCount.ToString(), frmOpt.lvCadi.Items[i].SubItems[0].Text, frmOpt.lvCadi.Items[i].SubItems[1].Text, 
                            tCust_Mult.Text, frmOpt.lvCadi.Items[i].SubItems[2].Text, 
                            Convert.ToString(Math.Round(Tools.Conv_Dbl(frmOpt.lvCadi.Items[i].SubItems[2].Text) * 
                            Tools.Conv_Dbl(tCust_Mult.Text) * Tools.Conv_Dbl(frmOpt.tOptqty.Text), MainMDI.Q_NB_DEC_AFF)), 
                            frmOpt.lvCadi.Items[i].SubItems[4].Text, frmOpt.lvCadi.Items[i].SubItems[3].Text, "C_TCC||A", "A");
                        Opt_added = true;
                    }
                    Ref_ALSTOT('A');
                }
                ...
                {
                    ItemCount++;
                    add_LVO(1, 0, ItemCount.ToString(), frmOpt.batt_ref.Text, frmOpt.tOptqty.Text, tCust_Mult.Text, frmOpt.tUPrice.Text, 
                        Convert.ToString(Math.Round(Tools.Conv_Dbl(frmOpt.tUPrice.Text) * Tools.Conv_Dbl(tCust_Mult.Text), 
                        MainMDI.Q_NB_DEC_AFF)), frmOpt.tDlvDelay.Text, " ", "C_TCC||A", "B"); //"1", "1", "9999999", "999999", frmOpt.tDlvDelay.Text, frmOpt.lvCadi.Items[i].SubItems[3].Text, "C_TCC||A", "A");
        ...
        */
        ...
        private void Add_optionoldnew()
        {
            ...
            if (frmOpt.lConsopt.Text == "Y")
            {
                ...
                lvI.SubItems.Add(Convert.ToString(Math.Round(Tools.Conv_Dbl(frmOpt.tUPrice.Text) * Tools.Conv_Dbl(tCust_Mult.Text), 
                    MainMDI.Q_NB_DEC_AFF)));
        ...
        private void Add_BATT()
        {
            ...
            if (mydlg.Save)
            {
                ...
                add_LVO(1, 0, ItemCount.ToString(), mydlg.in_arrBatt[0, 0] + "  " + mydlg.in_arrBatt[0, 1], "1", "1", 
                    mydlg.in_arrBatt[MainMDI.batt_nbL - 1, 1], mydlg.in_arrBatt[MainMDI.batt_nbL - 1, 1], " ", " ", st, "C"); //name, price
                for (int j = 1; j < MainMDI.batt_nbL - 1; j++)
                    if (mydlg.in_arrBatt[j, 1] != " " && mydlg.in_arrBatt[j, 1] != "") 
                        add_LVO(1, 1, "", mydlg.in_arrBatt[j, 0] + "  " + mydlg.in_arrBatt[j, 1], "", "", "", "", "", "", "", "C");
                Tosave = true;
            }
        }
        ...
        private void Add_CBR(char cbr)
        {
            ...
            if (pbsI.SaveOK)
            {
                ...
                switch (cbr)
                {
                    ...
                    case 'c':
                        ItemCount++;
                        add_LVO(1, 0, ItemCount.ToString(), "Cabinet " + pbsI.tcModel.Text, pbsI.tcQtyCab.Text, tCust_Mult.Text, 
                            pbsI.tcPrice.Text, pbsI.tcextCab.Text, pbsI.tcLT.Text, pbsI.tcModel.Text, "C_TCC||A", "A");
                        ...
                        {
                            if (pbsI.tc1Tstep.Text != "0") add_LVO(1, 1, "", "First Tier: " + pbsI.tc1Tstep.Text + " step(s)", 
                                pbsI.tc1Tstep.Text, tCust_Mult.Text, pbsI.tcstpUP.Text, pbsI.tc1TPrice.Text, pbsI.tcLT.Text, "", "", "A");
                            if (pbsI.tc2Tstep.Text != "0") add_LVO(1, 1, "", "Second Tier: " + pbsI.tc2Tstep.Text + " step(s)", 
                                pbsI.tc2Tstep.Text, tCust_Mult.Text, pbsI.tcstpUP.Text, pbsI.tc2TPrice.Text, pbsI.tcLT.Text, "", "", "A");
                        }
                        else
                        {
                            if (pbsI.tc1Tstep.Text != "0") add_LVO(1, 1, "", "Tiers # : " + pbsI.tc1Tstep.Text, pbsI.tc1Tstep.Text, 
                                tCust_Mult.Text, pbsI.tcstpUP.Text, pbsI.tc1TPrice.Text, pbsI.tcLT.Text, "", "", "A");
                        }
                        if (pbsI.chkprint.Checked) add_LVO(1, 1, "", "Cell# :" + pbsI.tcNBCell.Text, "", "", "", "", "", "", "", "A");
                        if (pbsI.tcITExt.Text != "0") add_LVO(1, 1, "", "Inter Tiers ", pbsI.tcITQty.Text, tCust_Mult.Text, pbsI.tcITup.Text, 
                            pbsI.tcITExt.Text, "", "", "", "A");
                        if (pbsI.tcBTBExt.Text != "0") add_LVO(1, 1, "", "Battery Terminal Block ", pbsI.tcBTBQty.Text, tCust_Mult.Text, 
                            pbsI.tcBTBup.Text, pbsI.tcBTBExt.Text, "", "", "", "A");
                        break;
                    case 'B':
                    case 'b':
                        ItemCount++;
                        ...
                        add_LVO(1, 0, ItemCount.ToString(), pbsI.tbType.Text, pbsI.tsysnb.Text, tCust_Mult.Text, UP.ToString(), 
                            NExt.ToString(), pbsI.tbLT.Text, pbsI.tbName.Text, "C_TCC||C", "C");
                        ...
                        if (pbsI.tbICExt.Text != "0") add_LVO(1, 1, "", "Inter Cell ", pbsI.tbICQty.Text, tCust_Mult.Text, pbsI.tbICup.Text, 
                            pbsI.tbICExt.Text, "", "", "", "C");
                        if (pbsI.tbELExt.Text != "0") add_LVO(1, 1, "", "End Lugs", pbsI.tbELQty.Text, tCust_Mult.Text, pbsI.tbELup.Text, 
                            pbsI.tbELExt.Text, "", "", "", "C");
                        break;
                    case 'R':
                    case 'r':
                        ItemCount++;
                        add_LVO(1, 0, ItemCount.ToString(), pbsI.tbType.Text + " Rack:  " + pbsI.trModel.Text, pbsI.trQty.Text, 
                            tCust_Mult.Text, pbsI.trPrice.Text, pbsI.trExt.Text, pbsI.trLT.Text, pbsI.trModel.Text, "C_TCC||C", "C");
        ...
        private void add_LVO(int ToBePrinted, int deb, string nb, string Desc, string Qt, string mult, string up, string ext, string LT, 
            string stPartnb, string TecVal, string Grp)
        {
            ...
            //if (ext != "" && ext != "0") 
            if (ext != "" && ext != "0") lvI.SubItems.Add(Grp); 
            else lvI.SubItems.Add(""); //Xchnge
            ...
            else if (LT != "") lvI.SubItems.Add(LT);
        ...
        private void add_LVO_NL(byte deb, string nb, string Desc, string Qt, string mult, string up, string ext, string LT, string stPartnb, 
            string Grp, string tva)
        {
            ListViewItem lvI = lvQITEMS.Items.Add("");
            if (deb == 0 || deb == 2 || deb == 3)
            {
                ...
                if (deb == 2) lvI.BackColor = Color.LightYellow;
                if (tva.Length > 6) 
                    if (tva.Substring(0, 5) == "BNS||") lvI.BackColor = BNS_color;
                lvI.SubItems.Add(nb);
            }
        ...
        private void Add_Rectif()
        {
            ...
            if (Rectifdlg.lsave.Text == "Y")
            {
                ...
                //string st = (frmchdlg.lvDefOption.Items[i].SubItems[1].Text != "") ? frmchdlg.lvDefOption.Items[i].SubItems[1].Text + " / " : "";  
                add_LVO(1, 0, ItemCount.ToString(), "EDI RECTIFIER " + Rectifdlg.lRecModel.Text, Rectifdlg.tIQty.Text, Rectifdlg.tSMRK.Text, 
                    Rectifdlg.tIPU.Text, Rectifdlg.tIExt.Text, Rectifdlg.tILT.Text, "", "", "A");
                if (Rectifdlg.chkEnc.Checked) add_LVO(1, 1, "", Rectifdlg.chkEnc.Text + ": " + Rectifdlg.cbEnc.Text, "", "", "", "", "", "", 
                    "", "A");
                if (Rectifdlg.chkInput.Checked) add_LVO(1, 1, "", Rectifdlg.chkInput.Text + ": " + Rectifdlg.cbInput.Text, "", "", "", "", 
                    "", "", "", "A");
                if (Rectifdlg.chkheat.Checked) add_LVO(1, 1, "", Rectifdlg.chkheat.Text + ": " + Rectifdlg.cbHeat.Text, "", "", "", "", "", 
                    "", "", "A");
                if (Rectifdlg.chkplc.Checked) add_LVO(1, 1, "", Rectifdlg.chkplc.Text + ": " + Rectifdlg.cbPLC.Text, "", "", "", "", "", "", 
                    "", "A");
                if (Rectifdlg.chkinternal.Checked) add_LVO(1, 1, "", Rectifdlg.chkinternal.Text + ": " + Rectifdlg.cbInternal.Text, "", "", 
                    "", "", "", "", "", "A");
                if (Rectifdlg.chk3PHS.Checked) add_LVO(1, 1, "", Rectifdlg.chk3PHS.Text + ": " + Rectifdlg.cb3PHS.Text, "", "", "", "", "", 
                    "", "", "A");
                if (Rectifdlg.chktermalP.Checked) add_LVO(1, 1, "", Rectifdlg.chktermalP.Text + ((Rectifdlg.ttermalP.Text == "STD") ? "" : 
                    ": " + Rectifdlg.ttermalP.Text), "", "", "", "", "", "", "", "A");
                if (Rectifdlg.chkApp.Checked) add_LVO(1, 1, "", Rectifdlg.chkApp.Text + ": " + Rectifdlg.tApp.Text, "", "", "", "", "", "", 
                    "", "A");
                Ref_ALSTOT('A');
            }
        }
        ...
        private void Add_SWMD_P600()
        {
            ...
            if (Rectifdlg.lsave.Text == "Y")
            {
                ...
                //add_LVO(0, ItemCount.ToString(), frmchdlg.lvDefOption.Items[i].SubItems[2].Text, frmchdlg.lvDefOption.Items[i].SubItems[3].Text, tCust_Mult.Text, frmchdlg.lvDefOption.Items[i].SubItems[4].Text, frmchdlg.lvDefOption.Items[i].SubItems[5].Text, frmchdlg.lvDefOption.Items[i].SubItems[6].Text);
                //string st = (frmchdlg.lvDefOption.Items[i].SubItems[1].Text != "") ? frmchdlg.lvDefOption.Items[i].SubItems[1].Text + " / " : "";  
                add_LVO(1, 0, ItemCount.ToString(), MainMDI.arr_EFSdict[49, MainMDI.Lang] + " " + Rectifdlg.lmodel.Text, Rectifdlg.tIQty.Text, 
                    Rectifdlg.tSMRK.Text, Rectifdlg.tIPU.Text, Rectifdlg.tIExt.Text, Rectifdlg.tILT.Text, "", " C_MODEL||" + 
                    Rectifdlg.lmodel.Text, "A");

                string Line1 = "Input Current: "  + Rectifdlg.txInputC.Text + ", " + "Enclosure: " + Rectifdlg.txEnc.Text + ", " + 
                    "Modules#: " + Rectifdlg.txModnb.Text + ", " + "Subrack: " + Rectifdlg.txSubRK.Text;
        ...
        private void Add_SWMD_P600_EZ()
        {
            ...
            P600_New P600Dialog = new P600_New();
            this.Hide();
            P600Dialog.ShowDialog();
            this.Visible = true;
        ...
        private void Add_ACDCbreaker(string ACDC)
        {
            Options_brkr_Cfgv2 Rectifdlg = new Options_brkr_Cfgv2(ACDC);
            this.Hide();
            Rectifdlg.ShowDialog();
            this.Visible = true;
            if (Rectifdlg.lsave.Text == "Y")
            {
                string stbrkr = (ACDC == "A") ? MainMDI.arr_EFSdict[74, MainMDI.Lang] : MainMDI.arr_EFSdict[73, MainMDI.Lang];
                //string Line1 = stbrkr + Rectifdlg.txf1.Text + "," + Rectifdlg.txf2.Text + "," + Rectifdlg.txf3.Text; 
                string Line1 = Rectifdlg.txddd.Text;
                ItemCount++;
                ...
                add_LVO(1, 0, ItemCount.ToString(), Line1, Rectifdlg.tIQty.Text, Rectifdlg.tSMRK.Text, Rectifdlg.tlPU.Text, 
                    Rectifdlg.tIExt.Text, Rectifdlg.tILT.Text, "", "", "A");
        ...
        */
            ...
            if (frmchdlgP5500.lSave.Text == "Y")
            {
                for (int i = 0; i < frmchdlgP5500.lvDefOption.Items.Count; i++)
                {
                    if (i == 0)
                    {
                        ...
                        for (int y = 0; y < Charger.NB_FRML; y++)
                        {
                            if (frmchdlgP5500.dlg_arr_CAL_FRML[y] != "") lFrml += " " + frmchdlgP5500.dlg_arr_CAL_FRML[y];
                            else break;
                        }
                        lFrml += " C_MODEL||" + model + " C_TCC||A";
                        //here add TV value to TEC_Val
                        lFrml += " " + frmchdlgP5500.lOth_TV;
                        add_LVO(1, 0, ItemCount.ToString(), frmchdlgP5500.lvDefOption.Items[i].SubItems[1].Text, 
                            frmchdlgP5500.lvDefOption.Items[i].SubItems[3].Text, tCust_Mult.Text, 
                            frmchdlgP5500.lvDefOption.Items[i].SubItems[4].Text, frmchdlgP5500.lvDefOption.Items[i].SubItems[5].Text, 
                            frmchdlgP5500.lvDefOption.Items[i].SubItems[6].Text, frmchdlgP5500.lvDefOption.Items[i].SubItems[11].Text, lFrml, 
                            "A");
                        //arr_Tech_values[lvQITEMS.Items.Count - 1] = lFrml;   
                    }
                    else
                    {
                        if (frmchdlgP5500.lvDefOption.Items[i].Checked)
                        {
                            ...
                            //}
                            //added on 07/12/05
                            string st = frmchdlgP5500.lvDefOption.Items[i].SubItems[1].Text == "" ? 
                                frmchdlgP5500.lvDefOption.Items[i].SubItems[2].Text : frmchdlgP5500.lvDefOption.Items[i].SubItems[1].Text + 
                                "= " + frmchdlgP5500.lvDefOption.Items[i].SubItems[2].Text;
                            if (frmchdlgP5500.lvDefOption.Items[i].SubItems[4].Text != "0")
                                add_LVO(1, 1, "", st, frmchdlgP5500.lvDefOption.Items[i].SubItems[3].Text, tCust_Mult.Text, 
                                    frmchdlgP5500.lvDefOption.Items[i].SubItems[4].Text, frmchdlgP5500.lvDefOption.Items[i].SubItems[5].Text, 
                                    frmchdlgP5500.lvDefOption.Items[i].SubItems[6].Text, frmchdlgP5500.lvDefOption.Items[i].SubItems[10].Text, 
                                    r_TecV, "A");
                            else add_LVO(1, 1, "", st, "", "", "", "", "", frmchdlgP5500.lvDefOption.Items[i].SubItems[10].Text, r_TecV, "A");
                            if (frmchdlgP5500.lvDefOption.Items[i].ForeColor == Color.Red) 
                                lvQITEMS.Items[lvQITEMS.Items.Count - 1].ForeColor = Color.Red;
                        }
                    }
                }
                Ref_ALSTOT('A');
        ...
        {
            string _desc = (MainMDI.Lang == 0) ? @"PC23 c/w touch screen, P4600 overlay and cabinet door cutout" : 
                "PC23 incluant écran tactil, membrane et ouverture dans la porte";
        ...
        private void Add_Charger(char tst)
        {
            ...
            //old
            //Chargerdlg frmchdlg = new Chargerdlg('0', MainMDI._connectionString,tst);
            //this.Hide();
            //frmchdlg.ShowDialog();
            //this.Visible = true;

            Chargerdlg_Cfg_v2 frmchdlg = new Chargerdlg_Cfg_v2('0', lQuoteID.Text, MainMDI.M_stCon);
            this.Hide();
            frmchdlg.ShowDialog();
            this.Visible = true;
            ...
            if (frmchdlg.lSave.Text == "Y")
            {
                for (int i = 0; i < frmchdlg.lvDefOption.Items.Count + 1; i++)
                {
                    if (i == 0)
                    {
                        ItemCount++;
                        ...
                        for (int y = 0; y < Charger.NB_FRML; y++)
                        {
                            if (frmchdlg.dlg_arr_CAL_FRML[y] != "") lFrml += " " + frmchdlg.dlg_arr_CAL_FRML[y];
                            else break;
                        }
                        B_model = model;
                        lFrml += " C_MODEL||" + model + " C_TCC||A";
                        ...
                        add_LVO(1, 0, ItemCount.ToString(), frmchdlg.lvDefOption.Items[i].SubItems[1].Text, 
                            frmchdlg.lvDefOption.Items[i].SubItems[3].Text, tCust_Mult.Text, frmchdlg.lvDefOption.Items[i].SubItems[4].Text, 
                            frmchdlg.lvDefOption.Items[i].SubItems[5].Text, frmchdlg.lvDefOption.Items[i].SubItems[6].Text, "~~", 
                            frmchdlg.lvDefOption.Items[i].SubItems[11].Text, "A");
                        //add_LVO(1, 0, ItemCount.ToString(), frmchdlg.lvDefOption.Items[i].SubItems[1].Text, frmchdlg.lvDefOption.Items[i].SubItems[3].Text, tCust_Mult.Text, frmchdlg.lvDefOption.Items[i].SubItems[4].Text, frmchdlg.lvDefOption.Items[i].SubItems[5].Text, frmchdlg.lvDefOption.Items[i].SubItems[6].Text, frmchdlg.lvDefOption.Items[i].SubItems[11].Text, lFrml, "A");
                        //arr_Tech_values[lvQITEMS.Items.Count - 1] = lFrml; 
                        //30052014 ede
                    ...
                    else
                    {
                        ...
                        {
                            //inside sales 
                            //string insideSales = this.sysProCode(cbEmploy.Text);
                            //string projectManager = this.sysProCode(cbIPmgr.Text);
                            //string st = "Inside Sales: " + insideSales + ", Primax Project Manager: " + projectManager;
                            //add_LVO(1, 1, "", st, "", "", "", "", "", "", "", "A");
                        }
                        else
                        {
                            if (frmchdlg.lvDefOption.Items[i - 1].Checked)
                            {
                                string r_TecV = frmchdlg.lvDefOption.Items[i - 1].SubItems[11].Text;
                                //added on 07/12/05
                                //string r_TecV = "";
                                //if (frmchdlg.lvDefOption.Items[i].SubItems[8].Text != "") 
                                //{
                                ...
                                //}
                                //added on 07/12/05
                                string st = frmchdlg.lvDefOption.Items[i - 1].SubItems[1].Text == "" ? 
                                    frmchdlg.lvDefOption.Items[i - 1].SubItems[2].Text : frmchdlg.lvDefOption.Items[i - 1].SubItems[1].Text 
                                    + "= " + frmchdlg.lvDefOption.Items[i - 1].SubItems[2].Text;
                                if (frmchdlg.lvDefOption.Items[i - 1].SubItems[4].Text != "0")
                                    add_LVO(1, 1, "", st, frmchdlg.lvDefOption.Items[i - 1].SubItems[3].Text, 
                                        frmchdlg.lvDefOption.Items[i - 1].SubItems[11].Text, 
                                        frmchdlg.lvDefOption.Items[i - 1].SubItems[4].Text, 
                                        frmchdlg.lvDefOption.Items[i - 1].SubItems[5].Text, 
                                        frmchdlg.lvDefOption.Items[i - 1].SubItems[6].Text, 
                                        frmchdlg.lvDefOption.Items[i - 1].SubItems[10].Text, r_TecV, "A");
                                else add_LVO(1, 1, "", st, "", "", "", "", "", frmchdlg.lvDefOption.Items[i - 1].SubItems[10].Text, r_TecV, 
                                    "A");
                                if (frmchdlg.lvDefOption.Items[i - 1].ForeColor == Color.Red) 
                                    lvQITEMS.Items[lvQITEMS.Items.Count - 1].ForeColor = Color.Red;
                            }
                        }
                    }
                }
                //adding diff bewn p4500 and P4600 cuase same in new list price
                //if (tst != 'T')
                //{
        ...
        */
        ...
        private void Add_NLItemOption_NEW()
        {
            ...
            string keyinfo =  MainMDI.A00(tQuoteID.Text, 8) + " / " + cbCompanyy.Text + " / " + lCurSoln.Text + "/" + lCurSPCn.Text + "/" + 
                lCurALSn.Text;
            ...
            if (frmNLIO.SaveOK)
            {
                ...
                add_LVO_NL(0, ItemCount.ToString(), st, frmNLIO.cal_qty.Text, frmNLIO.cal_multipl.Text, frmNLIO.cal_pu.Text, 
                    frmNLIO.cal_ext.Text, frmNLIO.tILT.Text, frmNLIO.tIModel.Text, "C", frmNLIO.lsavALLinfo.Text);
        ...
        private void Add_Service()
        {
            ...
            if (frmNLIO.SaveOK)
            {
                if (frmNLIO.chk_th.Checked) add_LVO_NL(0, ItemCount.ToString(), frmNLIO.txitem_th.Text, "1", "1", frmNLIO.txC39_th.Text, 
                    frmNLIO.txC39_th.Text, "", "", "A", "");
                if (frmNLIO.chk_ts.Checked) add_LVO_NL(0, ItemCount.ToString(), frmNLIO.txitem_ts.Text, "1", "1", frmNLIO.txC39_ts.Text, 
                    frmNLIO.txC39_ts.Text, "", "", "A", "");
                Ref_ALSTOT('A');
            }
        }
        ...
        private void Add_NLItemOption_OLD()
        {
            ...
            if (frmNLIO.SaveOK)
            {
                ...
                string st = (frmNLIO.tIModel.Text == "") ? frmNLIO.tIName.Text : frmNLIO.tIName.Text + " / " + frmNLIO.tIModel.Text;
                //add_LVO(0, ItemCount.ToString(), st, frmNLIO.tIQty.Text, tCust_Mult.Text, frmNLIO.tIPU.Text, frmNLIO.tIExt.Text, frmNLIO.tILT.Text);
                add_LVO_NL(0, ItemCount.ToString(), st, frmNLIO.tIQty.Text, frmNLIO.tSMRK.Text, frmNLIO.tIPU.Text, frmNLIO.tIExt.Text, 
                    frmNLIO.tILT.Text, frmNLIO.tIModel.Text, "C","  ");
                if (frmNLIO.tIdim.Text != "") 
                    add_LVO(1, 1, "", "Dimensions= " + frmNLIO.tIdim.Text, "", "", "", "", "", "", "", "C");
                if (frmNLIO.tIf1.Text != "") 
                    add_LVO(1, 1, "", frmNLIO.lif1.Text + "=  " + frmNLIO.tIf1.Text, "", "", "", "", "", "", "", "C");
                if (frmNLIO.tIf2.Text != "") 
                    add_LVO(1, 1, "", frmNLIO.lif2.Text + "=  " + frmNLIO.tIf2.Text, "", "", "", "", "", "", "", "C");
                if (frmNLIO.tIotherF.Text != "")
                {
                    st = frmNLIO.tIotherF.Text;
                    //if (frmNLIO.lIotherF.Text != "") add_LVO(1, "", frmNLIO.lIotherF.Text, "", "", "", "", ""); 
        ...
        private void Duplica_Sol()
        {
            ...
            string stSql = "SELECT PSM_Q_SOL.*, PSM_Q_SPCS.*, PSM_Q_ALS.*, PSM_Q_Details.*, PSM_Q_SPCS.Rnk AS SPCS_Rnk, PSM_Q_ALS.Rnk AS ALS_Rnk, PSM_Q_Details.Rnk AS Details_Rnk " +
                " FROM ((PSM_Q_IGen INNER JOIN PSM_Q_SOL ON PSM_Q_IGen.i_Quoteid = PSM_Q_SOL.I_Quoteid) INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID) INNER JOIN (PSM_Q_ALS INNER JOIN PSM_Q_Details ON PSM_Q_ALS.ALS_LID = PSM_Q_Details.ALS_LID) ON PSM_Q_SPCS.SPC_LID = PSM_Q_ALS.SPC_LID " +
                " WHERE (PSM_Q_IGen.i_Quoteid=" + lCurrIQID.Text + " and PSM_Q_SOL.Sol_Name='" + lCurSoln.Text + 
            ...
            while (Oreadr.Read())
            {
                ...
                if (Oals != Nals || Ospc != Nspc) //|| alsAdded) 
                {
                    ...
                    r_alsId = Save_ALS(r_Spcid, Nals, Oreadr["ALS_Rnk"].ToString(), Oreadr["Tot"].ToString(), Oreadr["PxPrice"].ToString(), 
                        Oreadr["AGPrice"].ToString(), Oreadr["AlsQty"].ToString());
                    //if (!alsAdded) 
        ...
        */
        ...
        private void Duplica_SPC()
        {
            ...
            string stSql = "SELECT PSM_Q_SOL.*, PSM_Q_SPCS.*, PSM_Q_ALS.*, PSM_Q_Details.* ,PSM_Q_ALS.Rnk as A_Rnk, PSM_Q_SOL.Sol_LID AS SOL_ID, PSM_Q_ALS.Rnk AS ALS_Rnk, PSM_Q_Details.Rnk AS Details_Rnk " +
                " FROM ((PSM_Q_IGen INNER JOIN PSM_Q_SOL ON PSM_Q_IGen.i_Quoteid = PSM_Q_SOL.I_Quoteid) INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID) INNER JOIN (PSM_Q_ALS INNER JOIN PSM_Q_Details ON PSM_Q_ALS.ALS_LID = PSM_Q_Details.ALS_LID) ON PSM_Q_SPCS.SPC_LID = PSM_Q_ALS.SPC_LID " +
                " WHERE (PSM_Q_IGen.i_Quoteid=" + lCurrIQID.Text + " and PSM_Q_SOL.Sol_Name='" + lCurSoln.Text + 
                "' and PSM_Q_SPCS.SPC_Name='" + lCurSPCn.Text.Replace("'", "''") + 
                "') ORDER BY PSM_Q_SOL.Rnk, PSM_Q_SPCS.Rnk, PSM_Q_ALS.Rnk, PSM_Q_Details.Rnk";
        ...
        private void Duplica_ALS()
        {
            ...
            while (Oreadr.Read())
            {
                ...
                {
                    //r_alsId = Save_ALS(r_Spcid, Nals, nbAls.ToString(), Oreadr["tot"].ToString());
                    r_alsId = Save_ALS(r_Spcid, Nals, Oreadr["PSM_Q_ALS.Rnk"].ToString(), Oreadr["Tot"].ToString(), 
                        Oreadr["PxPrice"].ToString(), Oreadr["AGPrice"].ToString(), Oreadr["AlsQty"].ToString());
                    if (!alsAdded)
                    {
                        addNode_Als(Nals, nbSol, nbSpc);
                        nbAls = (nbSpc == 0) ? 0 : tvSol.Nodes[nbSol].Nodes[nbSpc - 1].Nodes.Count;
                ...
                switch (tvSol.SelectedNode.ImageIndex)
                {
                    case 1: //Spec
                        ...
                        string st = MainMDI.Find_One_Field("SELECT PSM_Q_SPCS.SPC_LID FROM PSM_Q_SOL INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID " +
                            ...
                            "' and PSM_Q_SOL.I_Quoteid=" + lCurrIQID.Text);
                        if (st != MainMDI.VIDE) MainMDI.ExecSql("UPDATE PSM_Q_SPCS SET [SPC_Name]='" + NewLBL.Replace("'", "''") + 
                            "' where SPC_LID=" + st);
                        break;
                    case 0: //Alias
                    case 3:
                        ...
                        string stSql = "SELECT PSM_Q_ALS.ALS_LID FROM (PSM_Q_SOL INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID) INNER JOIN PSM_Q_ALS ON PSM_Q_SPCS.SPC_LID = PSM_Q_ALS.SPC_LID " +
                            " WHERE PSM_Q_SOL.Sol_Name='" + lCurSoln.Text + "' AND PSM_Q_SPCS.SPC_Name='" + lCurSPCn.Text.Replace("'", "''") + 
                            "' AND PSM_Q_ALS.ALS_Name='" + OldLbl.Replace("'", "''") + "' and PSM_Q_SOL.I_Quoteid=" + lCurrIQID.Text;
            ...
            if (lCurrIQID.Text != "0")
            {
                switch (tvSol.SelectedNode.ImageIndex)
                {
                    case 1: //Spec
                        lCurSPCn.Text = NewLBL;
                        string st = MainMDI.Find_One_Field("SELECT PSM_Q_SPCS.SPC_LID FROM PSM_Q_SOL INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID " +
                            " WHERE (((PSM_Q_SOL.Sol_Name)='" + lCurSoln.Text + "') AND ((PSM_Q_SPCS.SPC_Name)='" + 
                            OldLbl.Replace("'", "''") + "'))");
                        if (st != MainMDI.VIDE) MainMDI.ExecSql("UPDATE PSM_Q_SPCS SET [SPC_Name]='" + NewLBL.Replace("'", "''") + 
                            "' where SPC_LID=" + st);
                        break;
                    ...
                    case 3:
                        ...
                        string stSql = "SELECT PSM_Q_ALS.ALS_LID FROM (PSM_Q_SOL INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID) INNER JOIN PSM_Q_ALS ON PSM_Q_SPCS.SPC_LID = PSM_Q_ALS.SPC_LID " +
                            " WHERE (((PSM_Q_SOL.Sol_Name)='" + lCurSoln.Text + "') AND ((PSM_Q_SPCS.SPC_Name)='" + 
                            lCurSPCn.Text.Replace("'", "''") + "') AND ((PSM_Q_ALS.ALS_Name)='" + OldLbl.Replace("'", "''") + "'))";
                        stSql = MainMDI.Find_One_Field(stSql);
                        if (stSql != MainMDI.VIDE)
                        {
                            string myals = (NewLBL.Length > 0) ? NewLBL.Replace("'", "''") : "Toto";
                    ...
                    case 4:
                        lCurSoln.Text = NewLBL;
                        MainMDI.ExecSql("UPDATE PSM_Q_SOL SET [Sol_Name]='" + NewLBL + "' where I_Quoteid=" + lCurrIQID.Text + 
                            " and Sol_Name='" + tvSol.SelectedNode.Text + "'");
                        break;
                }
                OldLabel = "";
            }
        ...
        private void lvQITEMS_DoubleClick(object sender, System.EventArgs e)
        {
            ...
            if (lcurSol_Status.Text != "C" || MainMDI.User == "ede")
            {
                ...
                if (in_opera != 'C')
                {
                    ...
                    lALSmAmnt.Text = Convert.ToString(Math.Round(Tools.Conv_Dbl(AlsTOT_orig.Text) - Tools.Conv_Dbl(tExt.Text), 
                        MainMDI.Q_NB_DEC_AFF));
        ...
        private void btnOKchng_ClickOld(object sender, System.EventArgs e)
        {
            if (maj_LT())
            {
                if ((tExt.Text != "0" && tExt.Text != "") || lvQITEMS.SelectedItems[0].SubItems[7].Text == "")
                {
                    ...
                    if (tExt.Text != "") lvQITEMS.SelectedItems[0].SubItems[8].Text = LT.Text;
        ...
        */
        ...
        private void cal_SellExt()
        {
            ...
            if (tUprice.Text != "" && tqty.Text != "" && tmult.Text != "") 
                tExt.Text = Convert.ToString(Math.Round(Tools.Conv_Dbl(tmult.Text) * Tools.Conv_Dbl(tUprice.Text) * 
                    Tools.Conv_Dbl(tqty.Text) * Tools.Conv_Dbl(tXchng.Text), MainMDI.Q_NB_DEC_AFF));
        }
        ...
        {
            if (mt != "" && AlsTOT.Text != "") AlsTOT.Text = Convert.ToString(Math.Round(Tools.Conv_Dbl(mt) + Tools.Conv_Dbl(AlsTOT.Text), 
                MainMDI.Q_NB_DEC_AFF));
        }

        private void M_AlsTot(string mt)
        {
            if (mt != "" && AlsTOT.Text != "") AlsTOT.Text = Convert.ToString(Math.Round(Tools.Conv_Dbl(AlsTOT.Text) - Tools.Conv_Dbl(mt), 
                MainMDI.Q_NB_DEC_AFF));
        }
        ...
        private void apply_OGA()
        {
            if (lvQITEMS.Items.Count > 0)
            {
                ...
                {
                    if (lvQITEMS.Items[i].SubItems[7].Text != "") dtot = dtot + Tools.Conv_Dbl(lvQITEMS.Items[i].SubItems[7].Text);
                    if (lvQITEMS.Items[i].SubItems[1].Text == " " || (lvQITEMS.Items[i].SubItems[1].Text == "." && 
                        lvQITEMS.Items[i].BackColor == Color.WhiteSmoke)) nb++; //item # is always == " " not ""
                    else
                    {
                        if (i > 0) //&& lvQITEMS.Items[i].BackColor ==) 
                        {
                            ...
                            nb = 0;
                            if (Tools.Conv_Dbl(lvQITEMS.Items[lin].SubItems[8].Text) > Tools.Conv_Dbl(lHiDelv.Text)) 
                                lHiDelv.Text = lvQITEMS.Items[lin].SubItems[8].Text;
                        }
                    }
                }
                lvQITEMS.Items[lin].SubItems[9].Text = nb.ToString();
        ...
        */
        ...
        private void del_Als_IOold(int ndx)
        {
            ...
            for (int j = ndx + nbDef; j >= ndx; j--)
            {
                string st = MainMDI.Find_One_Field("SELECT  PSM_Q_Details.Detail_LID " +
                    " FROM PSM_Q_IGen INNER JOIN ((PSM_Q_SOL INNER JOIN PSM_Q_SPCS ON PSM_Q_SOL.Sol_LID = PSM_Q_SPCS.Sol_LID) INNER JOIN (PSM_Q_ALS INNER JOIN PSM_Q_Details ON PSM_Q_ALS.ALS_LID = PSM_Q_Details.ALS_LID) ON PSM_Q_SPCS.SPC_LID = PSM_Q_ALS.SPC_LID) ON PSM_Q_IGen.i_Quoteid = PSM_Q_SOL.I_Quoteid " +
                    " WHERE PSM_Q_ALS.ALS_Name='" + lCurALSn.Text.Replace("'", "''") + "' AND PSM_Q_Details.[Desc]='" + 
                    lvQITEMS.Items[j].SubItems[2].Text + "' AND PSM_Q_Details.Rnk =" + (j + ndell));
                if (st != MainMDI.VIDE)
                {
        ...
        */
        ...
        //close word button after wordfile was completed
                ...
                switch (e.Node.ImageIndex)
                {
                    ...
                    case 3:
                        ...
                        if (e.Node.Checked && e.Node.Parent.Parent.Text == curR_sol)
                        {
                            ...
                            //string TotALS = ????
                            add_LVR("    " + e.Node.Text, e.Node.Parent.Parent.Index.ToString(), e.Node.Parent.Index.ToString(), 
        ...
        private void delLvOrder(int Rndx)
        {
            ...
            if (lvOrder.SelectedItems.Count > 0)
            {
                ...
                {
                    int AI = (lvOrder.SelectedItems[Rndx].SubItems[3].Text != "") ? 
                        Convert.ToInt32(lvOrder.SelectedItems[Rndx].SubItems[3].Text) : -1;
                    int PI = (lvOrder.SelectedItems[Rndx].SubItems[2].Text != "") ? 
                        Convert.ToInt32(lvOrder.SelectedItems[Rndx].SubItems[2].Text) : -1;
                    int SI = (lvOrder.SelectedItems[Rndx].SubItems[1].Text != "") ? 
                        Convert.ToInt32(lvOrder.SelectedItems[Rndx].SubItems[1].Text) : -1;
        ...
        {
            this.Cursor = Cursors.WaitCursor;
            string stSql = MainMDI.Find_One_Field("select Sol_LID from PSM_Q_SOL where I_Quoteid=" + lCurrIQID.Text + " AND Sol_Name='" + 
                lRSoln.Text + "'");
            isDellAll = true;
            if (stSql != MainMDI.VIDE)
            {
                ...
                {
                    if (Tools.Conv_Dbl(lvOrder.Items[r].SubItems[4].Text) != 0) Nsrt_Det_OL(lvOrder.Items[r].SubItems[7].Text, 
                        lvOrder.Items[r].SubItems[6].Text, lvOrder.Items[r].SubItems[4].Text, lvOrder.Items[r].SubItems[5].Text);
        ...
        private void save_DetLID(string iQID, string solN, string SpcN, string AlsN, int r)
        {
            ...
            if (AI != -1) stSql += " AND ((PSM_Q_SPCS.SPC_Name)='" + tvSol.Nodes[SI].Nodes[PI].Text.Replace("'", "''") + 
                    "') AND ((PSM_Q_ALS.ALS_Name)='" + tvSol.Nodes[SI].Nodes[PI].Nodes[AI].Text.Replace("'", "''") + "') ";
            ...
            SqlDataReader Oreadr = Ocmd.ExecuteReader();
            while (Oreadr.Read())
                if (!(Tools.Conv_Dbl(Oreadr["Ext"].ToString()) == 0 && Oreadr["Aff_ID"].ToString() == ".")) 
                    Nsrt_Det_OL(Oreadr["Ext"].ToString(), Oreadr["AA_orig"].ToString(), Oreadr["Detail_LID"].ToString(), r.ToString());
            OConn.Close();
        }
        ...
        {
            string AA = (ext == "") ? "" : r_AA;
            MainMDI.ExecSql("INSERT INTO " + MainMDI.t_Det_OL + " ([detailLID],[AA_orig],[rank]) VALUES (" + r_det_LID + ", '" + AA + "', " + 
                r + ")");
        }
        ...
        public static long oldGen_ID(char tNm)
        {
            ...
            string Res = MainMDI.Find_One_Field("select " + tNm + "ID from " + tblNm + " where not flaged and not InUse order by  " + tNm + 
                "ID ");
            if (Res == MainMDI.VIDE)
            {
        ...
        public static bool lock_table(char tNm)
        {
            ...
            while (true)
            {
                ...
                {
                    DialogResult dr = MessageBox.Show("Can not Generate New ID  Table is Locked by another User, please try later or contact your Admin...", 
                        "Generating New ID", MessageBoxButtons.RetryCancel, MessageBoxIcon.Question);
        ...
        public static bool New100_QRID(char c, string IdFrom)
        {
            ...
            try
            {
                ...
                if (LastID < debId) 
                    for (long i = LastID; i < debId - 1; i++) 
                        MainMDI.ExecSql("INSERT INTO " + tblNm + " ([flaged],[inuse]) VALUES (TRUE,FALSE)");
        ...
        */
        ...
        private void btn_FND_Code_Click(object sender, EventArgs e)
        {
            ...
            if (CpnyNm == MainMDI.VIDE) MessageBox.Show("NOT FOUND..........!!!!");
            else cbCompanyy.Text = CpnyNm;
        }
        ...
        private void CHSPrt()
        {
            ...
            foreach (String strPrinter in PrinterSettings.InstalledPrinters)
            {
                ...
                if (strPrinter == strDefaultPrinter) cbprinters.SelectedIndex = cbprinters.Items.IndexOf(strPrinter);
            }
        }
        ...
        private void CutCopy(char c)
        {
            ...
            LstNdx = i;
            if (c == 'D') 
                while (lvQITEMS.SelectedItems.Count > 0) lvQITEMS.Items[lvQITEMS.SelectedItems[0].Index].Remove();
            //aff();
        ...
        private void CutCopyOKOLD(char c)
        {
            ...
            for (i = 0; i < lvQITEMS.SelectedItems.Count; i++)
            {
                ...
                for (int j = 0; j < lvQITEMS.Items[i].SubItems.Count; j++) MainMDI.arr_clpB[i, j] = lvQITEMS.SelectedItems[i].SubItems[j].Text;
        ...
        private void paste(int InsertNdx)
        {
            ...
            {
                K++;
                for (int j = 0; j < lvQITEMS.Items[i].SubItems.Count; j++) MainMDI.arr_clpB[K, j] = lvQITEMS.Items[i].SubItems[j].Text;
                //LstNdx++;
            }
            //aff();
            ...
            for (int i = 0; i < MainMDI.MAX_Quote_lines; i++)
            {
                ...
                else
                {
                    ...
                    while (k < 13) lv.SubItems.Add(MainMDI.arr_clpB[i, k++]);
                }
            }
            //vider_arr_clpB(); MainMDI.arr_clpB[i, j] = "~";
        ...
        {
            for (int i = 0; i < MainMDI.MAX_Quote_lines; i++)
                for (int j = 0; j < 12; j++) MainMDI.arr_clpB[i, j] = "~";
            LstNdx = 0;
        }
        ...
        private void printQSum()
        {
            ...
            if (lvQITEMS.Items.Count > 0)
            {
                ...
                //string prtNmeOLD = printDialog1.PrinterSettings.PrinterName;
                string prtNme = MainMDI.DYMOName;
                Print_label_3 ll = new Print_label_3('Q', "*", "*", "*", prtNme, null, this);
                ll.Wexport();
                MainMDI.OpenKnownFile(lOFName.Text);
            }
            this.Cursor = Cursors.Default;
            ...
            if (conti_ok)
            {
                ...
                if (tAmult.Text != MainMDI.VIDE || tAqty.Text != MainMDI.VIDE || tAup.Text != MainMDI.VIDE || cbCategory.Text != MainMDI.VIDE)
                {
                    for (int s = 0; s < lvQITEMS.SelectedItems.Count; s++)
                    {
                        ...
                        {
                            if (cbCategory.Text != MainMDI.VIDE) lvQITEMS.SelectedItems[s].SubItems[6].Text = cbCategory.Text[6].ToString();
                            ...
                            lvQITEMS.SelectedItems[s].SubItems[7].Text = Convert.ToString(Math.Round(
                                Tools.Conv_Dbl(lvQITEMS.SelectedItems[s].SubItems[4].Text) * 
                                Tools.Conv_Dbl(lvQITEMS.SelectedItems[s].SubItems[5].Text) * 
                                Tools.Conv_Dbl(lvQITEMS.SelectedItems[s].SubItems[3].Text) * 
                                Tools.Conv_Dbl(r_Xchng), MainMDI.Q_NB_DEC_AFF));
                            Tosave = true;
                        }
                    }
                    Ref_ALSTOT('C'); //????
        ...
        private void btnSavMLTP_Click(object sender, EventArgs e)
        {
            ...
            MainMDI.Exec_SQL_JFS("update PSM_Cmpny_CurrMLTP set [" + Curr_SQLMLTP.Trim() + "] = " + tCust_Mult.Text + " where Cpny_ID=" + 
                lcpnyID.Text, " Change Current multiplyer for Company=" + lCpnyName.Text.Replace("'", "''"));
        }
        ...
        private void btnChangMLTP_Click(object sender, EventArgs e)
        {
            ...
            if (MainMDI.profile != 'R')
            {
                ...
                MainMDI.Find_2_Field("SELECT multpl1, multpl1_US,multpl1_EURO FROM PSM_COMPANY inner join  PSM_CmpnyTYPE on PSM_COMPANY.CustomerType= PSM_CmpnyTYPE.CpnyType_ID where Cpny_ID=" + 
                    lcpnyID.Text, ref _stCAN, ref _stUS, ref _stEURO);
        ...
        {
            lCname.Text = MainMDI.Find_One_Field("SELECT PSM_Contacts.Contact_ID FROM PSM_Contacts WHERE ([PSM_Contacts].[First_Name] + ' ' + [PSM_Contacts].[Last_Name])='" + 
                cbContacts.Text.Replace("'", "''") + "' and Company_ID=" + lcpnyID.Text);

            lPGRname.Text = MainMDI.Find_One_Field("SELECT PSM_Contacts.Contact_ID FROM PSM_Contacts WHERE ([PSM_Contacts].[First_Name] + ' ' + [PSM_Contacts].[Last_Name])='" + 
                cbCPmgr.Text.Replace("'", "''") + "' and Company_ID=" + lcpnyID.Text);
        }
        ...
        private void fill_cb_S99()
        {
            ...
            SqlDataReader Oreadr = Ocmd.ExecuteReader();
            while (Oreadr.Read()) 
                MainMDI.add_CB_itm(cbS99, Oreadr[0].ToString(), Oreadr[1].ToString());
            OConn.Close();
        }
        ...
        void fill_dgCpnyQT()
        {
            ...
            QReq.Text = "|" + cbCompanyy.Text + "|" + lAdrs.Text + "|" + lPhone.Text + "|" + lFax.Text + "|" + cbContacts.Text + "|" + 
                cbCPmgr.Text;
        }
        ...
        */
            ...
            switch (MainMDI.User.ToLower())
            {
                ...
                case "hnasrat":
                    string stsql = " SELECT   SPC_LID FROM   PSM_Q_SPCS INNER JOIN  PSM_Q_SOL ON PSM_Q_SPCS.Sol_LID = PSM_Q_SOL.Sol_LID " +
                        " WHERE         (PSM_Q_SOL.I_Quoteid =" + lCurrIQID.Text + ") and (PSM_Q_SPCS.SPC_Name = '" + lCurSPCn.Text + 
                        "') and Sol_Name='" + lCurSoln.Text + "'";
                    string id = MainMDI.Find_One_Field(stsql);
                    if (id != MainMDI.VIDE)
                    {
                        if (MainMDI.Confirm("You Want to delete this Alternative ????")) 
                            MainMDI.Exec_SQL_JFS("delete PSM_Q_SPCS where  SPC_LID=" + id, "delete alternative / delete SPC....");
        ...
        private void p4600P4500ChargerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //Chargerdlg_Cfg_v2 frmchdlg = new Chargerdlg_Cfg_v2('0', "111111", MainMDI._connectionString);
            //this.Hide();
            //frmchdlg.ShowDialog();
            //this.Visible = true;
        }
        ...
        void fill_Quot_agents()
        {
            ...
            string Stsql = "SELECT [A_CMSLID]  ,[AG_Dest]  ,[AG_Infl]   ,[AG_Eng]   ,[AG_PO] FROM [Orig_PSM_FDB].[dbo].[PSM_R_REV_agCMS] where A_CMS_IQID=" + 
                lCurrIQID.Text;
        ...
        bool Save_CMS_Agent()
        {
            ...
            {
                if (lDesti.Text != "" && lInflu.Text != "" && lEng.Text != "" && lPO.Text != "" && (lDesti.Text != MainMDI.VIDE || 
                    lInflu.Text != MainMDI.VIDE || lEng.Text != MainMDI.VIDE || lPO.Text != MainMDI.VIDE))
                {
                    string All = (lDesti.Text != MainMDI.VIDE && lDesti.Text == lInflu.Text && lInflu.Text == lEng.Text && 
                        lEng.Text == lPO.Text) ? lDesti.Text : MainMDI.VIDE; 
                    if (lcms_ag.Text == "")
                    {
                        ...
                        MainMDI.Exec_SQL_JFS("update  PSM_Q_IGen set [AGency]='1' where i_Quoteid=" + lCurrIQID.Text, 
                            " Agents added..(quote).");
                        return true;
                    }
                    else
            ...
            else
            {
                ...
                MainMDI.Exec_SQL_JFS("update PSM_Q_IGen set [AGency]='" + stat + "' where i_Quoteid=" + lCurrIQID.Text, 
                    " No Agents... (quote)");
        ...
        {
            string solId = MainMDI.Find_One_Field("select  Sol_LID from PSM_Q_SOL where I_Quoteid=" + lCurrIQID.Text + " and Sol_Name='" + 
                lCurSoln.Text.Replace("'", "''") + "'");
            FichWord_Config FC = new FichWord_Config(lCurrIQID.Text, solId, cbTerms.Text, lHiDelv.Text, lcurDol.Text, "");
            FC.ShowDialog();
            this.Refresh();
            if (FC.NXT)
        ...
        void email_AGENCIES(FichWord_Config FC)
        {
            ...
            TXT += "\n\n\n\n" + "Best regards";
            MainMDI.Exec_SQL_JFS("update  [PSM_Q_IGen] set [AGmail]='" + SavTXT + " date='" + DateTime.Now.ToShortDateString() + 
                "'  where i_Quoteid=" + lCurrIQID.Text, " save TXT AGency Mail....");
        ...
        //fill quotation company if the user has permissions
        private void PicQT_cpny_Click(object sender, EventArgs e)
        {
            string stSql = $"select username from fill_quote_company_permissions WHERE username = '{MainMDI.User.ToLower()}';";
            bool hasQuoteOrderPermission = false;
            try
            {
                SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
                OConn.Open();
                ...
                while (Oreadr.Read())
                {
                    if ((string)Oreadr["username"] == MainMDI.User.ToLower())
                    {
                        hasQuoteOrderPermission = true;
                    }
                }
            }
            catch (Exception error)
        ...
        private void tls_ACbreaker_Click(object sender, EventArgs e)
        {
            Add_ACDCbreaker("A");
        }

        private void tls_Alarms_Click(object sender, EventArgs e)
        {
            Tollsbar_CLicK(toolbar1_btName_ndx(sender.ToString()));
        }

        private void tls_DCbreaker_Click(object sender, EventArgs e)
        {
            Add_ACDCbreaker("D");
        }

        private void tls_BA_Click(object sender, EventArgs e)
        {

        }

        void add_P580xx(char UpsOrInverter)
        {
            
            /*P850UI_UPS_INV mydlg = new P850UI_UPS_INV(UpsOrInverter);
            this.Hide();
            mydlg.ShowDialog();
            this.Visible = true;
            int efsndx = (UpsOrInverter == 'U') ? 69 : 70;
            if (mydlg.lsave.Text == "Y")
            {
                ItemCount++;
                add_LVO(1, 0, ItemCount.ToString(), MainMDI.arr_EFSdict[efsndx, MainMDI.Lang] + " " + mydlg.lmodel.Text, mydlg.tIQty.Text,
                    mydlg.tSMRK.Text, mydlg.tIPU.Text, mydlg.tIExt.Text, mydlg.tILT.Text, "", " C_MODEL||" + mydlg.lmodel.Text, "A");
                add_LVO(1, 1, "", mydlg.txIO.Text, "", "", "", "", "", "", "", " ");
                add_LVO(1, 1, "", mydlg.txcab.Text, "", "", "", "", "", "", "", " ");
                add_LVO(1, 1, "", mydlg.txbrkr.Text, "", "", "", "", "", "", "", " ");
                add_LVO(1, 1, "", mydlg.txdes.Text, "", "", "", "", "", "", "", " ");
                add_LVO(1, 1, "", mydlg.txbps.Text, "", "", "", "", "", "", "", " ");
                Ref_ALSTOT('A');
            }*/
            
            
            P850UI_UPS_INV_New P850Dialog = new P850UI_UPS_INV_New(tQuoteID.Text, tCust_Mult.Text, UpsOrInverter, cbLang.Text);

            this.Hide();
            P850Dialog.ShowDialog();
            this.Visible = true;

            int efsndx = (UpsOrInverter == 'U') ? 69 : 70;

            if (P850Dialog.saveText == "Y")
            {
                ItemCount++;

                var industrialUPSOptions = new List<string> { "Industrial UPS Model : ", "Modèle UPS industrielle : " };

                double totalPriceP850 = P850Dialog.getTotalPriceP850() * Double.Parse(tCust_Mult.Text);

                string stSql = "SELECT * FROM P850_validation_details  WHERE validationId = @validationId ";
                SqlConnection OConn = new SqlConnection(P850Dialog.M_stCon);
                OConn.Open();
                SqlCommand Ocmd = OConn.CreateCommand();
                Ocmd.CommandText = stSql;
                Ocmd.Parameters.AddWithValue("@validationId", P850Dialog.OldValidationId);
                SqlDataReader Oreadr = Ocmd.ExecuteReader();
                while (Oreadr.Read())
                {
                    string optionReference = Oreadr["optionReference"].ToString();
                    string itemDescription = Oreadr["itemDescription"].ToString();
                    string quantity = Oreadr["quantity"].ToString();
                    string multiplicator = Oreadr["multiplicator"].ToString();
                    string unitPrice = Oreadr["unitPrice"].ToString();
                    string totalPrice = Oreadr["totalPrice"].ToString();
                    string leadTime = Oreadr["leadTime"].ToString();

                    if (industrialUPSOptions.Contains(optionReference))
                    {
                        add_LVO(1, 0, ItemCount.ToString(), MainMDI.arr_EFSdict[efsndx, MainMDI.Lang] + " " + optionReference + itemDescription, quantity,
                        multiplicator, unitPrice, totalPriceP850.ToString(), leadTime, "", " C_MODEL||" + "", "A");
                    }
                    else
                    {
                        add_LVO(1, 1, "", optionReference + itemDescription, quantity, multiplicator, unitPrice, totalPrice, "", "", "", " ");
                    }
        ...
        private void iNVERTERP850iToolStripMenuItem_Click(object sender, EventArgs e)
        {
            add_P580xx('I');
        }
        ...
        void add_MBS()
        {
            ...
            {
                ItemCount++;
                add_LVO(1, 0, ItemCount.ToString(), mydlg.lmodel.Text, mydlg.tIQty.Text, mydlg.tSMRK.Text, mydlg.tIPU.Text, mydlg.tIExt.Text, 
                    mydlg.tILT.Text, "", " C_MODEL||" + mydlg.lmodel.Text, "A");
                Ref_ALSTOT('A');
            }
        }

        private void mBSToolStripMenuItem_Click(object sender, EventArgs e)
        {
            add_MBS();
        }
        ...
        {
            string lcurSOLid = MainMDI.Find_One_Field("select Sol_LID from PSM_Q_SOL where I_Quoteid=" + lCurrIQID.Text + " and Sol_Name='" + 
                lCurSoln.Text.Replace("'", "''") + "'");
            if (lcurSOLid != MainMDI.VIDE)
            {
                string lcurSPCid = MainMDI.Find_One_Field("SELECT  [SPC_LID]  FROM [Orig_PSM_FDB].[dbo].[PSM_Q_SPCS] where Sol_LID=" + 
                    lcurSOLid + " and SPC_Name='" + lCurSPCn.Text.Replace("'", "''") + "'");
        ...
        private void tNB_TextChanged(object sender, EventArgs e)
        {
        ...
        private void STDMultp_TextChanged_1(object sender, EventArgs e)
        {

        }

        private void tCust_Mult_TextChanged_1(object sender, EventArgs e)
        {

        }

        //function that will update the quote multiplier when the user change it
        public bool UpdateMultiplier()
        {
            bool multiplier = false;
            string stSql = "UPDATE PSM_Q_IGen SET Cust_Mult = @multiplier WHERE Quote_ID = @Quote_ID";
            ...
            try
            {
                ...
                SqlCommand Ocmd = OConn.CreateCommand();
                Ocmd.Parameters.AddWithValue("@Quote_ID", tQuoteID.Text);
                Ocmd.Parameters.AddWithValue("@multiplier", tCust_Mult.Text);
                Ocmd.CommandText = stSql;
                SqlDataReader Oreadr = Ocmd.ExecuteReader();
                multiplier = true;
            } 
        ...
        {
            UpdateMultiplier();
            MainMDI.multiplier = tCust_Mult.Text;
            MessageBox.Show("multiplier was successfully updated");
        }

        private void tProjNAME_TextChanged(object sender, EventArgs e)
        {
        ...
        private void test(int l)
        {
            ...
            switch (l) {
                case 0:
                    ...
                    message = "Would you like to have all the additionnal information?";
                    break;
                case 1:
                    title = "Informations Additionnels";
                    message = "Aimeriez-vous avoir toutes les informations additionnels?";
                    break;
                case 2:
                    title = "";
                    message = "";
        ...
        */
        ...
        public string sysProCode(string name)
        {
            string insideNameSysPro = "";
            string stSql = " SELECT [Salesperson] + ' - ' + [Name] AS Expr1 FROM SalSalesperson WHERE SalSalesperson.Branch='C1' And SUBSTRING([Salesperson],1,1)='I' order by Expr1";
            ...
            while (Oreadr.Read())
            {
                insideNameSysPro = Oreadr[0].ToString();
                if (insideNameSysPro.Contains(name)) return insideNameSysPro;
            }
            OConn.Close();
            return name;
        }
        ...
        public void exporterExcel()
        {
            this.Refresh();
            createExcelFile();
            MainMDI.OpenKnownFile(MainMDI.XL_Path + @"\SCRList.xlsx");
        }

        public void createExcelFile()
        {
            //System.IO.File.Delete(MainMDI.XL_Path + @"\SCRList.xlsx");
            debut = true;
            XLName = MainMDI.XL_Path + @"\XL_SCRList.xlsx";

            Object m_objOpt = System.Reflection.Missing.Value;
            if (m_objXL == null) m_objXL = new Excel.Application();

            Excel.Workbook m_objBook = m_objXL.Workbooks.Open(XLName, m_objOpt, m_objOpt, m_objOpt, m_objOpt, m_objOpt, m_objOpt, m_objOpt, 
                m_objOpt, m_objOpt, m_objOpt, m_objOpt, m_objOpt, m_objOpt, m_objOpt);
            Excel.Worksheet m_objSheet = m_objBook.Worksheets[1];

            //Idata = new string[XL_MaxRow, XL_MaxCol];
            //Idata[0, 1] = cbCompany.Text;

            //object[,] objData = new object[XL_MaxRow, XL_MaxCol];
            //objData[0, 1] = cbCompany.Text;


            m_objSheet.Cells[1, 2] = lCpnyName.Text;
            //model
            m_objSheet.Cells[2, 2] = "P4600";
            //project #
            m_objSheet.Cells[3, 2] = tQuoteID.Text;
            m_objSheet.Cells[4, 2] = "S12832-3";

            for (int i = 29; i < 37; i++)
            {
                m_objSheet.Cells[i, 2] = "null";
                m_objSheet.Cells[i, 3] = 0;
                m_objSheet.Cells[i, 4] = 0;
                m_objSheet.Cells[i, 5] = 0;
                m_objSheet.Cells[i, 6] = 5;
                m_objSheet.Cells[i, 7] = "FS";
                m_objSheet.Cells[i, 8] = "Major";
                m_objSheet.Cells[i, 9] = "On";
                m_objSheet.Cells[i, 10] = "Off";
            }


            for (int i = 9; i < 24; i++)
            {
                m_objSheet.Cells[i, 3] = "NA";
                m_objSheet.Cells[i, 6] = "OFF";
                //m_objSheet.Cells[i, 5] = 8;
                /*
                if (i == 15)
                {
                    m_objSheet.Cells[i, 5] = 10;
                }
                if (i == 18 && i <= 23)
                {
                    m_objSheet.Cells[i, 5] = "NA";
                }
                */
            }
            ...
            if (debut)
            {
                ...
                //XLName = path + @"\SCRList.xlsx";
                XLName = MainMDI.XL_Path + @"\SCRList.xlsx";
                try
                {
                    using (Stream stream = new FileStream(XLName, FileMode.Open))
                    {
                        // File/Stream manipulating code here
                        m_objBook.SaveAs(XLName, m_objOpt, m_objOpt, m_objOpt, m_objOpt, m_objOpt, Excel.XlSaveAsAccessMode.xlNoChange, m_objOpt,
                ...
                catch
                {
                    ...
                    MessageBox.Show("The file is already in use please close it to continue");
                }
            }
            else m_objBook.Save();
            ...
            //string value = lvQITEMS.Items[0].SubItems[1].Text;
                ...
                switch (compteur)
                {
                    case 0:
                        ...
                        while (compteur0 < 4)
                        {
                            ...
                            {
                                case 0:
                                    m_objSheet.Cells[row, col] = lCpnyName.Text;
                                    row++;
                                    compteur0++;
                                    break;
                                case 1:

...
namespace PGESCOM
{
    public class Order_TR_XLPrint
    {
        ...
        {
            for (int i = 1; i < 100; i++) 
                for (int j = 0; j < 5; j++) arr_PGC_IO[i, j] = "";

            arr_PGC_IO[0, 0] = In_RDR.LRID.Text + "   S/N: " + In_RDR.TRLsn.Text;
            arr_PGC_IO[0, 1] = In_RDR.lCpnyName.Text;
            string st = (In_RDR.tcust_Model.Text != MainMDI.VIDE && In_RDR.tcust_Model.Text.Length > 2) ? In_RDR.tcust_Model.Text + " (" + 
                In_RDR.PX_Model.Text + ") " : In_RDR.PX_Model.Text;
            arr_PGC_IO[0, 2] = st;

            //lvIOTest
            for (int i = 0; i < In_RDR.lvIOTest.Items.Count; i++)
                for (int j = 0; j < 5; j++) arr_PGC_IO[i + 1, j] = In_RDR.lvIOTest.Items[i].SubItems[j].Text;
            //lvLTest
            int II = In_RDR.lvIOTest.Items.Count + 1;
            for (int i = 0; i < In_RDR.lvLTest.Items.Count; i++)
            {
                for (int j = 0; j < 5; j++) arr_PGC_IO[II, j] = In_RDR.lvLTest.Items[i].SubItems[j].Text;
                II++;
            }
        }
        ...
        {
            for (int i =0; i < 100; i++) 
                for (int j = 0; j < 17; j++) arr_PGC_ALRMs[i, j] = "";
        ...
        private bool tstFileName()
        {
            ...
            while (i < 3)
            {
                ...
                catch (Exception ee)
                {
                    ...
                    DialogResult rep = MessageBox.Show("Excel Testing file is currently Opened, msut be closed !!! (" + i.ToString() + ")", 
                        "EXCEL TESTING File", MessageBoxButtons.RetryCancel);
        ...
        public void XLprocess()
        {
            if (tstFileName())
            {
                ...
                m_objbook = m_objXL.Workbooks.Open(MainMDI.XL_Path + @"\TR_Temp.xltx", 0, true, 5, "", "", true, 
                ...
                m_objbook.SaveAs(MainMDI.XL_Path + @"\TR_Temp_PRINTED.xlsx", m_objOpt, m_objOpt, m_objOpt, m_objOpt, m_objOpt, 
    ...
    public abstract class XL_Sheet
    {
        ...
        {
            foreach (XL_Cell my_cell in my_list) my_Sheet.Cells[my_cell.Row, my_cell.Col] = my_cell.txt;
        }
    ...
    public class IO_PERFO_sheet : XL_Sheet
    {
        ...
        public void fill_arr_CFG(int pageNb)
        {
            for (int ii = 1; ii < 100; ii++) 
                for (int j = 0; j < 5; j++) arr_cfg[ii, j] = "";
            ...
            for (int j = 0; j < 5; j++) arr_cfg[0, j] = arr_vals[j]; 
            string ShhetName = arr_vals[5];

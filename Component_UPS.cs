...
namespace PGESCOM
{
	...
	public class Component_UPS
	{
		...
		public Component_UPS(string x_UPSmodel, string x_phs_out, string x_KVA, string x_ACout, string x_DCbus, string x_phs_in, 
			string x_Acinput, string x_phs_bps, string x_bps_input, long x_Avail_id)
		{
			//
		...
		public void Cal_Induc(string _filter, string _phs, string _VDC, string _IDC, ref string Ind_C, ref string Inductance, 
			ref string Ind_Qty, ref string Capa, ref string Capa_V, ref string Capa_Qty, ref string Resist_ohm, ref string pwrW, 
			ref string Resist_Qty)
		{
			Ind_C = MainMDI.VIDE;
			...
			string stSql = "SELECT * FROM PSM_C_13_Inductance   where Filter='" + _filter + "'   and  PHASE='" + _phs + "'  and  VDC='" + 
				_VDC + "'   and  IDC='" + _IDC + "'";
		...
		{	
			...
			string stSql = (frml_ID == 0) ? "select * from " + In_VcsTblName + "  where VCS_NAME='" + FRML_NAME + "' and (PHS='2' OR PHS='" + 
				P + "')" : "select * from " + In_VcsTblName + " where VCS_ID=" + frml_ID + " and (PHS='2' OR PHS='" + P + "')";
			...
			{
				while (Oreadr.Read()) 
					Res = Deco_Frml(Oreadr["VCS_name"].ToString());
		...
		private string Deco_Frml(string frml)
		{
			...
			if (currFrml != -1)
			{
				...
				switch (Charger_UPS.arr_FRML[currFrml, 2])
				{
					case "S":
						...
						if (Deco_Frml_Res.Substring(0, 1) == "!") 
							Deco_Frml_Res = Deco_Frml(Deco_Frml_Res.Substring(1, Deco_Frml_Res.Length - 1));
			...
			else MessageBox.Show("FRML does not Exist..." + frml);
		...
		private long find_FRML(long Vcs_ID, string VCS_name)
		{
			...
			{ 
				while (i < Charger_UPS.NB_FRML && !found && Charger_UPS.arr_FRML[i, 1] != "")
				{
					...
					{
						if (VCS_name == "*")
							if (Charger_UPS.arr_FRML[i, 0] == Vcs_ID.ToString()) found = true; 
		...
		private string searchTBL6_RST(string tblName, string[] catN, string[] Cn)
		{
			...
			string stSql = "SELECT " + MainMDI.t_SeekTBL6 + ".* FROM " + MainMDI.t_SeekTBL6 + " WHERE ((TABLE_NAME='" + tblName + "')" + 
				whr1 + ")" + ord1;
			SqlConnection OConn = new SqlConnection(MainMDI.M_stCon);
			OConn.Open();
			SqlCommand Ocmd = OConn.CreateCommand();
		...
		public void Disp_FRMLS()
		{
			...
			{
				stOut = stOut + "\n";
				for (int j = 0; j < 5; j++) stOut = stOut + " / " + Charger_UPS.arr_FRML[i, j].ToString();
				if ((i % 20) == 0)
				{
		...
		public void Disp_CMODELS()
		{
			...
			{
				stOut = stOut + "\n";
				for (int j = 0; j < Charger_UPS.NB_MODELS_Flds; j++) stOut = stOut + " / " + Charger_UPS.arr_CModels[i, j].ToString();
				if ((i % 20) == 0)
				{
					MessageBox.Show(stOut);
					stOut = "";
		...
		public string Cal_chrg_CostADO(long compntID, string code)
		{
			...
			//if (AvailId != -1)
				...
				" Where (COMPNT_LIST_UPS.Component_ID = " + compntID + ") and (TBLAVAIL13_UPS.AvailID = " + AvailId + ")" +
				" ORDER BY TBLAVAIL13_UPS.UPS, COMPNT_LIST_UPS.Component_ID";
			...
			while (Oreadr.Read())
			{
				...
				switch (cc)
				{
					...
					case "C":
						...
						Catn_Val_ado(Convert.ToInt32(AvailId), Convert.ToInt32(Oreadr["Value_Type"].ToString()), Oreadr["UPS"].ToString(), 
							ref val1, ref val2, ref val3, ref val4, ref Qty, ref cptPrice_id, ref stout);
						//display results
					...
					case "Z":
						...
						prc = seekPriceADO(Convert.ToInt32(Oreadr["Component_ID"]), Convert.ToInt32(Oreadr["Component_ID"]), val1, val2, val3, 
							"=", "=", "=", code, D_qty); //* Conv_Dbl(rstAvail!Qty)
						break;
					case "%":
						G_PRICE = Convert.ToString(Math.Round(Tools.Conv_Dbl(Oreadr["Value_Type"].ToString()) * 
							Tools.Conv_Dbl(Oreadr["Qty"].ToString()), Charger_UPS.NB_DEC_AFF));
						Real_QTY = Oreadr["Qty"].ToString();
						break;
					case "$":
						string st_dollar = Cal_VCS(Int32.Parse(Oreadr["Value_Type"].ToString()), "*");
						G_PRICE = Convert.ToString(Math.Round(Tools.Conv_Dbl(st_dollar) * Tools.Conv_Dbl(Oreadr["Qty"].ToString()), 
							Charger_UPS.NB_DEC_AFF));
						Real_QTY = Oreadr["Qty"].ToString();
					break;
					default:
		...
		private string seekPriceADO(long Orig_compnt_ID, long Price_compnt_ID, string cat1V, string cat2V, string cat3V, string c1, string c2, 
			string c3, string code, string D_qty)
		{
			/*
			...
			*/
			...
			if (cat1V != Charger_UPS.VIDE && cat2V !=Charger_UPS.VIDE && cat3V != Charger_UPS.VIDE && D_qty != Charger_UPS.VIDE)
			{
				...
				string stSql = "SELECT COMPNT_PRICE_LIST.*, COMPNT_MANUFAC_FAMILY.Pref" +
					" FROM COMPNT_PRICE_LIST INNER JOIN COMPNT_MANUFAC_FAMILY ON COMPNT_PRICE_LIST.compnt_man_Fam_ID = COMPNT_MANUFAC_FAMILY.Compnt_Man_FAM_ID " +
				...
				while (Oreadr.Read())
				{
					...
					if (pref == "0")
					{
						...
						prc = Convert.ToString(Math.Round(Tools.Conv_Dbl(Oreadr["Cost_Price"].ToString()) * Tools.Conv_Dbl(D_qty), 
							Charger_UPS.NB_DEC_AFF)); //G_Price = COST_Price
						majREALV(Convert.ToInt32(Orig_compnt_ID), Oreadr["Cat1_Value"].ToString(), Oreadr["Cat2_Value"].ToString(), 
							Oreadr["Cat3_Value"].ToString(), Oreadr["Cat4_Value"].ToString(), Oreadr["Cat5_Value"].ToString(), 
							Oreadr["Cat6_Value"].ToString(), prc, Desc);
					}
					else
					{
			...
			else
			{
				...
				majREALV(Orig_compnt_ID, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, 
					Charger_UPS.VIDE);
			}
			if (pref != "0")
			{
				...
				if (prc == "0")
				{
					...
					majREALV(Orig_compnt_ID, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, "n/e", 
						Charger_UPS.VIDE);
				}
			}
			else majREALV(Orig_compnt_ID, cat1V, cat2V, cat3V, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, Charger_UPS.VIDE, 
				Charger_UPS.VIDE);
		...
		{	
			...
			Charger_UPS.arr_REALV[i, 1] = v1; CAP1 = v1; CAP2 = v2; CAP3 = v3; CAP4 = v4; CAP5 = v5; CAP6 = v6;
		...
		private void Catn_Val_ado(long Avail_id, int CM_ID, string UPS, ref string v1, ref string v2, ref string v3, ref string v4, 
			ref string Qty, ref long cptPrice_id, ref string stout)
		{
			string st;
			v1 = "0";
			v2 = "0";
			...
			while (Oreadr.Read())
			{
				...
				{
					if (Oreadr["TYPcat1"].ToString().Substring(0, 1) == "C") v1 = Cal_VCS(Convert.ToInt32(Oreadr["VCS1"].ToString()), "*");
					else
					{
				...
				{
					if (Oreadr["TYPcat2"].ToString().Substring(0, 1) == "C") v2 = Cal_VCS(Convert.ToInt32(Oreadr["VCS2"].ToString()), "*");
					else
					{
				...
				{
					if (Oreadr["TYPcat3"].ToString().Substring(0, 1) == "C") v3 = Cal_VCS(Convert.ToInt32(Oreadr["VCS3"].ToString()), "*");
					else
					{
				...
				{
					if (Oreadr["TYPcat4"].ToString().Substring(0, 1) == "C") v4 = Cal_VCS(Convert.ToInt32(Oreadr["VCS4"].ToString()), "*");
					else
					{
